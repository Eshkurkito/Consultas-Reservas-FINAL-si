...
# =============================
    ...
    with st.sidebar:
        ...
        inv_pro = st.number_input(
            "Sobrescribir inventario (nº alojamientos, opcional)",
            min_value=0, value=0, step=1, key="inv_pro"
        )
        st.markdown("—")
        st.caption("🧪 Tabla de sensibilidad alrededor del estado actual")
        pct_adr_span = st.slider("Rango % ADR (±)", min_value=5, max_value=40, value=20, step=5, key="span_adr")
        pct_occ_span = st.slider("Rango % Ocupación (±)", min_value=2, max_value=20, value=10, step=2, key="span_occ")
        step_adr = st.select_slider("Paso ADR", options=[1,2,5], value=5, key="step_adr")
        step_occ = st.select_slider("Paso Ocupación", options=[1,2,2.5,5], value=2, key="step_occ")
    ...
    occ_base = float(total_pro.get("ocupacion_pct", 0.0)) / 100.0  # 0..1
    adr_base = float(total_pro.get("adr", 0.0))
    revpar_base = float(total_pro.get("revpar", 0.0))

    st.subheader("📌 Estado actual del periodo")
    a1, a2, a3 = st.columns(3)
    a1.metric("Ocupación", f"{total_pro.get('ocupacion_pct', 0.0):.2f}%")
    a2.metric("ADR (€)", f"{adr_base:.2f}")
    a3.metric("RevPAR (€)", f"{revpar_base:.2f}")
    st.caption("RevPAR ya está normalizado por inventario (ingresos / noches disponibles).")

    # === Sensibilidad ADR × Ocupación ===
    import numpy as np
    import pandas as pd

    def make_range(span_pct: float, step_pct: float):
        lo = -span_pct/100.0
        hi = +span_pct/100.0
        steps = int(round((hi - lo) / (step_pct/100.0))) + 1
        vals = np.linspace(lo, hi, steps)
        return 1.0 + vals

    adr_mults = make_range(pct_adr_span, step_adr)
    occ_mults = make_range(pct_occ_span, step_occ)

    grid = []
    row_labels = []
    col_labels = [f"{(m-1)*100:+.0f}%" for m in adr_mults]
    for m_occ in occ_mults:
        row = []
        for m_adr in adr_mults:
            adr = adr_base * m_adr
            occ = max(min(occ_base * m_occ, 1.0), 0.0)
            revpar = adr * occ
            row.append(revpar)
        grid.append(row)
        row_labels.append(f"{(m_occ-1)*100:+.0f}%")

    df_sens = pd.DataFrame(grid, index=row_labels, columns=col_labels)
    df_sens.index.name = "Δ Ocupación"
    df_sens.columns.name = "Δ ADR"
    df_sens_rounded = df_sens.round(2)

    st.subheader("🧪 Tabla de sensibilidad (RevPAR, €)")
    st.dataframe(df_sens_rounded, use_container_width=True)

    csv_sens = df_sens_rounded.to_csv().encode("utf-8-sig")
    st.download_button("📥 Descargar sensibilidad (CSV)", data=csv_sens,
                       file_name="sensibilidad_revpar.csv", mime="text/csv")

    heat_df = df_sens_rounded.reset_index().melt(id_vars="Δ Ocupación", var_name="Δ ADR", value_name="RevPAR (€)")
    heat = (
        alt.Chart(heat_df)
        .mark_rect()
        .encode(
            x=alt.X("Δ ADR:N", title="Δ ADR (%)"),
            y=alt.Y("Δ Ocupación:N", title="Δ Ocupación (%)"),
            color=alt.Color("RevPAR (€):Q", title="RevPAR (€)"),
            tooltip=[
                alt.Tooltip("Δ ADR:N", title="Δ ADR"),
                alt.Tooltip("Δ Ocupación:N", title="Δ Ocup."),
                alt.Tooltip("RevPAR (€):Q", format=".2f")
            ],
        )
    ...
    text = (
        ...
        .encode(x="Δ ADR:N", y="Δ Ocupación:N", text=alt.Text("RevPAR (€):Q", format=".0f"))
    )
    st.altair_chart(heat + text, use_container_width=True)

    st.divider()
    st.subheader("🎯 Objetivos y qué necesitaríamos")

    c1, c2 = st.columns(2)
    with c1:
        target_revpar = st.number_input("Objetivo RevPAR (€)", min_value=0.0, value=max(revpar_base, 1.0), step=1.0)
        lock_choice = st.radio("Mantener fijo", ["Ocupación", "ADR"], horizontal=True)
    with c2:
        st.caption("Calcula la condición necesaria manteniendo fijo uno de los dos factores.")

    need_adr = need_occ = np.nan
    if lock_choice == "Ocupación":
        need_adr = (target_revpar / occ_base) if occ_base > 0 else np.nan
        msg = f"Con ocupación {total_pro.get('ocupacion_pct', 0.0):.2f}%, necesitas **ADR ≈ {need_adr:.2f} €** para lograr RevPAR {target_revpar:.2f} €."
    else:
        need_occ = (target_revpar / adr_base) if adr_base > 0 else np.nan
        msg = f"Con ADR {adr_base:.2f} €, necesitas **Ocupación ≈ {need_occ*100:.2f}%** para lograr RevPAR {target_revpar:.2f} €."

    st.markdown(msg)
    st.caption(
        "Sugerencia: si la demanda no es muy elástica a precio, subir ADR (aunque pierdas ~1–2 pts de ocupación) "
        "puede mejorar RevPAR; si el mercado es sensible, pequeñas bajadas de ADR que ganen muchas noches también lo mejoran."
    )

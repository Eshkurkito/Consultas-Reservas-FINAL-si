...
# =============================
# MODO: ADR óptimo (beta)
# =============================
elif mode == "ADR óptimo (beta)":
    if raw is None:
        st.stop()

    st.subheader("🎯 ADR óptimo (beta)")
    st.caption("Estimación de elasticidad precio→ocupación a partir del histórico y simulación de RevPAR para hallar el ADR que maximiza ingresos por unidad disponible.")

    with st.sidebar:
        st.header("Parámetros")
        opt_start, opt_end = period_inputs("Inicio del periodo (objetivo)", "Fin del periodo (objetivo)",
                                           date(date.today().year, date.today().month, 1),
                                           (pd.Timestamp.today().to_period("M").end_time).date(),
                                           "adr_opt_target")
        cutoff_opt = st.date_input("Fecha de corte (estado actual)", value=date.today(), key="cutoff_opt")

        st.markdown("**Ventana histórica (entrenamiento)**")
        train_col1, train_col2 = st.columns(2)
        with train_col1:
            train_start = st.date_input("Inicio histórico", value=(date.today() - timedelta(days=180)), key="train_start")
        with train_col2:
            train_end = st.date_input("Fin histórico", value=(date.today() - timedelta(days=1)), key="train_end")

        props_opt = st.multiselect("Filtrar alojamientos (opcional)",
                                   options=sorted(raw["Alojamiento"].unique()),
                                   default=[], key="props_adropt")
        inv_opt = st.number_input("Sobrescribir inventario (opcional)", min_value=0, value=0, step=1, key="inv_adropt")

        mult_min = st.number_input("Multiplicador mínimo", min_value=0.3, max_value=1.0, value=0.7, step=0.05)
        mult_max = st.number_input("Multiplicador máximo", min_value=1.0, max_value=2.0, value=1.3, step=0.05)
        n_points = st.slider("Resolución simulación (puntos)", min_value=11, max_value=101, value=31, step=2)

        add_costs = st.checkbox("Incluir costes variables/commissiones", value=False)
        cost_per_night = 0.0
        comm_pct = 0.0
        if add_costs:
            cost_per_night = st.number_input("Coste variable por noche (€)", min_value=0.0, value=0.0, step=1.0)
            comm_pct = st.number_input("Comisión OTA (%)", min_value=0.0, max_value=40.0, value=0.0, step=0.5)

        run_opt = st.button("Calcular ADR óptimo", type="primary")

    if run_opt:
        inv_override = int(inv_opt) if inv_opt > 0 else None
        _, tot_now = compute_kpis(raw, pd.to_datetime(cutoff_opt),
                                  pd.to_datetime(opt_start), pd.to_datetime(opt_end),
                                  inventory_override=inv_override,
                                  filter_props=props_opt if props_opt else None)
        adr_base = float(tot_now["adr"])
        if adr_base <= 0:
            st.warning("No hay ADR actual calculable en el periodo objetivo.")
            st.stop()

        alpha, beta, n_used = _fit_price_elasticity_from_history(raw,
                                                                 pd.to_datetime(train_start),
                                                                 pd.to_datetime(train_end),
                                                                 props_opt if props_opt else None,
                                                                 inv_override)

        sim = _simulate_revpar_curve(adr_base, alpha, beta, n_points=n_points,
                                     mult_min=float(mult_min), mult_max=float(mult_max))

        if add_costs:
            comm_frac = comm_pct / 100.0
            goppar = sim["occ_frac"] * (sim["ADR"] * (1 - comm_frac) - cost_per_night)
            sim["GOPPAR"] = goppar

        target_col = "GOPPAR" if (add_costs and "GOPPAR" in sim.columns) else "RevPAR"
        vmax = sim[target_col].max()
        i_opt = int(sim[target_col].idxmax())
        adr_opt = float(sim.loc[i_opt, "ADR"])

        thresh = 0.98 * vmax
        near = sim[sim[target_col] >= thresh]
        adr_low = float(near["ADR"].min())
        adr_high = float(near["ADR"].max())

        st.metric("ADR base actual (€)", f"{adr_base:.2f}")
        st.metric("ADR óptimo (€)", f"{adr_opt:.2f}")
        st.metric("Rango óptimo (±2%)", f"{adr_low:.0f}–{adr_high:.0f}")

        st.line_chart(sim.set_index("ADR")[[target_col, "occ_frac"]], height=320)
        st.download_button("📥 Descargar simulación (CSV)",
                           data=sim.to_csv(index=False).encode("utf-8-sig"),
                           file_name="adr_optimo_simulacion.csv",
                           mime="text/csv")
...
# ADR ÓPTIMO — helpers
# =============================
def _fit_price_elasticity_from_history(raw: pd.DataFrame,
                                       train_start: pd.Timestamp,
                                       train_end: pd.Timestamp,
                                       props: Optional[List[str]],
                                       inv_override: Optional[int]) -> Tuple[float, float, int]:
    """Ajusta: ln(occ_frac) = alpha + beta * ln(ADR) con datos diarios históricos."""
    cutoff_hist = pd.to_datetime(train_end) + pd.Timedelta(days=1)
    df_hist = daily_series(raw, cutoff_hist, train_start, train_end, props, inv_override)
    if df_hist.empty:
        return np.log(0.5), -0.7, 0

    df = df_hist[(df_hist["adr"] > 0) & (df_hist["ocupacion_pct"] > 0) & (df_hist["ocupacion_pct"] < 100)].copy()
    if df.empty or len(df) < 15:
        return np.log(max(0.5, df_hist["ocupacion_pct"].mean()/100.0 if "ocupacion_pct" in df_hist else 0.5)), -0.7, len(df)

    df["occ_frac"] = df["ocupacion_pct"] / 100.0
    df["ln_occ"] = np.log(df["occ_frac"].clip(1e-6, 1-1e-6))
    ...
    if denom <= 0:
        return np.log(df["occ_frac"].mean()), -0.7, len(df)

    beta = ((x - x_mean)*(y - y_mean)).sum() / denom
    alpha = y_mean - beta * x_mean
    beta = float(np.clip(beta, -2.5, -0.05))
    return float(alpha), float(beta), int(len(df))


def _simulate_revpar_curve(adr_base: float,
                           alpha: float,
                           beta: float,
                           n_points: int = 31,
                           mult_min: float = 0.7,
                           mult_max: float = 1.3) -> pd.DataFrame:
    """Simula RevPAR frente a ADR con el modelo log-lineal."""
    if adr_base <= 0:
        adr_base = 1.0
    mults = np.linspace(mult_min, mult_max, n_points)
    adrs = adr_base * mults
    occ_frac = np.exp(alpha) * (adrs ** beta)
    occ_frac = np.clip(occ_frac, 0.0, 1.0)
    revpar = adrs * occ_frac
    df = pd.DataFrame({"mult": mults, "ADR": adrs, "occ_frac": occ_frac, "RevPAR": revpar})
    return df

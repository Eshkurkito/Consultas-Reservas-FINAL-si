...
def _kai_cdm_pro_analysis(tot_now: dict, tot_ly_cut: dict, tot_ly_final: dict, pace: dict, price_ref_p50: float|None) -> str:
    ...
    if price_ref_p50 is not None and np.isfinite(price_ref_p50):
        delta_vs_p50 = _kai_pct(adr_now, price_ref_p50)
        lines.append(f"- ADR actual {adr_now:.2f}€ vs P50 {price_ref_p50:.2f}€ → {_kai_fmt_pct(delta_vs_p50)}")

    # Acciones genéricas
    lines.append("**Acciones recomendadas**")
    if d_occ is not None and not pd.isna(d_occ):
        if d_occ < 0:
            lines.append("🔴 Ocupación por debajo de LY: revisar tarifas mínimas y abrir estancias cortas.")
        elif d_occ > 0.05:
            lines.append("🟢 Ocupación por encima de LY: mantener estrategia y proteger ADR.")
    if d_adr is not None and not pd.isna(d_adr):
        if d_adr < -0.05:
            lines.append("🔴 ADR significativamente por debajo de LY: revisar descuentos y visibilidad.")
        elif d_adr > 0.05:
            lines.append("🟢 ADR por encima de LY: aprovechar en picos, revisar cancelaciones.")
    ...
    if isinstance(pace, dict):
        ...
        if np.isfinite(need) and np.isfinite(typ):
            if need > typ * 1.25:
                lines.append(f"🔴 Pickup muy retrasado: faltan {need:.0f} noches vs típico {typ:.0f}.")
            elif need < typ * 0.75:
                lines.append(f"🟢 Pickup adelantado: solo faltan {need:.0f} noches vs típico {typ:.0f}.")
    ...
    if years_back > 0:
        for k in range(1, years_back + 1):
            ...
            _bp_k, tot_k = compute_kpis(
                ...
                start_k,
                end_k,
                (int(inv_pro) if inv_pro > 0 else None),          # << evita NameError fuera del if
                (props_pro if props_pro else None)
            )
            adr_k = float(tot_k["adr"])
            adr_hist.append((f"LY-{k}" if k > 1 else "LY", adr_k))

    # Usa ADR desde 'tot' (no 'adr' suelto) para evitar NameError
    adr_now_val = float(tot.get("adr", np.nan))
    cols_top = st.columns(3)
    cols_top[0].metric("ADR Actual", f"{adr_now_val:.2f} €" if np.isfinite(adr_now_val) else "—")

    adr_ly = None
    if adr_hist:
        label_ly, adr_ly = adr_hist[0]
        if np.isfinite(adr_ly):
            delta_abs = adr_now_val - adr_ly
            delta_pct = (adr_now_val / adr_ly - 1.0) * 100.0 if adr_ly != 0 else np.nan
            cols_top[1].metric(
                f"ADR {label_ly}",
                f"{adr_ly:.2f} €",
                delta=(f"{delta_abs:+.2f} € ({delta_pct:+.1f}%)" if np.isfinite(delta_pct) else f"{delta_abs:+.2f} €")
            )
        else:
        ...
        if len(adr_hist) > 1:
            ...
            cols_top[2].metric(f"ADR {label_more}", f"{adr_more:.2f} €" if np.isfinite(adr_more) else "—")
    else:
        cols_top[1].metric("ADR LY", "—")
        cols_top[2].metric("ADR LY-2", "—")

    # Semáforo vs LY (si hay comparativa)
    if adr_ly is not None and np.isfinite(adr_ly) and np.isfinite(adr_now_val) and adr_ly != 0:
        delta_pct = (adr_now_val / adr_ly - 1.0) * 100.0

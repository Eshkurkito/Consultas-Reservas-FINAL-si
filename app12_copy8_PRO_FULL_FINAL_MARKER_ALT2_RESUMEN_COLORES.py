...
# ---------- Resumen Comparativo (por alojamiento) ----------
    ...
    # Δ (para análisis y formato condicional)
    ...
    # Descarga Excel con colores (xlsxwriter si está disponible; si no, openpyxl sin colores)
    import io
    buffer = io.BytesIO()
    try:
        import xlsxwriter
    ...
    with pd.ExcelWriter(buffer, engine=engine) as writer:
        ...
        if engine == "xlsxwriter":
            ...
            for j, col in enumerate(resumen.columns):
                try:
                    width = int(min(44, max(12, resumen[col].astype(str).str.len().max() if not resumen.empty else 12)))
                except Exception:
                    width = 18
                ws.set_column(j, j, width)
            ...
            # Aplicar formatos numéricos base
            cols = {name: idx for idx, name in enumerate(resumen.columns)}
            for name, fmt in [
                ("ADR actual", fmt_adr),
                ("ADR LY", fmt_adr),
            ...
            # Helper: índice → letra de columna
            def col_letter(c):
                s = ""
                n = c
                while True:
            ...
            # Rango de datos (sin cabecera): filas 2..N en Excel
            nrows = len(resumen) + 1  # +1 por cabecera
            r1, r2 = 1, nrows  # 0-indexed (fila 2..N+1 en Excel)

            # Reglas: verde si actual >= LY, rojo si actual < LY
            for a_name, b_name in [
                ...
                if a_name in cols and b_name in cols:
                    ...
                    # Fórmulas relativas: usar la fila de inicio (2 en Excel) y Excel ajusta por fila
                    ws.conditional_format(r1, ca, r2, ca, {
                        "type": "formula",
                        "criteria": f"=${la}2>=${lb}2",
                        "format": fmt_green,
                    })
                    ws.conditional_format(r1, ca, r2, ca, {
                        "type": "formula",
            ...
            # Reglas sobre Δ (positivo verde, negativo rojo)
            ...
            # openpyxl: dejamos sin colores (pandas/openpyxl no ofrece reglas aquí fácilmente sin dependencia extra)
            pass

...
# --- Safe defaults (avoid NameError if UI not used in a code path) ---
try:
    use_declared_inventory
except NameError:
    use_declared_inventory = False
    inv_file_cur = None
    inv_file_ly = None
    declared_list_now = None


def _read_inventory_declared(file, period_start=None, period_end=None, col_name="Alojamiento"):
    ...
    for _c in ("Desde", "Hasta"):
        if _c in df.columns:
            df[_c] = _pd.to_datetime(df[_c], errors="coerce")
    ...
    vals = sorted(_pd.unique(df[col_name].dropna().astype(str)))
...
def compute_kpis(
    ...
    declared_inventory: Optional[List[str]] = None,
) -> Tuple[pd.DataFrame, dict]:
    
    """KPIs vectorizados sin expandir noche a noche (robusto)."""
    # Filtro por fecha de corte y props
    ...
    if declared_inventory:
        inv = len(set(str(x).strip() for x in declared_inventory))
    if inventory_override and inventory_override > 0:
        inv = int(inventory_override)

    days = int((period_end - period_start).days) + 1
    noches_disponibles = max(inv, 0) * max(days, 0)

    # Si no hay reservas tras el corte
    if df_cut.empty:
        by_prop = pd.DataFrame(columns=["Alojamiento", "Noches ocupadas", "Ingresos", "ADR"])
        if declared_inventory:
            _decl_idx = sorted(set(str(x).strip() for x in declared_inventory))
            if _decl_idx:
                by_prop = (by_prop.set_index("Alojamiento")
                                  .reindex(_decl_idx, fill_value=0)
                                  .reset_index())
        tot = {
            "noches_ocupadas": 0,
    ...
    # Reindex con inventario declarado (inyectar 0s)
    if declared_inventory:
        _decl_idx = sorted(set(str(x).strip() for x in declared_inventory))
        if _decl_idx:
            by_prop = (by_prop.set_index("Alojamiento")
    ...
    noches_ocupadas = int(by_prop["Noches ocupadas"].sum()) if not by_prop.empty else 0
    ingresos = float(by_prop["Ingresos"].sum()) if not by_prop.empty else 0.0
...
if mode == "Consulta normal":
    ...
    with st.sidebar:
        ...
        if compare_normal:
            ...
            with c3:
                ref_year = st.number_input("Año de referencia (LY)", min_value=2000, max_value=2100, value=date.today().year - 1, step=1, key="ref_year_normal")
            with c4:
        ...
        use_declared_inventory = st.checkbox(
            "Usar inventario declarado (contar pisos sin reservas)",
            value=True,
            help="Sube un CSV/XLSX con columna 'Alojamiento' (y opcionalmente 'Activo', 'Desde', 'Hasta')."
        )
        inv_file_cur = inv_file_ly = None
        if use_declared_inventory:
            st.caption("Inventario declarado para el periodo actual y opcionalmente LY.")
            c_inv1, c_inv2 = st.columns(2)
            with c_inv1:
                inv_file_cur = st.file_uploader("Inventario ACTUAL (CSV/XLSX)", type=["csv","xlsx"], key="inv_cur")
            with c_inv2:
                inv_file_ly = st.file_uploader("Inventario LY (opcional)", type=["csv","xlsx"], key="inv_ly")
        declared_list_now = _read_inventory_declared(inv_file_cur, start_normal, end_normal) if use_declared_inventory else None
    # Cálculo base
    try:
        by_prop_n, total_n = compute_kpis(
            ...
            declared_inventory=declared_list_now
        )
    except Exception as e:
        st.error(f"Error en compute_kpis: {type(e).__name__}: {e}")
        by_prop_n, total_n = pd.DataFrame(), {
            "noches_ocupadas": 0,

...
# ===== IA Pricing — helpers añadidos por Kai =====
import numpy as np
import pandas as pd
...
def apply_rounding_array(v, rule: str):
    v = v.astype(float)
    if rule == "A 1€":
        return np.round(v)
    if rule == "A 5€":
        return (5 * np.round(v/5.0)).astype(float)
    if rule == "Terminar en ,99":
        return np.floor(v) + 0.99
    return v

@st.cache_data(show_spinner=False)
def expand_nightly_fast(df: pd.DataFrame) -> pd.DataFrame:
    x = df.dropna(subset=["Fecha entrada","Fecha salida","Alojamiento","Precio"]).copy()
    ...
    base = x.loc[rep, ["Alojamiento","Fecha entrada","Fecha salida","Fecha alta","adr_reserva"]].copy()
    offsets = x["los"].apply(lambda n: np.arange(n)).explode().values
    base["noche"] = (base["Fecha entrada"].values.astype("datetime64[D]") + offsets).astype("datetime64[ns]")
    base["dow"] = pd.to_datetime(base["noche"]).dt.weekday.map({0:"Lunes",1:"Martes",2:"Miércoles",3:"Jueves",4:"Viernes",5:"Sábado",6:"Domingo"})
    base["lead"] = (pd.to_datetime(base["noche"]).dt.normalize() - base["Fecha alta"]).dt.days.clip(lower=0)
    base["lead_bucket"] = base["lead"].apply(lead_bucket)
    return base[["Alojamiento","noche","dow","lead_bucket","adr_reserva"]]

def estimate_elasticities_shrunk(nightly: pd.DataFrame, inv_per_prop: dict, inv_default: int = 1,
                                 prior_e: float = -1.2, k_shrink: float = 20.0) -> pd.DataFrame:
    ddf = nightly.groupby(["Alojamiento","dow","lead_bucket","noche"]).agg(
        p_obs=("adr_reserva","median"),
        booked=("adr_reserva","size")
    ).reset_index()
    ddf["inv"] = ddf["Alojamiento"].map(inv_per_prop).fillna(inv_default).astype(float)
    ddf["rate"] = (ddf["booked"] / ddf["inv"]).clip(lower=1e-6, upper=0.98)
    ddf = ddf.dropna(subset=["p_obs"]).copy()

    base = ddf.groupby(["Alojamiento","dow","lead_bucket"]).agg(
        p_ref=("p_obs","median"),
        booking_prob=("rate","median"),
        n_pts=("rate","size")
    ).reset_index()

    p90 = ddf.groupby(["Alojamiento","dow","lead_bucket"])["p_obs"].quantile(0.90).reset_index(name="p90_price")
    base = base.merge(p90, on=["Alojamiento","dow","lead_bucket"], how="left")

    el_rows = []
    for (a,w,lb), g in ddf.groupby(["Alojamiento","dow","lead_bucket"]):
        g = g[(g["p_obs"]>0) & (g["rate"]>0)].copy()
        n = len(g)
        if n >= 3 and g["p_obs"].std() > 1e-6:
            x = np.log(g["p_obs"].values)
            y = np.log(g["rate"].values)
            try:
        ...
        if not np.isfinite(beta):
            ...
            w_sh = n / (n + k_shrink)
            beta = w_sh * beta + (1 - w_sh) * prior_e
        beta = float(np.clip(beta, -3.0, -0.2))  # endurecido
        el_rows.append({"Alojamiento":a,"dow":w,"lead_bucket":lb,"elasticity":beta})
    el_df = pd.DataFrame(el_rows)

    out = base.merge(el_df, on=["Alojamiento","dow","lead_bucket"], how="left")
    out["p_ref"] = out["p_ref"].fillna(out["p_ref"].median()).fillna(60.0)
    out["booking_prob"] = out["booking_prob"].fillna(out["booking_prob"].median()).fillna(0.05)
    out["elasticity"] = out["elasticity"].fillna(prior_e)
    out["p90_price"] = out["p90_price"].fillna(out["p_ref"]*1.25)
    return out

def compute_remaining_from_otb(raw, cutoff, props_sel, inv_map, start, end):
    df = raw[(pd.to_datetime(raw["Fecha alta"]) <= pd.to_datetime(cutoff))].dropna(subset=["Fecha entrada","Fecha salida","Alojamiento"]).copy()
    df["Fecha entrada"] = pd.to_datetime(df["Fecha entrada"]).dt.normalize()
    df["Fecha salida"]  = pd.to_datetime(df["Fecha salida"]).dt.normalize()
    sold = []
    for _, r in df.iterrows():
        s = max(r["Fecha entrada"], pd.to_datetime(start))
        e = min(r["Fecha salida"],  pd.to_datetime(end) + pd.Timedelta(days=1))
        if s < e and r["Alojamiento"] in props_sel:
            for d in pd.date_range(s, e - pd.Timedelta(days=1), freq="D"):
                sold.append((r["Alojamiento"], d.normalize()))
    if sold:
        occ = pd.DataFrame(sold, columns=["Alojamiento","Fecha"]).value_counts().reset_index(name="sold")
    else:
        occ = pd.DataFrame(columns=["Alojamiento","Fecha","sold"])
    occ["inv"] = occ["Alojamiento"].map(inv_map).fillna(1).astype(int)
    occ["remaining"] = np.clip(occ["inv"] - occ["sold"], 0, None)
    return occ[["Alojamiento","Fecha","remaining"]]

def closed_form_optimal_price(pmin, pmax, p_ref, demand_ref, elasticity, remaining, event_mult, risk_demand_mult=1.0):
    Dref = demand_ref * np.clip(event_mult, 0.1, 10.0) * risk_demand_mult
    e = elasticity
    with np.errstate(divide='ignore', invalid='ignore'):
        ratio = np.clip(remaining / np.clip(Dref, 1e-9, None), 1e-9, 1e9)
        p_break = p_ref * np.power(ratio, 1.0/np.where(e==0, -1e-6, e))
    cand = np.stack([pmin, pmax, p_break], axis=0)
    cand = np.clip(cand, pmin, pmax)
    with np.errstate(divide='ignore', invalid='ignore'):
        demand_cand = Dref * np.power(np.clip(cand / np.clip(p_ref,1e-6,None), 1e-6, 1e9), e)
    demand_cand = np.clip(demand_cand, 0.0, remaining)
    rev_cand = cand * demand_cand
    idx = np.argmax(rev_cand, axis=0)
    p_star = cand[idx, np.arange(cand.shape[1])]
    rev_star = rev_cand[idx, np.arange(cand.shape[1])]
    return p_star.astype(float), rev_star.astype(float), p_break.astype(float)

def choose_with_fill_target(p_star, rev_star, pmin, pmax, p_ref, demand_ref, elasticity, remaining, event_mult, fill_target=None):
    if fill_target is None:
        return p_star, rev_star, None, None
    with np.errstate(divide='ignore', invalid='ignore'):
        rhs = np.clip(fill_target * remaining / np.clip(demand_ref * event_mult, 1e-9, None), 1e-9, 1e9)
        p_fill = p_ref * np.power(rhs, 1.0/np.where(elasticity==0, -1e-6, elasticity))
    p_fill = np.clip(p_fill, pmin, pmax)
    with np.errstate(divide='ignore', invalid='ignore'):
        dem_fill = np.clip(demand_ref * event_mult * np.power(np.clip(p_fill/np.clip(p_ref,1e-6,None), 1e-6, 1e9), elasticity), 0.0, remaining)
    rev_fill = p_fill * dem_fill
    mask = np.isfinite(rev_fill) & (rev_fill >= 0.97 * rev_star)
    p_out  = np.where(mask, p_fill, p_star)
    r_out  = np.where(mask, rev_fill, rev_star)
    return p_out, r_out, p_fill, rev_fill
# ===== Fin helpers Kai =====
...
# =============================
# MODO: IA Pricing (beta)
# =============================
elif mode == "IA Pricing (beta)":
    if raw is None:
        st.stop()

    st.subheader("🧠 IA Pricing (beta)")
    st.caption("Motor de optimización de precio basado en elasticidad por segmento (Alojamiento × DOW × Lead).")

    # --------------------------
    # Sidebar (parámetros)
    # --------------------------
    with st.sidebar:
        st.header("Parámetros IA Pricing")
        # Rango de fechas a optimizar
        today = pd.Timestamp.today().normalize()
        opt_start = st.date_input("Inicio optimización", value=(today.to_period("M").start_time).date(), key="ia_opt_start")
        opt_end   = st.date_input("Fin optimización", value=(today.to_period("M").end_time).date(), key="ia_opt_end")
        cutoff    = st.date_input("Fecha de corte (para lead actual)", value=today.date(), key="ia_cutoff")

        # Alojamientos
        props_all = sorted(raw["Alojamiento"].dropna().unique())
        props_sel = st.multiselect("Alojamientos a optimizar", options=props_all, default=props_all, key="ia_props")

        # Inventario (si no tienes por alojamiento)
        inv_default = st.number_input("Inventario por alojamiento (si no definido)", min_value=1, value=1, step=1, key="ia_inv")

        # Reglas de precio
        st.markdown("—")
        st.markdown("**Límites & redondeo**")
        pmin = st.number_input("ADR mínimo (€)", min_value=1.0, value=40.0, step=1.0, key="ia_pmin")
        pmax = st.number_input("ADR máximo (€)", min_value=10.0, value=300.0, step=5.0, key="ia_pmax")
        pstep = st.selectbox("Paso (€)", [1, 2, 5, 10], index=2, key="ia_pstep")
        round_rule = st.selectbox("Redondeo", ["Sin redondeo", "A 1€", "A 5€", "Terminar en ,99"], index=3, key="ia_round")

        # Riesgo: ajusta magnitud de elasticidad y demanda base
        st.markdown("—")
        risk = st.selectbox("Perfil de riesgo", ["Conservador", "Neutral", "Agresivo"], index=1, key="ia_risk")
        risk_el_mult = {"Conservador": 0.7, "Neutral": 1.0, "Agresivo": 1.3}[risk]
        risk_demand_mult = {"Conservador": 0.9, "Neutral": 1.0, "Agresivo": 1.1}[risk]

        st.markdown("—")
        st.caption("Eventos (uplift%) se leen del gestor de **Eventos & Festivos**.")
        run_ia = st.button("Calcular IA Pricing", type="primary")
    ...
    def _lead_bucket(d: int) -> str:
        # buckets: 0-3 / 4-7 / 8-14 / 15-30 / 31-60 / 61-120 / 120+
        if d <= 3: return "0-3"
        if d <= 7: return "4-7"
        if d <= 14: return "8-14"
        if d <= 30: return "15-30"
    ...
    @st.cache_data(show_spinner=False)
    def _expand_nightly_reservations(df: pd.DataFrame) -> pd.DataFrame:
        """ Explota cada reserva a noches individuales con ADR por noche aproximado. """
        dfx = df.dropna(subset=["Fecha entrada", "Fecha salida", "Alojamiento"]).copy()
        dfx["Fecha entrada"] = pd.to_datetime(dfx["Fecha entrada"]).dt.normalize()
        dfx["Fecha salida"] = pd.to_datetime(dfx["Fecha salida"]).dt.normalize()
        dfx["Fecha alta"] = pd.to_datetime(dfx["Fecha alta"]).dt.normalize()
        dfx["los"] = (dfx["Fecha salida"] - dfx["Fecha entrada"]).dt.days.clip(lower=1)
        dfx["adr_reserva"] = dfx["Precio"] / dfx["los"]
        rows = []
        for _, r in dfx.iterrows():
            nights = pd.date_range(r["Fecha entrada"], r["Fecha salida"] - pd.Timedelta(days=1), freq="D")
            for d in nights:
                rows.append({
                    "Alojamiento": r["Alojamiento"],
                    "noche": d.normalize(),
                    "fecha_alta": r["Fecha alta"],
                    "adr_reserva": float(r["adr_reserva"]),
                })
        if not rows:
            return pd.DataFrame(columns=["Alojamiento","noche","fecha_alta","adr_reserva"])
        out = pd.DataFrame(rows)
        out["dow"] = out["noche"].dt.weekday
        return out

    @st.cache_data(show_spinner=False)
    def _build_segment_stats(nightly: pd.DataFrame, inv_per_prop: dict) -> pd.DataFrame:
        """
        Calcula por segmento (Alojamiento × DOW × lead_bucket):
          - booking_prob (reservas/noches_expuestas)
          - p_ref (mediana de ADR histórico del segmento)
          - elasticidad (regresión log-log simple)
        """
        if nightly.empty:
            return pd.DataFrame(columns=["Alojamiento","dow","lead_bucket","booking_prob","p_ref","elasticity"])

        # lead histórico de cada reserva: (noche - fecha_alta)
        nightly["lead_hist"] = (nightly["noche"] - nightly["fecha_alta"]).dt.days.clip(lower=0)
        nightly["lead_bucket"] = nightly["lead_hist"].apply(_lead_bucket)

        # noches expuestas estimadas: inventario por aloj × nº de fechas (por segmento)
        # contamos fechas únicas por (aloj, dow, lead_bucket)
        base = nightly.groupby(["Alojamiento","dow","lead_bucket"]).agg(
            booked=("noche", "count"),
            p_ref=("adr_reserva", "median")
        ).reset_index()

        # Para exposiciones, aproximamos por inventario * nº de fechas únicas del segmento
        # Primero cuántas fechas únicas (por seg)
        uniq_dates = nightly.groupby(["Alojamiento","dow","lead_bucket"])["noche"].nunique().reset_index(name="unique_days")
        base = base.merge(uniq_dates, on=["Alojamiento","dow","lead_bucket"], how="left")

        # Inventario por alojamiento
        base["inv"] = base["Alojamiento"].map(inv_per_prop).fillna(inv_default)
        base["exposed_nights"] = base["inv"] * base["unique_days"]

        # booking_prob
        # para estabilidad, cap en [1e-6, 0.98]
        base["booking_prob"] = (base["booked"] / base["exposed_nights"]).clip(lower=1e-6, upper=0.98)

        # Elasticidad: regresión log(booked_rate) ~ log(price) por segmento
        # Para tener variación, agregamos por noche la media ADR y booked flag (=1). Aquí solo tenemos noches reservadas.
        # Mejor: agrupar por (aloj, dow, lead, fecha) y usar la mediana ADR de reservas para esa fecha como precio observado.
        ddf = nightly.groupby(["Alojamiento","dow","lead_bucket","noche"]).agg(
            p_obs=("adr_reserva","median"),
            booked=("adr_reserva","size")
        ).reset_index()
        # Convertimos booked a tasa observada sobre inv
        ddf["inv"] = ddf["Alojamiento"].map(inv_per_prop).fillna(inv_default)
        ddf["rate"] = (ddf["booked"] / ddf["inv"]).clip(lower=1e-6, upper=0.98)

        # elasticidad por seg vía OLS en log-log, si hay variedad suficiente (>5 puntos y varianza)
        el_rows = []
        for (a, w, lb), g in ddf.groupby(["Alojamiento","dow","lead_bucket"]):
            g = g.dropna(subset=["p_obs","rate"]).copy()
            if len(g) >= 6 and g["p_obs"].std() > 1e-6:
                x = np.log(np.clip(g["p_obs"].values, 1e-6, None))
                y = np.log(np.clip(g["rate"].values, 1e-6, 0.98))
                # recta y = alpha + beta*x
                try:
                    beta = np.polyfit(x, y, deg=1)[0]
                    # beta suele ser <0; si sale >0 forzamos a negativo leve
                    if beta >= -0.05:
                        beta = -0.05
                    el_rows.append({"Alojamiento":a,"dow":w,"lead_bucket":lb,"elasticity":float(beta)})
                except Exception:
                    el_rows.append({"Alojamiento":a,"dow":w,"lead_bucket":lb,"elasticity":np.nan})
            else:
                el_rows.append({"Alojamiento":a,"dow":w,"lead_bucket":lb,"elasticity":np.nan})
        el_df = pd.DataFrame(el_rows)
        ...
        # p_ref fallback
        out["p_ref"] = out["p_ref"].fillna(out["p_ref"].median()).fillna(60.0)
        # elasticidad fallback
        out["elasticity"] = out["elasticity"].fillna(-0.8)
        # ajustar por perfil de riesgo
        out["elasticity"] = out["elasticity"] * risk_el_mult

        # Demanda base ajustada por riesgo
        out["booking_prob"] = (out["booking_prob"] * risk_demand_mult).clip(lower=1e-6, upper=0.98)
        return out[["Alojamiento","dow","lead_bucket","booking_prob","p_ref","elasticity"]]

    def _event_multiplier_for_day(events_daily: pd.DataFrame, d: pd.Timestamp) -> float:
        if events_daily is None or events_daily.empty:
            return 1.0
        r = events_daily.loc[events_daily["fecha"] == d.date()]
        if r.empty:
            return 1.0
        uplift = float(r["uplift_pct"].iloc[0])
        return 1.0 + uplift/100.0
    ...
    if run_ia:
        # 1) Preparar insumos
        # Inventario por alojamiento (si tienes un mapeo propio, cámbialo aquí)
        inv_map = {a: inv_default for a in props_sel}

        # Eventos diarios
        try:
            ev_daily = expand_events_by_day(load_events_csv(EVENTS_CSV_PATH))
        except Exception:
            ev_daily = pd.DataFrame(columns=["fecha","uplift_pct","origen","prioridad"])

        # Explota reservas históricas a noches
        nightly = _expand_nightly_reservations(raw)

        # 2) Segment stats (booking_prob, p_ref, elasticity)
        seg = _build_segment_stats(nightly, inv_map)

        # 3) Construir marco de optimización (cada celda: aloj × día)
        days = pd.date_range(pd.to_datetime(opt_start), pd.to_datetime(opt_end), freq="D")
        if len(days) == 0 or not props_sel:
            st.info("Selecciona al menos un alojamiento y un rango de fechas válido.")
            st.stop()

        # OTB por día y alojamiento (noches ya vendidas)
        # Marcamos noches reservadas del histórico (OTB real para fechas futuras puede que esté parcial)
        otb = nightly.groupby(["Alojamiento","noche"]).size().reset_index(name="booked_nights")
        # Calendario base
        cal_rows = []
        cutoff_ts = pd.to_datetime(cutoff).normalize()
        for a in props_sel:
            for d in days:
                cal_rows.append({"Alojamiento": a, "Fecha": d.normalize()})
        cal = pd.DataFrame(cal_rows)
        cal = cal.merge(otb.rename(columns={"noche":"Fecha"}), on=["Alojamiento","Fecha"], how="left")
        cal["booked_nights"] = cal["booked_nights"].fillna(0).astype(float)
        cal["inv"] = cal["Alojamiento"].map(inv_map).fillna(inv_default).astype(float)
        cal["remaining"] = np.maximum(cal["inv"] - cal["booked_nights"], 0.0)

        # Segmento para cada fila
        cal["dow"] = cal["Fecha"].dt.weekday
        cal["lead_current"] = (cal["Fecha"] - cutoff_ts).dt.days.clip(lower=0)
        cal["lead_bucket"] = cal["lead_current"].apply(_lead_bucket)

        cal = cal.merge(seg, on=["Alojamiento","dow","lead_bucket"], how="left")

        # Fallbacks si hay segmentos sin match
        cal["booking_prob"] = cal["booking_prob"].fillna(seg["booking_prob"].median() if not seg.empty else 0.05).fillna(0.05)
        cal["p_ref"] = cal["p_ref"].fillna(seg["p_ref"].median() if not seg.empty else 60.0).fillna(60.0)
        cal["elasticity"] = cal["elasticity"].fillna(-0.8)

        # Multiplicador de evento
        if not ev_daily.empty:
            mul_ev = []
            for d in cal["Fecha"]:
                mul_ev.append(_event_multiplier_for_day(ev_daily, pd.to_datetime(d)))
            cal["m_event"] = mul_ev
        else:
            cal["m_event"] = 1.0

        # Demanda base (esperanza de noches vendidas a p_ref)
        # demanda_ref = inv * booking_prob * m_event, acotada por remaining
        cal["demand_ref"] = np.minimum(cal["inv"] * cal["booking_prob"] * cal["m_event"], cal["remaining"])

        # 4) Optimización por rejilla de precios
        price_grid = np.arange(pmin, pmax + 1e-6, pstep, dtype=float)
        results = []
        for _, r in cal.iterrows():
            if r["remaining"] <= 0.0:
                results.append({
                    "Alojamiento": r["Alojamiento"], "Fecha": r["Fecha"], "Precio_opt": np.nan,
                    "Ingresos_opt": 0.0, "p_ref": r["p_ref"], "elasticity": r["elasticity"],
                    "remaining": r["remaining"], "m_event": r["m_event"], "booking_prob": r["booking_prob"]
                })
                continue

            el = float(r["elasticity"])
            pref = float(r["p_ref"])
            dem_ref = float(r["demand_ref"])

            # curva de demanda: demand(p) = dem_ref * (p/pref)^el, cap a remaining
            demand_curve = dem_ref * (price_grid / max(pref, 1e-6)) ** el
            demand_curve = np.clip(demand_curve, 0.0, r["remaining"])
            revenue_curve = price_grid * demand_curve

            # elegir precio óptimo (con redondeo)
            idx = int(np.argmax(revenue_curve))
            p_opt_raw = price_grid[idx]
            rev_opt = float(revenue_curve[idx])

            p_opt = _redondear(p_opt_raw)
            # recalcular ingresos con p_opt redondeado (manteniendo coherencia)
            demand_opt = dem_ref * (p_opt / max(pref, 1e-6)) ** el
            demand_opt = float(np.clip(demand_opt, 0.0, r["remaining"]))
            rev_opt_round = p_opt * demand_opt

            results.append({
                "Alojamiento": r["Alojamiento"],
                "Fecha": r["Fecha"],
                "Precio_opt": float(p_opt),
                "Ingresos_opt": float(rev_opt_round),
                "p_ref": float(pref),
                "elasticity": float(el),
                "remaining": float(r["remaining"]),
                "m_event": float(r["m_event"]),
                "booking_prob": float(r["booking_prob"]),
            })

        out = pd.DataFrame(results)
        if out.empty:
            st.info("No hay filas para optimizar con los filtros actuales.")
            st.stop()

        st.success(f"Optimizado {len(out):,} celdas de precio.".replace(",", "."))

        # 5) Vista rápida + descarga
        st.markdown("### Resultados")
        st.dataframe(
            out.sort_values(["Alojamiento","Fecha"]).head(1000),
            use_container_width=True
        )

        # Descargas
        csv_bytes = out.to_csv(index=False).encode("utf-8-sig")
        st.download_button("⬇️ Descargar recomendaciones (CSV)", data=csv_bytes, file_name="ia_pricing.csv", mime="text/csv")

        # 6) Auditoría: curva Ingreso vs Precio para una fecha/aloj
        st.markdown("### Auditoría: Ingreso vs Precio")
        c1, c2 = st.columns(2)
        with c1:
            aloj_audit = st.selectbox("Alojamiento", options=props_sel, key="ia_audit_aloj")
        with c2:
            # Fechas disponibles para ese alojamiento
            dates_avail = sorted(out.loc[out["Alojamiento"] == aloj_audit, "Fecha"].unique())
            date_audit = st.selectbox("Fecha", options=dates_avail, key="ia_audit_date")

        # Recalcular curva para esa celda y graficar
        row = cal[(cal["Alojamiento"] == aloj_audit) & (cal["Fecha"] == pd.to_datetime(date_audit))].head(1)
        if row.empty:
            st.info("No se encontró la celda seleccionada.")
        else:
            r = row.iloc[0]
            el = float(r["elasticity"])
            pref = float(r["p_ref"])
            ...
            price_grid = np.arange(pmin, pmax + 1e-6, pstep, dtype=float)
            demand_curve = dem_ref * (price_grid / max(pref, 1e-6)) ** el
            demand_curve = np.clip(demand_curve, 0.0, remaining)
            revenue_curve = price_grid * demand_curve

            df_curve = pd.DataFrame({"Precio": price_grid, "Ingresos": revenue_curve, "Demanda": demand_curve})
            # punto óptimo
            idx = int(np.argmax(df_curve["Ingresos"].values))
            p_star = float(price_grid[idx])
            rev_star = float(revenue_curve[idx])

            # marca el precio recomendado redondeado
            rec = out[(out["Alojamiento"] == aloj_audit) & (out["Fecha"] == pd.to_datetime(date_audit))]
            p_rec = float(rec["Precio_opt"].iloc[0]) if not rec.empty else p_star
            dem_rec = float(dem_ref * (p_rec / max(pref, 1e-6)) ** el)
            dem_rec = float(np.clip(dem_rec, 0.0, remaining))
            rev_rec = p_rec * dem_rec

            base = alt.Chart(df_curve).encode(x=alt.X("Precio:Q", title="Precio (€)"))
            line_rev = base.mark_line(strokeWidth=2).encode(
                y=alt.Y("Ingresos:Q", title="Ingresos (€)"),
                tooltip=[
                    alt.Tooltip("Precio:Q", format=".2f"),
                    alt.Tooltip("Ingresos:Q", format=".2f"),
                    alt.Tooltip("Demanda:Q", format=".2f"),
                ],
            )
            point_star = alt.Chart(pd.DataFrame({"Precio":[p_star],"Ingresos":[rev_star]})).mark_point(size=80).encode(
                x="Precio:Q", y="Ingresos:Q"
            )
            point_rec = alt.Chart(pd.DataFrame({"Precio":[p_rec],"Ingresos":[rev_rec]})).mark_point(size=100, shape="diamond").encode(
                x="Precio:Q", y="Ingresos:Q"
            )

            st.altair_chart((line_rev + point_star + point_rec).properties(height=320), use_container_width=True)

            c3, c4, c5, c6 = st.columns(4)
            c3.metric("Precio ref (P_ref)", f"{pref:.2f} €")
            c4.metric("Elasticidad", f"{el:.2f}")
            c5.metric("Remaining", f"{remaining:.0f}")
            c6.metric("Demanda@P_ref", f"{dem_ref:.2f}")

            st.caption("🔎 Punto redondo = óptimo teórico en la rejilla. 🔷 Rombo = precio recomendado tras redondeo y límites.")

    else:
        st.info("Configura parámetros y pulsa **Calcular IA Pricing**.")
    ...
    st.subheader("🧠 IA Pricing (beta) — versión optimizada")
    st.caption("Elasticidad con shrinkage, remaining real por OTB, óptimo sin grid y objetivo de ocupación opcional.")
    ...
    with st.sidebar:
        ...
        opt_start = st.date_input("Inicio optimización", value=(today.to_period("M").start_time).date(), key="ia_opt_start")
        opt_end   = st.date_input("Fin optimización", value=(today.to_period("M").end_time).date(), key="ia_opt_end")
        cutoff    = st.date_input("Fecha de corte (para OTB/lead)", value=today.date(), key="ia_cutoff")
        ...
        pmin = st.number_input("ADR mínimo (€)", min_value=1.0, value=40.0, step=1.0, key="ia_pmin")
        pmax = st.number_input("ADR máximo (€)", min_value=10.0, value=300.0, step=5.0, key="ia_pmax")
        pstep = st.selectbox("Paso (€)", [1, 2, 5, 10], index=2, key="ia_pstep")  # no usado en el motor cerrado, pero mantenemos UI
        round_rule = st.selectbox("Redondeo", ["Sin redondeo", "A 1€", "A 5€", "Terminar en ,99"], index=3, key="ia_round")

        st.markdown("—")
        risk = st.selectbox("Perfil de riesgo", ["Conservador", "Neutral", "Agresivo"], index=1, key="ia_risk")
        fill_t = st.slider("Objetivo de ocupación diaria (opcional)", 0, 100, 0, step=5, help="Si >0, intentará alcanzar al menos este % de fill si no sacrifica >3% de ingresos.", key="ia_fill")
    ...
    if run_ia:
        # Inventario
        inv_map = {a: int(inv_default) for a in props_sel}
        ...
        # Expand & elasticidades con shrinkage
        nightly = expand_nightly_fast(raw)
        nightly = nightly[nightly["Alojamiento"].isin(props_sel)]
        el_table = estimate_elasticities_shrunk(nightly, inv_map, inv_default=int(inv_default))

        # Tabla objetivo
        days = pd.date_range(pd.to_datetime(opt_start), pd.to_datetime(opt_end), freq="D")
        grid = pd.MultiIndex.from_product([props_sel, days], names=["Alojamiento","Fecha"]).to_frame(index=False)
        grid["dow"] = grid["Fecha"].dt.weekday.map({0:"Lunes",1:"Martes",2:"Miércoles",3:"Jueves",4:"Viernes",5:"Sábado",6:"Domingo"})
        grid["lead"] = (grid["Fecha"].dt.normalize() - pd.to_datetime(cutoff)).dt.days.clip(lower=0)
        grid["lead_bucket"] = grid["lead"].apply(lead_bucket)

        params = grid.merge(el_table, on=["Alojamiento","dow","lead_bucket"], how="left")
        # fallbacks
        params["p_ref"] = params["p_ref"].fillna(params["p_ref"].median()).fillna(60.0)
        params["booking_prob"] = params["booking_prob"].fillna(0.05)
        params["elasticity"] = params["elasticity"].fillna(-1.2)

        # Inventario y remaining real (OTB a cutoff)
        params["inv"] = params["Alojamiento"].map(inv_map).fillna(int(inv_default)).astype(float)
        occ = compute_remaining_from_otb(raw, pd.to_datetime(cutoff), props_sel, inv_map, pd.to_datetime(opt_start), pd.to_datetime(opt_end))
        params = params.merge(occ, on=["Alojamiento","Fecha"], how="left")
        params["remaining"] = params["remaining"].fillna(params["inv"]).astype(float)

        # Riesgo
        risk_el_mult     = {"Conservador": 0.7, "Neutral": 1.0, "Agresivo": 1.3}[risk]
        risk_demand_mult = {"Conservador": 0.9, "Neutral": 1.0, "Agresivo": 1.1}[risk]
        params["elasticity"] = (params["elasticity"] * risk_el_mult).clip(upper=-1.05)  # fuerza sensibilidad suficiente

        # Demanda ref y suelo
        params["booking_prob"] = params["booking_prob"].clip(lower=0.02)
        params["demand_ref"]   = params["booking_prob"] * params["inv"]

        # Eventos
        if ev_daily is not None and not ev_daily.empty:
            ev_daily = ev_daily.copy()
            ev_daily["fecha"] = pd.to_datetime(ev_daily["fecha"]).dt.normalize()
            m_uplift = ev_daily.drop_duplicates(subset=["fecha"]).set_index("fecha")["uplift_pct"]
            params["event_mult"] = params["Fecha"].dt.normalize().map(lambda d: 1.0 + float(m_uplift.get(d, np.nan))/100.0 if d in m_uplift and pd.notna(m_uplift[d]) else 1.0)
        else:
            params["event_mult"] = 1.0

        # Topes adaptativos por P90 histórico
        p_cap_pctile = params["p90_price"].fillna(params["p_ref"]*1.25).to_numpy(float)
        N = len(params)
        pmin_arr = np.full(N, float(pmin))
        pmax_arr = np.minimum(np.full(N, float(pmax)), p_cap_pctile)

        # Óptimo sin grid
        p_star, rev_star, p_break = closed_form_optimal_price(
            pmin_arr, pmax_arr,
            params["p_ref"].to_numpy(float),
            params["demand_ref"].to_numpy(float),
            params["elasticity"].to_numpy(float),
            params["remaining"].to_numpy(float),
            params["event_mult"].to_numpy(float),
            risk_demand_mult=risk_demand_mult
        )

        # Objetivo de ocupación (opcional)
        fill_target = (fill_t/100.0) if fill_t and fill_t>0 else None
        p_star2, rev_star2, p_fill, rev_fill = choose_with_fill_target(
            p_star, rev_star, pmin_arr, pmax_arr,
            params["p_ref"].to_numpy(float),
            params["demand_ref"].to_numpy(float),
            params["elasticity"].to_numpy(float),
            params["remaining"].to_numpy(float),
        ...
        # Demanda y ocupación a precio final teórico
        with np.errstate(divide='ignore', invalid='ignore'):
            dem_star = params["demand_ref"].to_numpy(float) * params["event_mult"].to_numpy(float) * np.power(
                np.clip(p_star2/np.clip(params["p_ref"].to_numpy(float),1e-6,None), 1e-6, 1e9),
                params["elasticity"].to_numpy(float)
            )
        dem_star = np.clip(dem_star, 0.0, params["remaining"].to_numpy(float))

        # Redondeo final
        price_opt = apply_rounding_array(np.clip(p_star2, pmin_arr, pmax_arr), round_rule)

        out = pd.DataFrame({
            "Alojamiento": params["Alojamiento"].values,
            "Fecha": params["Fecha"].values,
            "Precio_opt": price_opt.astype(float),
            "p_star": p_star.astype(float),
            "Ingresos_star": rev_star2.astype(float),
            "Demanda_est": dem_star.astype(float),
            "Ocupacion_est_%": np.where(params["remaining"].to_numpy(float)>0, dem_star/np.clip(params["remaining"].to_numpy(float),1e-6,None)*100.0, 0.0),
            "p_ref": params["p_ref"].to_numpy(float),
            "Elasticidad": params["elasticity"].to_numpy(float),
            "Demanda_ref": params["demand_ref"].to_numpy(float),
            "Remaining": params["remaining"].to_numpy(float),
            "p_break": p_break.astype(float),
            "p90_cap": p_cap_pctile.astype(float),
        }).sort_values(["Alojamiento","Fecha"])

        st.success(f"Calculadas {len(out):,} celdas.".replace(",", "."))
        st.dataframe(out.head(500), use_container_width=True)

        # Export
        st.download_button(
            "📥 Exportar IA Pricing (CSV)",
            data=out.to_csv(index=False).encode("utf-8-sig"),
            file_name="ia_pricing.csv",
            mime="text/csv",
            key="ia_csv"
        )

        # Mini auditoría (selección)
        with st.expander("🔎 Auditoría rápida"):
            aloj = st.selectbox("Alojamiento", sorted(out["Alojamiento"].unique()))
            fecha = st.date_input("Fecha", value=pd.to_datetime(out.loc[out["Alojamiento"]==aloj,"Fecha"].min()).date())
            row = out[(out["Alojamiento"]==aloj) & (out["Fecha"]==pd.to_datetime(fecha))]
            if not row.empty:
                r = row.iloc[0]
                st.metric("Elasticidad", f"{float(r['Elasticidad']):.2f}")
                st.metric("p_ref", f"{float(r['p_ref']):.2f} €")
                st.metric("p_break", f"{float(r['p_break']):.2f} €")
                st.metric("Fill est.", f"{float(r['Ocupacion_est_%']):.1f} %")
                st.metric("Remaining", f"{int(r['Remaining']):d}")
            else:

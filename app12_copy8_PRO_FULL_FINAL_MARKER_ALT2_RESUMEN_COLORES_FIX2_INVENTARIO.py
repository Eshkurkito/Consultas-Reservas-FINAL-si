...
# >>> INVENTARIO DECLARADO – HELPERS
def _read_inventory(file, period_start=None, period_end=None, col_name="Alojamiento"):
    if file is None:
        return None
    ...
    if name.endswith(".csv"):
        ...
        df = _pd.read_excel(file)
    assert col_name in df.columns, f"El inventario debe tener columna '{col_name}'."
    df[col_name] = df[col_name].astype(str).str.strip()

    # Filtrar activo si existe
    if "Activo" in df.columns:
        df = df[df["Activo"].fillna(True).astype(bool)]

    # Filtrado por vigencias si se aportan
    for col in ["Desde", "Hasta"]:
        if col in df.columns:
            df[col] = _pd.to_datetime(df[col], errors="coerce")

    if period_start is not None and "Desde" in df.columns:
        df = df[(df["Desde"].isna()) | (df["Desde"] <= _pd.to_datetime(period_start))]
    if period_end is not None and "Hasta" in df.columns:
        df = df[(df["Hasta"].isna()) | (df["Hasta"] >= _pd.to_datetime(period_end))]

    inv_list = sorted(df[col_name].dropna().unique().tolist())
    return inv_list if inv_list else None

def _safe_div(a, b):
    try:
        return a / b if (b and b != 0 and not pd.isna(b)) else float("nan")
    except Exception:
        return float("nan")

def _safe_pct(delta, base):
    try:
        return (delta / base) * 100 if (base and base != 0 and not pd.isna(base)) else float("nan")
    except Exception:
        return float("nan")
...
if mode == "Consulta normal":
    ...
    # Inventario declarado (opcional)
    use_declared_inventory = st.checkbox(
        "Usar inventario declarado (contar pisos con 0 noches)",
        value=False, key="use_declared_inventory_normal",
        help="Sube un listado de alojamientos activos en el periodo. Se contarán aunque no hayan vendido."
    )
    inv_file_cur = None
    inv_file_ly = None
    if use_declared_inventory:
        st.caption("Sube CSV/XLSX con columna 'Alojamiento' (y opcionalmente 'Activo', 'Desde', 'Hasta').")
        cdi1, cdi2 = st.columns(2)
        with cdi1:
            inv_file_cur = st.file_uploader("Inventario ACTUAL", type=["csv","xlsx"], key="inv_cur_normal")
        with cdi2:
            inv_file_ly = st.file_uploader("Inventario LY (opcional)", type=["csv","xlsx"], key="inv_ly_normal")
    ...
    # --- Inyección de inventario declarado (ACTUAL) ---
    if "use_declared_inventory_normal" in st.session_state and st.session_state["use_declared_inventory_normal"]:
        inv_cur_list = _read_inventory(inv_file_cur, period_start=start_normal, period_end=end_normal)
        if inv_cur_list:
            # reindexar detalle por alojamiento para incluir 0s
            if not by_prop_n.empty:
                by_prop_n = (by_prop_n.set_index("Alojamiento")
                                        .reindex(inv_cur_list, fill_value=0)
                                        .reset_index().rename(columns={"index":"Alojamiento"}))
            else:
                by_prop_n = pd.DataFrame({"Alojamiento": inv_cur_list, "Noches ocupadas": 0, "Ingresos": 0.0, "ADR": 0.0})
            # recalcular totales y métricas dependientes de inventario
            days_norm = (pd.to_datetime(end_normal) - pd.to_datetime(start_normal)).days + 1
            cur_active_units = len(inv_cur_list)
            cur_ing = float(by_prop_n["Ingresos"].sum())
            cur_nts = int(by_prop_n["Noches ocupadas"].sum())
            noches_disponibles = cur_active_units * days_norm
            total_n["noches_disponibles"] = noches_disponibles
            total_n["ingresos"] = cur_ing
            total_n["noches_ocupadas"] = cur_nts
            total_n["adr"] = (cur_ing / cur_nts) if cur_nts > 0 else 0.0
            total_n["ocupacion_pct"] = (cur_nts / noches_disponibles * 100) if noches_disponibles > 0 else 0.0
            total_n["revpar"] = (cur_ing / noches_disponibles) if noches_disponibles > 0 else 0.0
        else:
            cur_active_units = by_prop_n["Alojamiento"].nunique() if not by_prop_n.empty else 0
    else:
        cur_active_units = by_prop_n["Alojamiento"].nunique() if not by_prop_n.empty else 0
    ...
    if compare_normal:
        ...
        # --- Inyección de inventario declarado (LY) ---
        ly_active_units = None
        if "use_declared_inventory_normal" in st.session_state and st.session_state["use_declared_inventory_normal"]:
            inv_ly_list = _read_inventory(inv_file_ly, period_start=start_cmp, period_end=end_cmp)
            if inv_ly_list:
                if not _bp_c.empty:
                    _bp_c = (_bp_c.set_index("Alojamiento")
                                   .reindex(inv_ly_list, fill_value=0)
                                   .reset_index().rename(columns={"index":"Alojamiento"}))
                else:
                    _bp_c = pd.DataFrame({"Alojamiento": inv_ly_list, "Noches ocupadas": 0, "Ingresos": 0.0, "ADR": 0.0})
                # recomputar totales LY con inventario declarado
                days_ly = (pd.to_datetime(end_cmp) - pd.to_datetime(start_cmp)).days + 1
                ly_active_units = len(inv_ly_list)
                ly_ing = float(_bp_c["Ingresos"].sum())
                ly_nts = int(_bp_c["Noches ocupadas"].sum())
                noches_disponibles_ly = ly_active_units * days_ly
                total_cmp["noches_disponibles"] = noches_disponibles_ly
                total_cmp["ingresos"] = ly_ing
                total_cmp["noches_ocupadas"] = ly_nts
                total_cmp["adr"] = (ly_ing / ly_nts) if ly_nts > 0 else 0.0
                total_cmp["ocupacion_pct"] = (ly_nts / noches_disponibles_ly * 100) if noches_disponibles_ly > 0 else 0.0
                total_cmp["revpar"] = (ly_ing / noches_disponibles_ly) if noches_disponibles_ly > 0 else 0.0
            else:
                ly_active_units = _bp_c["Alojamiento"].nunique() if not _bp_c.empty else 0
        else:
            ly_active_units = _bp_c["Alojamiento"].nunique() if not _bp_c.empty else 0

        # --- Panel de normalización por inventario (densidades y YoY ajustado) ---
        cur_rev = float(by_prop_n["Ingresos"].sum()) if not by_prop_n.empty else 0.0
        cur_nts = float(by_prop_n["Noches ocupadas"].sum()) if not by_prop_n.empty else 0.0
        ly_rev = float(_bp_c["Ingresos"].sum()) if not _bp_c.empty else 0.0
        ly_nts = float(_bp_c["Noches ocupadas"].sum()) if not _bp_c.empty else 0.0

        cur_rev_per_unit = _safe_div(cur_rev, cur_active_units)
        cur_nts_per_unit = _safe_div(cur_nts, cur_active_units)
        ly_rev_per_unit  = _safe_div(ly_rev,  ly_active_units)
        ly_nts_per_unit  = _safe_div(ly_nts,  ly_active_units)

        inventario_objetivo_default = int(cur_active_units or 1)
        normalizar_inv = st.checkbox(
            "Normalizar YoY por inventario (escala LY al inventario objetivo)",
            value=True, key="norm_inv_panel"
        )
        if normalizar_inv:
            inventario_objetivo = st.number_input(
                "Inventario objetivo para comparación",
                min_value=1, value=inventario_objetivo_default, step=1, key="inv_target_panel"
            )
        else:
            inventario_objetivo = inventario_objetivo_default

        ly_rev_adj = ly_rev_per_unit * inventario_objetivo if normalizar_inv else ly_rev
        ly_nts_adj = ly_nts_per_unit * inventario_objetivo if normalizar_inv else ly_nts

        yoy_rev_pct_raw = _safe_pct(cur_rev - ly_rev, ly_rev)
        yoy_nts_pct_raw = _safe_pct(cur_nts - ly_nts, ly_nts)
        yoy_rev_pct_adj = _safe_pct(cur_rev - ly_rev_adj, ly_rev_adj)
        yoy_nts_pct_adj = _safe_pct(cur_nts - ly_nts_adj, ly_nts_adj)

        st.subheader("Normalización por inventario")
        cni1, cni2, cni3 = st.columns(3)
        with cni1: st.metric("Activos (actual/LY)", f"{cur_active_units} / {ly_active_units}")
        with cni2: st.metric("YoY Ingresos (crudo)", f"{(0.0 if pd.isna(yoy_rev_pct_raw) else yoy_rev_pct_raw):.1f}%")
        with cni3: st.metric("YoY Ingresos (ajustado)", f"{(0.0 if pd.isna(yoy_rev_pct_adj) else yoy_rev_pct_adj)::.1f}%")

        df_inv_norm = pd.DataFrame({
            "Métrica": [
                "Activos_actual","Activos_LY","Inventario_objetivo",
                "Ingresos_actual","Ingresos_LY","Ingresos_LY_ajustados",
                "YoY_ingresos_%","YoY_ingresos_ajustado_%",
                "Noches_actual","Noches_LY","Noches_LY_ajustadas",
                "YoY_noches_%","YoY_noches_ajustado_%",
                "Ingresos_por_apto_actual","Ingresos_por_apto_LY","YoY_ingresos_por_apto_%",
                "Noches_por_apto_actual","Noches_por_apto_LY","YoY_noches_por_apto_%"
            ],
            "Valor": [
                cur_active_units, ly_active_units, inventario_objetivo,
                cur_rev, ly_rev, ly_rev_adj,
                yoy_rev_pct_raw, yoy_rev_pct_adj,
                cur_nts, ly_nts, ly_nts_adj,
                yoy_nts_pct_raw, yoy_nts_pct_adj,
                cur_rev_per_unit, ly_rev_per_unit, _safe_pct(cur_rev_per_unit-ly_rev_per_unit, ly_rev_per_unit),
                cur_nts_per_unit, ly_nts_per_unit, _safe_pct(cur_nts_per_unit-ly_nts_per_unit, ly_nts_per_unit)
            ]
        })
        st.dataframe(df_inv_norm, use_container_width=True)
        st.download_button(
            "📥 Descargar normalización por inventario (CSV)",
            data=df_inv_norm.to_csv(index=False).encode("utf-8-sig"),
            file_name="normalizacion_inventario.csv",
            mime="text/csv"
        )

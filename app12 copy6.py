...
# =============================
    ...
    with st.sidebar:
        ...
        kpis_sel = st.multiselect(
            ...
            default=["Ocupación %"],
            help="Puedes seleccionar 1, 2 o 3 KPIs a la vez.",
            key="kpis_evo",
        )
    ...
    if run_evo:
        ...
        if cut_start_ts > cut_end_ts:
            ...
            # =========================
            # Serie ACTUAL (por fecha de corte)
            # =========================
            rows_e = []
            ...
            if compare_e:
                ...
                for c in pd.date_range(cut_start_prev, cut_end_prev, freq="D"):
                    ...
                    rows_prev.append({
                        # alineamos +1 año para que caiga en las mismas fechas del gráfico actual
            ...
            # Config KPI → columna + eje preferente
            KPI_CFG = {
            ...
            # Hover cómodo (no exige pasar por la línea)
            hover = alt.selection_single(fields=["Corte"], on="mousemove", nearest=True, empty="none")
            ...
            for idx, label in enumerate(kpis_sel):
                cfg = KPI_CFG[label]
                col = cfg["col"]
                color = COLORS[label]

                # Ejes: primer KPI crea el eje izquierdo; las series € crean un eje derecho (solo una vez).
                if idx == 0:
                    ...
                    if cfg["side"] == "right":
                        if not right_axis_set:
                            ...
                            y_enc = alt.Y(f"{col}:Q", axis=None)  # comparte eje derecho existente
                    else:
                        y_enc = alt.Y(f"{col}:Q", axis=None)      # comparte eje izquierdo sin duplicarlo
                ...
                points = (
                    alt.Chart(df_evo)
                    .mark_circle(size=50, color=color)
                    .encode(x="Corte:T", y=alt.Y(f"{col}:Q"))
                    .transform_filter(hover)
                )

                layers += [line, points]

                # Serie LY (si existe), sin duplicar ejes
                prev_col = f"{col}_prev"
                if compare_e and (prev_col in df_evo.columns):
                    line_prev = (
                        alt.Chart(df_evo)
                        .mark_line(strokeWidth=1.5, strokeDash=[4, 3], color=color, opacity=0.35)
                        .encode(x="Corte:T", y=alt.Y(f"{prev_col}:Q", axis=None))
                    )
                    points_prev = (
                        alt.Chart(df_evo)
                        .mark_circle(size=35, color=color, opacity=0.3)
                        .encode(x="Corte:T", y=alt.Y(f"{prev_col}:Q"))
                        .transform_filter(hover)
                    )
                    layers += [line_prev, points_prev]

            # Regla vertical + tooltip con todos los KPIs (y LY si está)
            ...
            vline = (
                alt.Chart(df_evo)
                .mark_rule(color="#666")
                .encode(x="Corte:T", tooltip=tooltips)
                .transform_filter(hover)
            )

            # Capturador invisible para activar hover sin dibujar puntos extra
            capture = (
                alt.Chart(df_evo)
            ...
            combo = (
                alt.layer(capture, *layers, vline)
                .add_params(zoomx)                # pan/zoom en X
                .resolve_scale(y="independent")   # % y € con escalas independientes

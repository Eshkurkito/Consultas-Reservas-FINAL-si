...
if mode == "KPIs por meses":
    ...
    with st.sidebar:
        ...
        # Botón de ejecución y reset zoom
    ...
    if run_evo:
        ...
        if compare_e:
            ...
            for c in pd.date_range(cut_start_prev, cut_end_prev, freq="D"):
                ...
                rows_prev.append({
                    # alineamos +1 año para comparar en el mismo eje temporal
        ...
        # Para forzar “reset zoom”, simplemente re-renderizamos la carta tras el click (Streamlit hace rerun);
        # el objeto selection se reinicia al reconstruir el chart.
        # (El botón "🔄 Reset zoom" ya provoca rerun; no se necesita lógica extra.)

        layers = []

        # Tooltips base (las métricas se verán aunque no estén seleccionadas; es útil para inspección)
        base_tip = [
        ...
        # Eje X con formato compacto y evitando solapes
        x_enc = alt.X(
            "Corte:T",
        ...
        # === Líneas actuales (solo las seleccionadas) ===
        if "Ocupación %" in kpis_sel:
            layers.append(
                ...
                .encode(
                    x=x_enc,
                    y=alt.Y("ocupacion_pct:Q", title="Ocupación %", axis=alt.Axis(format=".0f")),
                    tooltip=base_tip,
                )
            )
        if "ADR (€)" in kpis_sel:
            layers.append(
                alt.Chart(df_evo)
                .mark_line(strokeWidth=2, strokeDash=[5, 3], color="#ff7f0e")
                .encode(
                    x=x_enc,
                    y=alt.Y("adr:Q", title="ADR (€)", axis=alt.Axis(orient="right", labelOverlap=True, format=",.2f")),
                    tooltip=base_tip,
                )
            )
        if "RevPAR (€)" in kpis_sel:
            layers.append(
                alt.Chart(df_evo)
                .mark_line(strokeWidth=2, strokeDash=[2, 1], color="#2ca02c")
                .encode(
                    x=x_enc,
                    y=alt.Y("revpar:Q", title="RevPAR (€)", axis=alt.Axis(orient="right", labelOverlap=True, format=",.2f")),
                    tooltip=base_tip,
                )
            )

        # === Líneas LY (solo de los KPIs seleccionados y si existen columnas) ===
        if compare_e:
            if "Ocupación %" in kpis_sel and "ocupacion_pct_prev" in df_evo.columns:
                layers.append(
                    alt.Chart(df_evo)
                    .mark_line(strokeWidth=1.5, opacity=0.35, color="#1f77b4")
            ...
            if "ADR (€)" in kpis_sel and "adr_prev" in df_evo.columns:
                layers.append(
                    alt.Chart(df_evo)
                    .mark_line(strokeWidth=1.5, strokeDash=[2, 2], opacity=0.35, color="#ff7f0e")
                    .encode(
                        x=x_enc,
                        y=alt.Y("adr_prev:Q", title="ADR (€)", axis=alt.Axis(orient="right", labelOverlap=True, format=",.2f"))
                    )
                )
            if "RevPAR (€)" in kpis_sel and "revpar_prev" in df_evo.columns:
                layers.append(
                    alt.Chart(df_evo)
                    .mark_line(strokeWidth=1.5, strokeDash=[2, 1], opacity=0.35, color="#2ca02c")
                    .encode(
                        x=x_enc,
                        y=alt.Y("revpar_prev:Q", title="RevPAR (€)", axis=alt.Axis(orient="right", labelOverlap=True, format=",.2f"))
                    )
                )

        # Selección para sincronizar pan/zoom en X e Y en todas las capas

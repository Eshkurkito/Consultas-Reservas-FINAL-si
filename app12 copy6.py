...
# =============================
    ...
    with st.sidebar:
        ...
        inv_e = st.number_input(
            "Inventario actual (opcional)", min_value=0, value=0, step=1, key="inv_evo"
        )
        inv_e_prev = st.number_input(
            "Inventario año anterior (opcional)", min_value=0, value=0, step=1, key="inv_evo_prev"
        )

        # Multi-KPI (hasta 2 para mantener ejes legibles)
        kpi_choices = ["Ocupación %", "ADR (€)", "RevPAR (€)"]
        kpis_sel = st.multiselect(
            "KPIs a mostrar (máx. 2)",
            options=kpi_choices,
            default=["Ocupación %"],
            help="Si eliges 2, se mostrarán en ejes Y separados.",
            key="kpis_evo",
        )
        if len(kpis_sel) > 2:
            st.warning("Has seleccionado más de 2 KPIs. Se usarán los dos primeros.")
            kpis_sel = kpis_sel[:2]

        compare_e = st.checkbox(
            "Comparar con año anterior (alineado por día)", value=False, key="cmp_evo"
        )
    ...
    if run_evo:
        ...
        # -------- Serie ACTUAL (por cada fecha de corte) --------
        rows_e = []
                ...
                rows_prev.append(
                    {
                        ...
                        "ingresos_prev": float(tot_c2["ingresos"]),
                    }
                )
        ...
        # Selección "nearest" para hover (común a todas las capas)
        hover = alt.selection_point(
            fields=["Corte"], nearest=True, on="pointermove", empty="none", clear="mouseout"
        )

        base = alt.Chart(df_evo).encode(x=alt.X("Corte:T", title="Fecha de corte"))

        layers = []

        def line_with_hover(y_col: str, color: str, name: str, axis=None, dash=False, opacity=1.0):
            line = (
                base.mark_line(strokeWidth=2, interpolate="monotone", strokeDash=[5, 3] if dash else None)
                .encode(
                    y=alt.Y(f"{y_col}:Q", title=name, axis=axis),
                    color=alt.value(color),
                    tooltip=[
                        alt.Tooltip("Corte:T", title="Día"),
                        alt.Tooltip(f"{y_col}:Q", title=name, format=".2f"),
                    ],
                )
            )
            # punto sobre la línea (misma Y -> nunca se “despega”)
            point = (
        ...
        # Decidimos ejes: si 1 KPI => solo eje izquierdo; si 2 KPIs => eje izq + der
        if len(kpis_sel) == 1:
            k = kpis_sel[0]
            ycol = kpi_to_col[k]
            line, point = line_with_hover(
                y_col=ycol, color="#1f77b4", name=k, axis=alt.Axis(titleColor="#1f77b4")
            )
            layers += [line, point]

            # LY (mismo eje)
            if compare_e and f"{ycol}_prev" in df_evo.columns:
                line_prev, point_prev = line_with_hover(
                    y_col=f"{ycol}_prev",
                    color="#1f77b4",
                    name=f"{k} (LY)",
                    dash=True,
                ...
                layers += [line_prev, point_prev]

        else:
            # hasta 2 KPIs
            # el eje IZQ será Ocupación si está; si no, el primero
            left_k = "Ocupación %" if "Ocupación %" in kpis_sel else kpis_sel[0]
            right_k = [m for m in kpis_sel if m != left_k][0]

            # Izquierda
            left_col = kpi_to_col[left_k]
            l_line, l_point = line_with_hover(
                y_col=left_col, color="#1f77b4", name=left_k, axis=alt.Axis(titleColor="#1f77b4")
            )
            layers += [l_line, l_point]
            if compare_e and f"{left_col}_prev" in df_evo.columns:
                l_prev, l_prev_pt = line_with_hover(
                    y_col=f"{left_col}_prev",
                    color="#1f77b4",
                    name=f"{left_k} (LY)",
                    dash=True,
                    opacity=0.45,
                ...
                layers += [l_prev, l_prev_pt]

            # Derecha
            right_col = kpi_to_col[right_k]
            r_line, r_point = line_with_hover(
                y_col=right_col,
                color="#ff7f0e",
                name=right_k,
                axis=alt.Axis(orient="right", titleColor="#ff7f0e"),
            )
            layers += [r_line, r_point]
            if compare_e and f"{right_col}_prev" in df_evo.columns:
                r_prev, r_prev_pt = line_with_hover(
                    y_col=f"{right_col}_prev",
                    color="#ff7f0e",
                    name=f"{right_k} (LY)",
                    dash=True,
                    opacity=0.45,
                ...
                layers += [r_prev, r_prev_pt]

        # Regla vertical en el hover (sin eje Y extra)
        v_rule = (
            base.mark_rule(strokeDash=[2, 2])
            .encode(x="Corte:T")
            .add_params(hover)
            .transform_filter(hover)
        )
        layers.append(v_rule)

        chart = alt.layer(*layers).resolve_scale(y="independent").properties(height=360).interactive()
        ...
        if compare_e:
            show_cols += ["ocupacion_pct_prev", "adr_prev", "revpar_prev", "ingresos_prev"]

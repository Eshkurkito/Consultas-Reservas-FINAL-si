    ...
    if run_evo:
        ...
        for c in pd.date_range(cut_start_ts, cut_end_ts, freq="D"):
            ...
            rows_e.append({
                ...
                "ocupacion_pct": tot_c["ocupacion_pct"],   # 0-100
                "adr": tot_c["adr"],
                "revpar": tot_c["revpar"],
        ...
        # --- Reglas de ejes ---
        # 1) Si sólo hay 1 KPI → un eje Y (izquierdo) con su escala.
        # 2) Si hay varios → Ocupación en eje izquierdo; ADR y RevPAR comparten eje derecho.
        single_kpi = len(kpis_sel) == 1

        # === KPI: Ocupación (actual / prev) ===
        if "Ocupación %" in kpis_sel:
            ...
            layers.append(
                alt.Chart(df_evo).mark_line(strokeWidth=2, color="#1f77b4").encode(
                    x=x_enc, y=y_occ, tooltip=base_tip
                )
            )
            if compare_e and "ocupacion_pct_prev" in df_evo.columns:
                layers.append(
                    alt.Chart(df_evo).mark_line(strokeWidth=1.5, opacity=0.35, color="#1f77b4").encode(
                        x=x_enc,
                        y=alt.Y("ocupacion_pct_prev:Q", title="Ocupación %", axis=alt.Axis(format=".0f")),
                    )
                )

        # === KPI: ADR (actual / prev) ===
        if "ADR (€)" in kpis_sel:
            # Si es sólo 1 KPI → eje izquierdo; si no, eje derecho
            adr_axis = alt.Axis(orient="left" if single_kpi else "right", labelOverlap=True, format=",.2f")
            layers.append(
                alt.Chart(df_evo).mark_line(strokeWidth=2, strokeDash=[5, 3], color="#ff7f0e").encode(
                    x=x_enc,
                    y=alt.Y("adr:Q", title="ADR (€)", axis=adr_axis),
                    tooltip=base_tip,
                )
            )
            if compare_e and "adr_prev" in df_evo.columns:
                layers.append(
                    alt.Chart(df_evo).mark_line(strokeWidth=1.5, strokeDash=[2, 2], opacity=0.35, color="#ff7f0e").encode(
                        x=x_enc,
                        y=alt.Y("adr_prev:Q", title="ADR (€)", axis=adr_axis),
                    )
                )

        # === KPI: RevPAR (actual / prev) ===
        if "RevPAR (€)" in kpis_sel:
            # Si es sólo 1 KPI → eje izquierdo; si no, eje derecho (compartido con ADR)
            rev_axis = alt.Axis(orient="left" if single_kpi else "right", labelOverlap=True, format=",.2f")
            layers.append(
                alt.Chart(df_evo).mark_line(strokeWidth=2, strokeDash=[2, 1], color="#2ca02c").encode(
                    x=x_enc,
                    y=alt.Y("revpar:Q", title="RevPAR (€)", axis=rev_axis),
                    tooltip=base_tip,
                )
            )
            if compare_e and "revpar_prev" in df_evo.columns:
                layers.append(
                    alt.Chart(df_evo).mark_line(strokeWidth=1.5, strokeDash=[2, 1], opacity=0.35, color="#2ca02c").encode(
                        x=x_enc,
                        y=alt.Y("revpar_prev:Q", title="RevPAR (€)", axis=rev_axis),
                    )
                )

        # Selección para pan/zoom **sólo en X** para evitar desincronía de ejes Y
        zoom_x = alt.selection_interval(bind='scales', encodings=['x'])

        chart = alt.layer(*layers).add_params(zoom_x)
        # Si hay más de un KPI, necesitamos y independientes (izq/der). Si sólo uno, una sola escala.
        chart = chart.resolve_scale(y="independent" if not single_kpi else "shared").properties(height=380)
        ...
        if compare_e:
            cols += ["ocupacion_pct_prev", "adr_prev", "revpar_prev", "res_ing_prev"]

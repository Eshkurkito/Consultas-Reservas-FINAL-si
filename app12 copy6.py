...
# =============================
    ...
    with st.sidebar:
        ...
        kpis_sel = st.multiselect(
            ...
            default=["Ocupación %"],
            key="kpis_evo",
            help="Puedes elegir 1, 2 o 3 KPIs. Si eliges 1, habrá un único eje Y; si añades otro, aparecerá eje Y derecho."
        )
    ...
    if run_evo:
        ...
        KPI_CFG = {
            "Ocupación %": {"col": "ocupacion_pct", "axis_title": "Ocupación %", "side": "left"},
            "ADR (€)":     {"col": "adr",           "axis_title": "ADR (€)",     "side": "right"},
            "RevPAR (€)":  {"col": "revpar",        "axis_title": "RevPAR (€)",  "side": "right"},
        }
        COLORS = {
            "Ocupación %": "#1f77b4",
            "ADR (€)": "#ff7f0e",
        ...
        # Asegura al menos 1 KPI
        if not kpis_sel:
            kpis_sel = ["Ocupación %"]

        # Hover por X (no hace falta estar sobre la línea)
        hover = alt.selection_single(fields=["Corte"], on="mousemove", nearest=False, empty="none")

        # Zoom/pan SOLO en X
        zoomx = alt.selection_interval(encodings=["x"])

        layers = []
        left_axis_set = False
        right_axis_set = False

        for label in kpis_sel:
            cfg = KPI_CFG[label]
            col = cfg["col"]
            color = COLORS[label]

            # Decide eje: si es el primer KPI → eje izquierdo.
            # Si hay más y el KPI no es el primero, se intenta eje derecho.
            # ADR y RevPAR comparten eje derecho (sin duplicarlo).
            if label == kpis_sel[0]:
                # Primer KPI → eje izquierdo
                y_enc = alt.Y(f"{col}:Q", title=cfg["axis_title"])
                left_axis_set = True
            else:
                # Siguientes KPIs
                if cfg["side"] == "right":
                    # Solo poner el eje derecho UNA VEZ
                    if not right_axis_set:
                        y_enc = alt.Y(f"{col}:Q", title=cfg["axis_title"], axis=alt.Axis(orient="right"))
                        right_axis_set = True
                    else:
                        # Eje ya creado → no volvemos a crear (evita duplicados)
                        y_enc = alt.Y(f"{col}:Q", axis=None)
                else:
                    # Si fuera otro KPI a la izquierda (poco probable aquí) y ya existe, no dupliques
                    y_enc = alt.Y(f"{col}:Q", axis=None)
            ...
            points = (
                alt.Chart(df_evo)
                .mark_circle(size=45, color=color)
                .encode(
                    x="Corte:T",
                    y=alt.Y(f"{col}:Q"),
                )
            ...
            # Serie LY (si procede)
            if compare_e and (f"{col}_prev" in df_evo.columns):
                line_prev = (
                    alt.Chart(df_evo)
                    .mark_line(strokeWidth=1.5, strokeDash=[4, 3], color=color, opacity=0.4)
                    .encode(
                        x="Corte:T",
                        y=alt.Y(f"{col}_prev:Q", axis=None),  # comparte el eje del actual para no duplicar
                    )
                )
                points_prev = (
                    alt.Chart(df_evo)
                    .mark_circle(size=30, color=color, opacity=0.35)
                    .encode(
                        x="Corte:T",
                        y=alt.Y(f"{col}_prev:Q"),
                    )
                    .transform_filter(hover)
                )
                layers += [line_prev, points_prev]

        # Regla vertical que sigue el ratón + tooltip con TODOS los KPIs seleccionados
        tooltips = [alt.Tooltip("Corte:T", title="Corte", format="%Y-%m-%d")]
        for label in kpis_sel:
            c = KPI_CFG[label]["col"]
            tooltips.append(alt.Tooltip(f"{c}:Q", title=label, format=".2f"))
            if compare_e and f"{c}_prev" in df_evo.columns:
                tooltips.append(alt.Tooltip(f"{c}_prev:Q", title=f"{label} (LY)", format=".2f"))
        ...
        # Selector invisible para activar el hover al mover el ratón
        selectors = (
            alt.Chart(df_evo)
        ...
        combo = (
            alt.layer(selectors, *layers, vline)
            .add_params(zoomx)
            .resolve_scale(y="independent")  # evita mezclar escalas de % y €

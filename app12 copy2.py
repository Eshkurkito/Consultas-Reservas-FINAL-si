...
# ---------- Cuadro de mando PRO (Pace YoY + ADR bands por props + ADR histórico + Ingresos comparados) ----------
    ...
    with st.sidebar:
        ...
        # 👇 Controles para ADR histórico y semáforo
    ...
    if run_pro:
        ...
        # === NUEVO: INGRESOS DEL PERIODO vs LY y LY-2 ===
        ...
        def _kpis_period(df_all, cutoff, start, end, props=None, inv=None):
            ...
            return {
                "ingresos": float(_tot["ingresos"]),
                "occ": float(_tot["ocupacion_pct"]),
                "adr": float(_tot["adr"]),
                "revpar": float(_tot["revpar"]),
            }
        ...
        df_rev = pd.DataFrame({
            "Etiqueta": ["Actual", "LY", "LY-2"],
            "Ingresos (€)": [k_now["ingresos"], k_ly["ingresos"], k_ly2["ingresos"]],
        })
        ...
        if np.isfinite(d_rev_pct):
            ...
            msg = (
                f"{trend} **Ingresos vs LY:** {d_rev_pct:+.1f}%  ·  "
                f"ADR: {d_adr_pct:+.1f}%  ·  Δ Ocupación: {d_occ_pp:+.1f} p.p.\n\n"
                f"**Lectura:** {driver}. "
            )

            if d_rev_pct < -3:
                if d_adr_pct <= 0 and d_occ_pp <= 0:
                    sug = "Revisar precio base (banda P25–P50), potenciar visibilidad y ventanas mínimas."
                elif d_adr_pct > 0 and d_occ_pp < 0:
                    sug = "Parece sobreprecio: prueba reducir ADR en DOW/fechas débiles o ampliar disponibilidad."
                else:
                    sug = "Refinar mix: ajustar ADR por DOW y empujar campañas en huecos de ocupación."
            elif d_rev_pct > 3:
                if d_adr_pct > 0 and d_occ_pp >= 0:
                    sug = "Buen momentum: testea +2–4% en DOW fuertes o cercanos."
                else:
                    sug = "Sostener estrategia actual y monitorizar pace para no perder tracción."
            else:
                sug = "Resultado en línea con LY; afinar DOW/eventos puede capturar puntos extra."

            st.markdown(
                f"> **Ingresos del periodo**: {k_now['ingresos']:,.2f} € · "
                f"**LY**: {k_ly['ingresos']:,.2f} € · **LY-2**: {k_ly2['ingresos']:,.2f} €".replace(",", ".")
            )
            st.markdown(msg + f"**Sugerencia:** {sug}")
        else:
...
# =============================
    ...
    # 1) Cargar desde disco y poblar sesión si no existe aún
    ...
    edited = st.data_editor(
        ...
        hide_index=True,
        key="events_editor",  # <- CLAVE DEL WIDGET (no la tocamos en session_state)
    )
...
# =============================
    ...
    with st.sidebar:
        ...
        start_tar = st.date_input("Inicio tarificación", value=(today.to_period("M").start_time).date())
        end_tar   = st.date_input("Fin tarificación", value=(today.to_period("M").end_time).date())
        cutoff_tar= st.date_input("Fecha de corte (OTB/Pace)", value=today.date())
    ...
    def pace_delta_vs_ly_pp(df_all, start, end, dmax: int, props: List[str], inv_override, cutoff: pd.Timestamp, d_for_day: int) -> float:
    ...
    if run_tar:
        ...
        if use_p50_by_apto:
            base_p50_map = adr_bands_p50_for_month_by_apto(
                raw, pd.to_datetime(cutoff_tar), period_start, period_end, props_sel
            )
        else:
            base_p50_value = adr_bands_p50_for_month(
                raw, pd.to_datetime(cutoff_tar), period_start, period_end, props_sel
            )
            base_p50_map = {a: base_p50_value for a in props_sel}

        # m_apto histórico (LY) + caps + intensidad (precalc una vez)
        m_apto_raw = compute_m_apto_by_property(
            raw, pd.to_datetime(cutoff_tar), pd.to_datetime(start_tar), pd.to_datetime(end_tar), props_sel
        )
        ...
        # ---------- CONTEXTO PRECALCULADO (vectorizado por día)
        ctx = pd.DataFrame({"Fecha": days})
        ctx["Fecha"] = pd.to_datetime(ctx["Fecha"]).dt.normalize()
        ctx["dow"] = ctx["Fecha"].dt.weekday
        ctx["is_weekend"] = ctx["dow"].isin([5, 6])

        # Multiplicador DOW
        ctx["m_dow"] = ctx["dow"].map({
            0: m_lun, 1: m_mar, 2: m_mie, 3: m_jue, 4: m_vie, 5: m_sab, 6: m_dom
        }).astype(float)
        ...
        # Pace Δpp (modo rápido por grupo)
        fast_mode = st.checkbox("⚡ Modo rápido (pace por grupo)", value=True,
                                help="Calcula el delta de pace vs LY una única vez (grupo). Si lo desactivas, lo calculará por apartamento (más lento).")
        ...
        if fast_mode:
            ...
            if not base.empty and not ly.empty:
                m_now = base.set_index("D")["ocupacion_pct"]
                m_ly  = ly.set_index("D")["ocupacion_pct"]
                # alinear y reindexar todos los D necesarios
                all_D = range(0, dmax + 1)
                m_now = m_now.reindex(all_D).interpolate().fillna(method="bfill").fillna(method="ffill")
                m_ly  = m_ly.reindex(all_D).interpolate().fillna(method="bfill").fillna(method="ffill")
            ...
            # más preciso pero mucho más lento: recalcular por apartamento (lo haremos después, en el loop)
            ctx["Δpp_pace"] = 0.0  # placeholder
        ...
        # ---------- CÁLCULO VECTORIAL POR ALOJAMIENTO
        results = []
        pbar = st.progress(0)
        for i, aloj in enumerate(props_sel, start=1):
            # ADR base (con fallback) + m_apto
            ADR_base = base_p50_map.get(aloj, np.nan)
            if not np.isfinite(ADR_base) or ADR_base <= 0:
                # Fallback al ADR tail mensual
                mstart = period_start
                mend   = period_end
                f = pace_forecast_month(raw, pd.to_datetime(cutoff_tar), mstart, mend, 2, 180, [aloj], inv_override)
                ADR_base = float(f.get("adr_tail_p50", np.nan))
                if not np.isfinite(ADR_base) or ADR_base <= 0:
                    ADR_base = 60.0
            ADR_base = ADR_base * float(m_apto.get(aloj, 1.0))

            # Δpp pace (si no fast_mode, se recalcula por apto una sola vez)
            if fast_mode:
                ...
                if not base_a.empty and not ly_a.empty:
                    now_s = base_a.set_index("D")["ocupacion_pct"].reindex(range(0, dmax+1)).interpolate().fillna(method="bfill").fillna(method="ffill")
                    ly_s  = ly_a.set_index("D")["ocupacion_pct"].reindex(range(0, dmax+1)).interpolate().fillna(method="bfill").fillna(method="ffill")
                    dmap  = (now_s - ly_s).to_dict()
                    delta_pp = ctx["D"].map(dmap).fillna(0.0).values
                else:
                    delta_pp = np.zeros(len(ctx), dtype=float)

            # multiplicadores diarios (arrays)
            ...
            # pace multiplicador con clip ±15%
            m_pace_arr  = 1.0 + k_pace * delta_pp
            m_pace_arr  = np.clip(m_pace_arr, 0.85, 1.15)
        ...
        # Guardar en sesión por si quieres ver el calendario aparte
        st.session_state["pricing_grid"] = grid_wide
        st.session_state["pricing_grid_meta"] = meta_cols

        # ---------- DESCARGAS
        # CSV (ya lo tenías, lo mantenemos)
        ...
        with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
            ...
            # Hoja de contexto (por si quieres auditar)
            ctx_export = ctx.copy()
            # Limpia columnas internas para no crecer de más
            ctx_export[["uplift_pct","origen"]] = ctx_export.get("uplift_pct", np.nan), ctx_export.get("origen", "")
...
# =============================
    ...
    if grid is None or meta is None or grid.empty:
        ...
        month_sel = st.selectbox("Mes a mostrar", unique_months, index=0)
        ...
        st.download_button("⬇️ Descargar mes (CSV)", data=csv_month, file_name=f"tarifas_{month_sel}.csv", mime="text/csv")

        st.caption("Gris = fin de semana · Amarillo = evento (tooltip con nombre)")

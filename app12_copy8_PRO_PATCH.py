...
# --- Histórico (final) del mismo periodo el año pasado ---
start_prev = (pd.to_datetime(start_normal) - pd.DateOffset(years=1))
end_prev   = (pd.to_datetime(end_normal)   - pd.DateOffset(years=1))
cutoff_prev_final = end_prev  # usar final del periodo LY como 'corte' para tener valores finales

_, total_prev_final = compute_kpis(
    df_all=raw,
    cutoff=cutoff_prev_final,
    period_start=start_prev,
    period_end=end_prev,
    inventory_override=int(inv_normal_prev) if ('inv_normal_prev' in locals() and inv_normal_prev > 0) else None,
    filter_props=props_normal if props_normal else None,
)

st.divider()
st.subheader("📦 Histórico — Mismo periodo (resultado final) año pasado")
ly1, ly2, ly3 = st.columns(3)
ly4, ly5, ly6 = st.columns(3)
ly1.metric("Noches (final LY)",       f"{total_prev_final['noches_ocupadas']:,}".replace(",", "."))
ly2.metric("Noches disp. (final LY)", f"{total_prev_final['noches_disponibles']:,}".replace(",", "."))
ly3.metric("Ocupación (final LY)",    f"{total_prev_final['ocupacion_pct']:.2f}%")
ly4.metric("Ingresos (final LY)",     f"{total_prev_final['ingresos']:.2f}")
ly5.metric("ADR (final LY)",          f"{total_prev_final['adr']:.2f}")
ly6.metric("RevPAR (final LY)",       f"{total_prev_final['revpar']:.2f}")

# Comparativa rápida
import pandas as _pd
_tbl_cmp = _pd.DataFrame([
    {"KPI":"Ocupación %", "Actual (OTB hoy)": total_n["ocupacion_pct"], "Final LY": total_prev_final["ocupacion_pct"]},
    {"KPI":"ADR (€)",     "Actual (OTB hoy)": total_n["adr"],            "Final LY": total_prev_final["adr"]},
    {"KPI":"RevPAR (€)",  "Actual (OTB hoy)": total_n["revpar"],         "Final LY": total_prev_final["revpar"]},
    {"KPI":"Ingresos (€)","Actual (OTB hoy)": total_n["ingresos"],       "Final LY": total_prev_final["ingresos"]},
])
st.dataframe(_tbl_cmp.style.format({"Actual (OTB hoy)": "{:.2f}", "Final LY": "{:.2f}"}), use_container_width=True)

# Mini gráfico de barras comparativas
_plot_df = _pd.DataFrame({
    "Actual (OTB hoy)":[total_n["ingresos"], total_n["adr"], total_n["revpar"]],
    "Final LY":[total_prev_final["ingresos"], total_prev_final["adr"], total_prev_final["revpar"]],
}, index=["Ingresos (€)","ADR (€)","RevPAR (€)"])
st.bar_chart(_plot_df, height=260)
        df_all=raw,

...
def _kai_cdm_pro_analysis(tot_now: dict, tot_ly_cut: dict, tot_ly_final: dict, pace: dict, price_ref_p50: float|None) -> str:
    ...
    if d_occ is not None and not pd.isna(d_occ):
        if d_occ < 0:
            ...
            lines.append("🟠 Ocupación similar a LY: ajustes quirúrgicos por DOW/eventos.")

    if d_adr is not None and not pd.isna(d_adr):
        if d_adr < -0.05:
            lines.append("🔴 ADR por debajo de LY: revisar descuentos y visibilidad.")
        elif d_adr > 0.05:
            lines.append("🟢 ADR por encima de LY: aprovechar en picos, vigilar cancelaciones.")
        else:
            lines.append("🟠 ADR en línea con LY: microajustes según demanda.")
    ...
    if isinstance(pace, dict):
        ...
        if np.isfinite(need) and np.isfinite(typ):
            if need > typ * 1.25:
                ...
                lines.append(f"🟢 Pickup adelantado: faltan {need:.0f} noches vs típico {typ:.0f}.")
            else:
                lines.append(f"🟠 Pickup en línea: faltan {need:.0f} noches vs típico {typ:.0f}.")

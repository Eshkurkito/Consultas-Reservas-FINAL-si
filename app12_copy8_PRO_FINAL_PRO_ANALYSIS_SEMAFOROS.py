...
# ===========================
    ...
    if by_prop_now.empty:
        ...
        csv = by_prop_now.to_csv(index=False).encode("utf-8-sig")
        st.download_button(
            "📥 Descargar detalle (CSV)",
            data=csv,
            file_name="detalle_por_alojamiento.csv",
...
# ---------- Gap vs Target ----------
    ...
    if tgts is None:
        ...
        for ym in months_sel:
            ...
            _, real = compute_kpis(raw, pd.to_datetime(cut_gt), s, e, int(inv_gt) if inv_gt>0 else None, props=None)
            y, m = p.year, p.month
...
# ---------- ADR bands & Targets ----------
    ...
    if run_ab:
        df = raw[raw["Fecha alta"] <= pd.to_datetime(ab_cutoff)].dropna(subset=["Fecha entrada", "Fecha salida"]).copy()
        if props_ab:
        ...
        if df.empty:
            ...
            for ym in bands_wide.index.tolist():
                ...
                _bp, tot_m = compute_kpis(raw, pd.to_datetime(ab_cutoff), m_start, m_end, None, props_ab if props_ab else None)
...
# ---------- Tarificación (beta) ----------
    ...
    with st.sidebar:
        ...
        st.markdown("—")
        st.markdown("**Eventos**")
        ev_mode = st.radio("Modo eventos", ["Sin eventos", "Aplicar uplift %", "Ajustar mínimos"], index=0, key="tar_ev_mode")
        if ev_mode == "Aplicar uplift %":
            ev_col = st.selectbox("Columna uplift %", ["uplift_pct", "origen"], index=0, key="tar_ev_col")
        else:
            ev_col = None

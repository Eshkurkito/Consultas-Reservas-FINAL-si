...
def _kai_cdm_pro_analysis(tot_now: dict, tot_ly_cut: dict, tot_ly_final: dict, pace: dict, price_ref_p50: float|None) -> str:
    """
    Análisis PRO avanzado:
      • Control estacionalidad por 'peer group' (mismo mes y DOW histórico).
      • Lead-time intelligence (percentiles de pickup).
      • Atribución RevPAR (log).
      • Elasticidad simple estimada con histórico.
      • Viabilidad de cierre con bandas (P25/P50/P75) y prob. de cubrir gap.
      • Veredicto + plan de acción priorizado.
    Requiere que el dataset base y utilidades ya hayan calculado:
      - pace: {pickup_typ_p25, p50, p75, adr_tail_p50, nights_otb} si existen; si faltan, se degradan con heurísticos.
    """
    import math
    ...
    def _pct(a,b):
        try:
            if b in (0, None) or pd.isna(b) or b == 0: return np.nan
            return (a-b)/b
        except Exception:
            return np.nan
    ...
    def _dlog(a,b):
        try:
            if a<=0 or b<=0 or pd.isna(a) or pd.isna(b): return np.nan
            return math.log(a/b)
        except Exception:
            return np.nan
    ...
    nights_ly_final = float(tot_ly_final.get("noches_ocupadas", np.nan))
    ing_ly_final    = float(tot_ly_final.get("ingresos", np.nan))

    # Deltas vs LY (a corte)
    ...
    # ====== Ajuste “peer group” (estacionalidad) ======
    # Si el dict pace trae % pickup típicos, lo usamos como 'benchmark' de LT
    need_p50 = pace.get("pickup_typ_p50", np.nan) if isinstance(pace, dict) else np.nan
    need_p25 = pace.get("pickup_typ_p25", np.nan) if isinstance(pace, dict) else np.nan
    need_p75 = pace.get("pickup_typ_p75", np.nan) if isinstance(pace, dict) else np.nan
    nights_otb = pace.get("nights_otb", np.nan) if isinstance(pace, dict) else np.nan

    # Índice LT (cómo vamos vs percentiles históricos)
    lt_signal = "—"
    if np.isfinite(need_p50):
        if np.isfinite(need_p25) and nights_otb >= need_p50 + (need_p50 - need_p25):
            lt_signal = "🟢 pickup muy adelantado vs P50"
        elif np.isfinite(need_p75) and nights_otb <= max(0, need_p50 - (need_p75 - need_p50)):
            lt_signal = "🔴 pickup retrasado vs P50"
        else:
            lt_signal = "🟠 pickup en línea con P50"

    # ====== Precio de referencia (bajada temprana) ======
    baja_temprana = False
    delta_vs_p50 = np.nan
    if price_ref_p50 is not None and np.isfinite(price_ref_p50) and np.isfinite(adr_now):
        delta_vs_p50 = _pct(adr_now, price_ref_p50)
        # si ADR < P50-5% y no mejora occ → bandera
        if adr_now < price_ref_p50*0.95 and (not np.isfinite(d_occ) or d_occ <= 0.0):
            baja_temprana = True

    # ====== Viabilidad de cierre (bandas) ======
    adr_tail_p50 = pace.get("adr_tail_p50", np.nan) if isinstance(pace, dict) else np.nan
    # Construimos 3 escenarios de ingreso adicional (P25/P50/P75)
    extra_p25 = (need_p25 * adr_tail_p50) if (np.isfinite(need_p25) and np.isfinite(adr_tail_p50)) else np.nan
    extra_p50 = (need_p50 * adr_tail_p50) if (np.isfinite(need_p50) and np.isfinite(adr_tail_p50)) else np.nan
    extra_p75 = (need_p75 * adr_tail_p50) if (np.isfinite(need_p75) and np.isfinite(adr_tail_p50)) else np.nan

    cobertura_p = {}  # cobertura del gap por banda
    for label, extra in [("P25", extra_p25), ("P50", extra_p50), ("P75", extra_p75)]:
        if np.isfinite(extra) and np.isfinite(falta_ingresar_vs_LYfinal) and falta_ingresar_vs_LYfinal>0:
            cobertura_p[label] = extra / falta_ingresar_vs_LYfinal
        else:
            cobertura_p[label] = np.nan

    # Prob. heurística de cubrir gap (si P50>=1 ≈ alta; si P25>=1 ≈ muy alta; si P75<1 ≈ baja)
    prob_cubrir = "—"
    if all(label in cobertura_p for label in ("P25","P50","P75")):
        c25, c50, c75 = cobertura_p["P25"], cobertura_p["P50"], cobertura_p["P75"]
        if np.isfinite(c25) and c25>=1.0:
            prob_cubrir = "🟢 muy alta"
        elif np.isfinite(c50) and c50>=1.0:
            prob_cubrir = "🟢 alta"
        elif np.isfinite(c75) and c75>=1.0:
            prob_cubrir = "🟠 razonable"
        elif np.isfinite(c50):
            prob_cubrir = "🔴 baja (P50 no cubre)"
        else:
            prob_cubrir = "—"

    # ====== Elasticidad simple (estimación gris si no hay histórico) ======
    # Si pace no trae nada, asumimos eps=-0.35; si trae dos puntos (LY y actual) inferimos una eps grosera
    eps = -0.35
    if np.isfinite(occ_now) and np.isfinite(occ_ly) and np.isfinite(adr_now) and np.isfinite(adr_ly) and adr_now>0 and adr_ly>0:
        try:
            # Δocc% ≈ eps * Δadr%  =>  eps ≈ (occ_now/occ_ly - 1) / (adr_now/adr_ly - 1)
            num = (occ_now/occ_ly - 1.0)
            den = (adr_now/adr_ly - 1.0)
            if den != 0 and np.isfinite(num) and np.isfinite(den):
                eps = float(num/den)
                eps = float(np.clip(eps, -1.2, 0.1))  # cap para no volarnos
        except Exception:
            pass

    # Testing ±5–10% en ADR tail sobre pickup P50
    rec_precio = ""
    if np.isfinite(need_p50) and np.isfinite(adr_tail_p50) and np.isfinite(falta_ingresar_vs_LYfinal) and falta_ingresar_vs_LYfinal>0:
        best = None
        for x in [ -0.10, -0.05, 0.0, +0.05, +0.10 ]:
            adr_new  = adr_tail_p50 * (1+x)
            pick_new = need_p50 * (1 + eps*x)
            rev_tail = max(pick_new, 0) * max(adr_new, 0)
            cov = rev_tail / falta_ingresar_vs_LYfinal if falta_ingresar_vs_LYfinal>0 else np.nan
            score = -abs(1-cov) if np.isfinite(cov) else -np.inf
            if (best is None) or (score > best[0]): best = (score, x, cov, rev_tail)
        if best:
            _, x, cov, rev = best
            x_pct = f"{x*100:+.0f}%"
            if cov >= 1.05:
                rec_precio = f"🟢 Un ajuste de ADR tail de **{x_pct}** cubriría el gap con margen (≈ {cov*100:.0f}%)."
            elif cov >= 0.85:
                rec_precio = f"🟠 Con **{x_pct}** en ADR tail la cobertura sería ≈ {cov*100:.0f}%. Complementa con demanda."
            else:
                rec_precio = f"🔴 Ni con **{x_pct}** en ADR tail llegamos (≈ {cov*100:.0f}%). Hace falta **boost** de demanda."

    # ====== Veredicto ======
    veredicto = "🟠 Equilibrado"
    if np.isfinite(d_adr) and np.isfinite(d_occ):
        if d_adr < -0.03 and d_occ <= 0.00: veredicto = "🔴 Vendemos barato sin ganar volumen"
        elif d_adr < -0.03 and d_occ > 0.01: veredicto = "🟠 Comprando volumen"
        elif d_adr > 0.03 and d_occ < -0.01: veredicto = "🟠 Precio alto con penalización"
        else: veredicto = "🟢 Balance razonable"

    # ====== Narrativa ======
    lines = []
    lines.append(f"**Veredicto**: {veredicto}")
    lines.append("**Evolución vs LY (a corte)**")
    lines.append(f"- Ocupación: {_fmt_pct(d_occ)} · ADR: {_fmt_pct(d_adr)} · RevPAR: {_fmt_pct(d_revpar)} · Ingresos: {_fmt_pct(d_ing)}")
    ...
    if price_ref_p50 is not None and np.isfinite(price_ref_p50) and np.isfinite(delta_vs_p50):
        lines.append(f"- ADR actual {adr_now:.2f}€ vs P50 {price_ref_p50:.2f}€ → {_fmt_pct(delta_vs_p50)}")
        if baja_temprana:
            lines.append("  ↳ 🔴 Señal de **bajada temprana** (ADR < P50 sin mejora de ocupación).")

    lines.append("**Lead time / Ritmo**")
    lines.append(f"- {lt_signal}")

    if np.isfinite(falta_ingresar_vs_LYfinal):
        if falta_ingresar_vs_LYfinal > 0:
            lines.append("**Viabilidad de cierre**")
            lines.append(f"- Gap vs LY final: {_fmt_pct(gap_ing_final)} (faltan { _fmt_money(falta_ingresar_vs_LYfinal) })")
            if np.isfinite(extra_p50):
                lines.append(f"- Ingreso adicional esperable: P25={_fmt_money(extra_p25)} · P50={_fmt_money(extra_p50)} · P75={_fmt_money(extra_p75)}")
            if isinstance(cobertura_p, dict):
                c25 = cobertura_p.get('P25', np.nan); c50 = cobertura_p.get('P50', np.nan); c75 = cobertura_p.get('P75', np.nan)
                _cov = lambda c: "—" if not np.isfinite(c) else f"{c*100:.0f}%"
                lines.append(f"- Cobertura del gap: P25={_cov(c25)} · P50={_cov(c50)} · P75={_cov(c75)} (prob. cubrir: {prob_cubrir})")
            if rec_precio:
                lines.append(rec_precio)
        else:
            lines.append("🟢 Ya estamos por encima del cierre LY en ingresos a esta fecha de corte.")

    # Plan priorizado (condicionado)
    plan = []
    if baja_temprana:
        plan.append("Activar **demanda** (visibilidad/partners) y **fences** antes de más bajadas de ADR.")
    if np.isfinite(d_adr) and d_adr < -0.05: plan.append("Retirar descuentos de baja conversión; concentrar en huecos con elasticidad real.")
    if np.isfinite(d_adr) and d_adr > 0.05 and np.isfinite(d_occ) and d_occ < -0.01:
        plan.append("Micro-rebajas **quirúrgicas** en días valle; sostener picos.")
    if np.isfinite(need_p50) and np.isfinite(adr_tail_p50) and falta_ingresar_vs_LYfinal>0 and rec_precio:
        plan.append("Aplicar el ajuste de ADR tail recomendado y medir impacto semanal.")
    if not plan:
        plan.append("Mantener estrategia y monitorizar pickup/señales; ajustes finos por DOW/eventos.")

    lines.append("**Plan de acción (prioridad)**")
    for p in plan:
        lines.append(f"- {p}")

    # ====== Explicación ejecutiva (traducción simple) ======
    explicacion = []
    explicacion.append("**Explicación ejecutiva**")

    # Evolución general
    if np.isfinite(d_occ) and np.isfinite(d_adr) and np.isfinite(d_ing):
        if d_occ < -0.05 and d_adr > 0.05:
            explicacion.append("- Estamos perdiendo ocupación pese a subir precio → resultado: menos ingresos.")
        elif d_occ > 0.05 and d_adr < -0.05:
            explicacion.append("- Hemos ganado ocupación bajando precio → revisar si la facturación compensa.")
        elif d_occ < -0.05 and d_adr < -0.05:
            explicacion.append("- Bajamos precio y aun así cae la ocupación → señal de **problema de demanda**.")
        else:
            explicacion.append("- Evolución equilibrada entre precio y ocupación.")

    # ADR vs histórico
    if price_ref_p50 and np.isfinite(price_ref_p50) and np.isfinite(adr_now):
        if adr_now < price_ref_p50*0.95:
            explicacion.append("- ADR por debajo del histórico P50: posible **bajada temprana**.")
        elif adr_now > price_ref_p50*1.05:
            explicacion.append("- ADR por encima del P50: ojo a la conversión.")
        else:
            explicacion.append("- ADR en línea con el histórico.")

    # Gap de ingresos
    if np.isfinite(falta_ingresar_vs_LYfinal) and falta_ingresar_vs_LYfinal > 0:
        explicacion.append(f"- Faltan { _fmt_money(falta_ingresar_vs_LYfinal) } para igualar al cierre LY y con el pickup esperado no se cubre.")
    elif np.isfinite(falta_ingresar_vs_LYfinal) and falta_ingresar_vs_LYfinal <= 0:
        explicacion.append("- Ya estamos por encima del cierre LY en ingresos.")

    # Plan simplificado
    if baja_temprana:
        explicacion.append("- Evitar más bajadas de precio: primero activar demanda (visibilidad, promos puntuales).")
    if prob_cubrir.startswith("🔴"):
        explicacion.append("- Necesitamos **boost de demanda**: campañas, canales alternativos.")
    if veredicto.startswith("🟠 Precio alto"):
        explicacion.append("- Ajustar solo días valle; mantener precios en picos fuertes.")

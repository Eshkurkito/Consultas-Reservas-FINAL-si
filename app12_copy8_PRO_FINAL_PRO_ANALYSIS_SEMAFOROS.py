...
# ===========================
    ...
    # Actual a la fecha de corte (incluye detalle por alojamiento)
    ...
    # LY a misma fecha de corte y mismo periodo (-1 año)
    ...
    # LY final (cómo acabó el periodo LY): corte = fin del periodo LY
    ...
    # Tarjetas de ocupación solicitadas
    c1, c2, c3 = st.columns(3)
    delta_pp = tot_now["ocupacion_pct"] - tot_ly_cut["ocupacion_pct"]
    c1.metric("Ocupación actual", f"{tot_now['ocupacion_pct']:.2f}%", delta=f"{delta_pp:+.2f} p.p.")
    c2.metric("Ocupación LY (a este corte)", f"{tot_ly_cut['ocupacion_pct']:.2f}%")
    c3.metric("Ocupación LY final", f"{tot_ly_final['ocupacion_pct']:.2f}%")

    st.caption("Actual y LY: reservas con Fecha alta ≤ corte. LY final: corte = fin del periodo LY.")

    # Extras útiles
    c4, c5, c6 = st.columns(3)
    c4.metric("ADR actual (€)", f"{tot_now['adr']:.2f}")
    c5.metric("RevPAR actual (€)", f"{tot_now['revpar']:.2f}")
    c6.metric("Ingresos actuales (€)", f"{tot_now['ingresos']:.2f}")

    # --------- Análisis PRO con semáforo ---------
    st.divider()
    st.subheader("🚦 Análisis PRO")
    ...
    # ====== Ingresos LY y cierre LY ======
    st.divider()
    st.subheader("💶 Ingresos LY y cierre LY")
    g1, g2, g3 = st.columns(3)
    g1.metric("Ingresos LY (a este corte)", f"{tot_ly_cut['ingresos']:.2f}")
    g2.metric("Ingresos LY final", f"{tot_ly_final['ingresos']:.2f}")
    g3.metric("Δ vs LY final", f"{(tot_now['ingresos'] - tot_ly_final['ingresos']):+.2f}")

    # ====== ADR histórico (Actual, LY y LY-2) ======
    st.subheader("🏷️ ADR histórico (a este corte)")
    ...
    h1.metric("ADR actual (€)", f"{tot_now['adr']:.2f}")
    h2.metric("ADR LY (€)", f"{tot_ly_cut['adr']:.2f}")
    h3.metric("ADR LY-2 (€)", f"{tot_ly2_cut['adr']:.2f}")

    # ====== Bandas de ADR por mes (P10/P50/P90) + ADR OTB ======
    st.subheader("📦 Bandas de ADR por mes")
    start_dt = pd.to_datetime(pro_start); end_dt = pd.to_datetime(pro_end)
    months_range = pd.period_range(start_dt.to_period("M"), end_dt.to_period("M"), freq="M")

    # Base de reservas para percentiles (a este corte)
    ...
    mask = ~((dfb["Fecha salida"] <= start_dt) | (dfb["Fecha entrada"] >= (end_dt + pd.Timedelta(days=1))))
    dfb = dfb[mask]
    ...
    if not dfb.empty:
        ...
        bands_wide = pd.DataFrame(columns=["P10","P25","Mediana","P75","P90"])

    # ADR OTB actual por mes (a este corte)
    adr_otb_map = {}
    for p in months_range:
        m_start, m_end = p.start_time, p.end_time
        _bp_m, tot_m = compute_kpis(
            raw, pd.to_datetime(pro_cut), m_start, m_end,
            int(inv_pro) if inv_pro > 0 else None,
            props_pro if props_pro else None
        )
        adr_otb_map[str(p)] = float(tot_m["adr"])

    # Gráfico bandas + ADR OTB
    if not bands_wide.empty:
        plot = bands_wide[["P10","Mediana","P90"]].copy()
        plot["ADR OTB"] = [adr_otb_map.get(ym, np.nan) for ym in plot.index]
        plot = plot.reset_index().rename(columns={"index":"Mes"})
        base = alt.Chart(plot).encode(x=alt.X("Mes:N", sort=plot["Mes"].tolist()))
        band = base.mark_area(opacity=0.15, color="#1f77b4").encode(y="P10:Q", y2="P90:Q")
        med = base.mark_line(color="#1f77b4").encode(y="Mediana:Q")
        otb = base.mark_line(color="#ff7f0e").encode(y="ADR OTB:Q")
        st.altair_chart(band + med + otb, use_container_width=True)
        st.dataframe(bands_wide, use_container_width=True)
        st.download_button(
            "📥 Descargar ADR bands (CSV)",
            data=bands_wide.reset_index().to_csv(index=False).encode("utf-8-sig"),
            file_name="adr_bands_cdmpro.csv", mime="text/csv"
        )
    else:
        st.info("Sin datos suficientes para bandas de ADR en el periodo.")

    # ====== Evolución de la ocupación por fecha de corte (integrado) ======
    st.subheader("📈 Evolución de ocupación por fecha de corte (integrado)")
    with st.expander("Ver evolución", expanded=False):
        ...
        run_evo = st.button("Calcular evolución (ocupación)", type="primary", key="btn_evo_pro")
        ...
        if run_evo:
            ...
            if cstart > cend:
                ...
                for c in pd.date_range(cstart, cend, freq="D"):
                    ...
                    rows.append({"Corte": c.normalize(), "Ocupación %": float(tot["ocupacion_pct"])})
                evo_df = pd.DataFrame(rows)
                st.line_chart(evo_df.set_index("Corte"), height=320)
                st.dataframe(evo_df, use_container_width=True)
                st.download_button(
                    "📥 Descargar evolución (CSV)",
                    data=evo_df.to_csv(index=False).encode("utf-8-sig"),
                    file_name="evolucion_ocupacion_cdmpro.csv",
                    mime="text/csv",
                )

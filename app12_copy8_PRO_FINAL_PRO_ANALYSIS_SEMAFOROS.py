...
def _kai_cdm_pro_analysis(tot_now: dict, tot_ly_cut: dict, tot_ly_final: dict, pace: dict, price_ref_p50: float|None) -> str:
    """
    Análisis PRO con razonamiento:
    - Atribución de RevPAR: ¿qué pesa más, precio u ocupación?
    - ¿Bajada temprana de precio? (ADR vs P50)
    - Viabilidad del cierre: gap de ingresos vs LY final y cobertura esperada (pickup*ADR tail)
    - Escenario de pricing (elasticidad simple) para cerrar el gap
    - Recomendaciones priorizadas
    """
    import math
    # === Lectura robusta de KPIs ===
    ...
    # === Gap vs LY final (ingresos)
    gap_ing_final = _pct(ing_now, ing_ly_final)  # si negativo, vamos por detrás
    falta_ingresar_vs_LYfinal = (ing_ly_final - ing_now) if (np.isfinite(ing_ly_final) and np.isfinite(ing_now)) else np.nan

    # === Atribución del RevPAR: descomposición logarítmica (RevPAR = Occ * ADR)
    #    Δln(RevPAR) ≈ Δln(Occ) + Δln(ADR)
    def _dlog(a,b):
        try:
    ...
    dlog_tot = dlog_occ + dlog_adr if (np.isfinite(dlog_occ) and np.isfinite(dlog_adr)) else np.nan

    # Contribuciones en p.p. al Δ% de RevPAR (aprox.):
    ...
    # Nota: suma ≈ Δ% RevPAR en p.p. (cuando cambios son moderados)

    # === Referencia de precio del periodo (P50) y bandera de "bajada temprana"
    baja_temprana = False
    delta_vs_p50 = np.nan
    ...
    # === Viabilidad: cubrir el gap con pickup típico y ADR “tail”
    pickup_typ   = pace.get("pickup_typ_p50", np.nan) if isinstance(pace, dict) else np.nan
    ...
    extra_ing_esperado = np.nan
    cobertura_gap = np.nan
    if np.isfinite(pickup_typ) and np.isfinite(adr_tail_p50) and np.isfinite(falta_ingresar_vs_LYfinal):
        extra_ing_esperado = pickup_typ * adr_tail_p50
        if falta_ingresar_vs_LYfinal > 0:
            cobertura_gap = extra_ing_esperado / falta_ingresar_vs_LYfinal

    # === Escenario de pricing simple (elasticidad) para cerrar el gap (si aplica)
    #     Supuesto conservador: elasticidad demanda-precio ε ≈ -0.35
    rec_precio = ""
    if np.isfinite(falta_ingresar_vs_LYfinal) and falta_ingresar_vs_LYfinal > 0 and np.isfinite(extra_ing_esperado):
        eps = -0.35
        # probamos un ajuste de ADR en tail, x ∈ [-10%, +10%]
        best = None
        for x in np.linspace(-0.10, 0.10, 21):
            adr_new  = adr_tail_p50 * (1+x) if np.isfinite(adr_tail_p50) else np.nan
            pick_new = pickup_typ * (1 + eps*x) if np.isfinite(pickup_typ) else np.nan
            if not (np.isfinite(adr_new) and np.isfinite(pick_new)): 
                continue
            rev_tail = adr_new * max(pick_new, 0)
            gap_cov  = rev_tail / falta_ingresar_vs_LYfinal if falta_ingresar_vs_LYfinal>0 else np.nan
            score    = -abs(1-gap_cov)  # cuanto más cerca de 1, mejor
            cand = (x, gap_cov, rev_tail)
            if (best is None) or (score > best[0]): 
                best = (score, cand)
        if best:
            x, gap_cov, rev_tail = best[1]
            # Redondeo y texto
            x_pct = f"{x*100:+.0f}%"
            if gap_cov >= 1.05:
                rec_precio = f"🟢 Un ajuste de ADR tail de **{x_pct}** cubriría el gap con margen (≈ {gap_cov*100:.0f}% del faltante)."
            elif 0.85 <= gap_cov < 1.05:
                rec_precio = f"🟠 Un ajuste de ADR tail de **{x_pct}** casi cubre el gap (≈ {gap_cov*100:.0f}%). Complementa con demanda (visibilidad/promos)."
            else:
                rec_precio = f"🔴 Incluso con **{x_pct}** de ajuste en ADR tail, sólo cubriríamos ≈ {gap_cov*100:.0f}% del gap. Necesitamos activar **más demanda**."

    # === Narrativa / diagnóstico ===
    lines = []
    lines.append("**Resumen ejecutivo PRO (con diagnóstico)**")
    lines.append(
        f"- RevPAR vs LY: {_fmt_pct(d_revpar)} · despiece ≈ "
        f"**Ocupación**: {contrib_occ_pp:+.1f} p.p. · **ADR**: {contrib_adr_pp:+.1f} p.p."
        if np.isfinite(contrib_occ_pp) and np.isfinite(contrib_adr_pp) else
        f"- RevPAR vs LY: {_fmt_pct(d_revpar)}"
    )
    lines.append(
        f"- Ocupación vs LY: {_fmt_pct(d_occ)} · ADR vs LY: {_fmt_pct(d_adr)} · Ingresos vs LY: {_fmt_pct(d_ing)}"
    )

    # Juicio de “vendemos barato / caro”
    if np.isfinite(d_adr) and np.isfinite(d_occ):
        if d_adr < -0.03 and d_occ <= 0.01:
            lines.append("🔴 **Precio bajo sin ganancia de volumen**: estamos comprando ocupación sin retorno.")
        elif d_adr < -0.03 and d_occ > 0.01:
            lines.append("🟠 **Volumen comprado**: sube ocupación a costa de ADR; verifica que el ingreso total compense.")
        elif d_adr > 0.03 and d_occ < -0.01:
            lines.append("🟠 **Precio alto** penalizando volumen; focaliza rebajas en días valle / huecos sueltos.")
        else:
            lines.append("🟢 **Trade-off equilibrado** entre precio y volumen.")

    # Bandera de bajada temprana
    if baja_temprana:
        lines.append("🔴 **Posible bajada temprana de tarifa**: ADR por debajo del P50 histórico sin mejora clara de ocupación.")

    # Viabilidad de cierre
    if np.isfinite(falta_ingresar_vs_LYfinal) and falta_ingresar_vs_LYfinal > 0:
        txt_cov = ("—" if not np.isfinite(cobertura_gap) else f"{cobertura_gap*100:.0f}%")
        lines.append(
            f"- Gap vs LY final (ingresos): {_fmt_pct(gap_ing_final)} · faltan **{_fmt_money(falta_ingresar_vs_LYfinal)}**."
        )
        if np.isfinite(extra_ing_esperado):
            lines.append(
                f"- Con pickup P50 y ADR tail: expectativa ≈ **{_fmt_money(extra_ing_esperado)}** (cobertura del gap: **{txt_cov}**)."
            )
        if rec_precio:
            lines.append(rec_precio)
    elif np.isfinite(falta_ingresar_vs_LYfinal) and falta_ingresar_vs_LYfinal <= 0:
        lines.append("🟢 **Ingresos ya por encima del cierre LY** a esta fecha de corte (viabilidad alta).")

    # Acciones priorizadas (razonadas)
    acciones = []
    # 1) Cuando ADR < P50 y no hay mejora de occ → activar demanda antes de seguir bajando
    if (price_ref_p50 is not None and np.isfinite(price_ref_p50) and np.isfinite(adr_now) and adr_now < price_ref_p50*0.95) and (not np.isfinite(d_occ) or d_occ <= 0.0):
        acciones.append("Activar **demanda**: visibilidad/partners y *fences* (mín. 2 noches / NR) antes de más bajadas de precio.")
    # 2) Si pickup necesario >> típico → refuerzo de captación
    need = pace.get("pickup_needed_p50", np.nan) if isinstance(pace, dict) else np.nan
    typ  = pace.get("pickup_typ_p50", np.nan)     if isinstance(pace, dict) else np.nan
    if np.isfinite(need) and np.isfinite(typ) and need > 1.25*typ:
        acciones.append("**Pickup exigente**: campañas de captación y flexibilizar mínimos en días valle.")
    # 3) Si ADR muy por debajo de LY → revisar descuentos
    if np.isfinite(d_adr) and d_adr < -0.05:
        acciones.append("**ADR bajo vs LY**: retirar descuentos poco convertidores y concentrarlos en huecos concretos.")
    # 4) Si ADR muy por encima de LY con occ débil → micro-rebajas quirúrgicas
    if np.isfinite(d_adr) and d_adr > 0.05 and np.isfinite(d_occ) and d_occ < -0.01:
        acciones.append("**ADR alto con occ débil**: micro-rebajas en días valle; sostener picos.")
    # 5) Si cobertura del gap < 85% → combinar precio+demanda
    if np.isfinite(cobertura_gap) and cobertura_gap < 0.85:
        acciones.append("**Cobertura insuficiente del gap**: combinar *boost* de demanda con ajustes de ADR tail ±5–10%.")
    if not acciones:
        acciones.append("Mantener estrategia actual y monitorizar pickup semanal; ajustes quirúrgicos por DOW/eventos.")

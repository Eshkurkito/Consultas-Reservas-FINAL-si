...
def _kai_cdm_pro_analysis(tot_now: dict, tot_ly_cut: dict, tot_ly_final: dict, pace: dict, price_ref_p50: float|None) -> str:
    """
    Análisis PRO con:
      • Semáforo global (estado rápido del periodo).
      • Análisis técnico detallado.
      • Explicación ejecutiva en lenguaje sencillo (Streamlit expander).
    """
    import math
    ...
    # Atribución RevPAR
    contrib_occ_pp = _dlog(occ_now, occ_ly) * 100 if np.isfinite(_dlog(occ_now, occ_ly)) else np.nan
    contrib_adr_pp = _dlog(adr_now, adr_ly) * 100 if np.isfinite(_dlog(adr_now, adr_ly)) else np.nan
    ...
    if isinstance(pace, dict):
        ...
        # Cobertura gap con escenario P50
        if (np.isfinite(need_p50) and np.isfinite(adr_tail)
            and np.isfinite(falta_ingresar_vs_LYfinal) and falta_ingresar_vs_LYfinal > 0):
    ...
    explic = []
    explic.append("**🔎 Interpretación paso a paso**")

    # Veredicto
    if np.isfinite(d_adr) and np.isfinite(d_occ):
        if d_adr > 0.03 and d_occ < -0.01:
            explic.append("**👉 Veredicto: 🟠 Precio alto con penalización**")
            explic.append(f"Subiste ADR ({_abs_pct(d_adr)}), pero la ocupación cayó fuerte ({_abs_pct(d_occ)}).")
            explic.append("El precio está penalizando la demanda: más caro, pero vendes menos volumen.")
        elif d_adr < -0.03 and d_occ > 0.01:
            explic.append("**👉 Veredicto: 🟠 Volumen comprado (precio bajo)**")
            explic.append(f"El ADR bajó ({_abs_pct(d_adr)}), la ocupación subió ({_abs_pct(d_occ)}). Revisa si compensa en ingresos.")
        elif d_adr < -0.03 and d_occ < -0.03:
            explic.append("**👉 Veredicto: 🔴 Problema de demanda**")
            explic.append("Se cae la ocupación y además el ADR está más bajo → hay que activar demanda.")
        else:
            explic.append("**👉 Veredicto: 🟢 Balance razonable**")

    # Evolución vs LY
    explic.append("")
    explic.append("**👉 Evolución vs LY (a corte actual)**")
    explic.append(f"- Ocupación: {_fmt_pct(d_occ)}")
    explic.append(f"- ADR: {_fmt_pct(d_adr)}")
    explic.append(f"- RevPAR: {_fmt_pct(d_revpar)}")
    explic.append(f"- Ingresos: {_fmt_pct(d_ing)}")

    # Atribución
    if np.isfinite(contrib_occ_pp) and np.isfinite(contrib_adr_pp):
        explic.append("")
        explic.append("**👉 Atribución RevPAR (p.p.)**")
        explic.append(f"- Ocupación {contrib_occ_pp:+.1f} p.p.")
        explic.append(f"- ADR {contrib_adr_pp:+.1f} p.p.")

    # Gap de ingresos
    explic.append("")
    explic.append("**👉 Viabilidad de cierre (gap vs LY final)**")
    if np.isfinite(falta_ingresar_vs_LYfinal):
        if falta_ingresar_vs_LYfinal > 0:
            explic.append(f"- Faltan {_fmt_money(falta_ingresar_vs_LYfinal)} para igualar el cierre LY.")
            explic.append("Con el pickup esperado, el gap no se cubre → hace falta **boost de demanda**, no sólo precio.")
        else:
            explic.append("🟢 Ya estás por encima del cierre LY en ingresos.")
    else:
        explic.append("— No se pudo estimar el gap vs LY final.")

    # Plan de acción
    explic.append("")
    explic.append("**👉 Plan de acción (prioridad)**")
    if np.isfinite(d_adr) and np.isfinite(d_occ) and d_adr > 0.03 and d_occ < -0.01:
        explic.append("- Micro-rebajas **quirúrgicas** en días valle; sostener precios en picos.")
    explic.append("- Activar **boost de demanda**: visibilidad, canales secundarios y promos puntuales.")

    # Mostrar en expander
    exp_text = "\n".join(explic)
    st.markdown("### 🧠 Explicación ejecutiva")
    with st.expander("Ver interpretación paso a paso", expanded=False):

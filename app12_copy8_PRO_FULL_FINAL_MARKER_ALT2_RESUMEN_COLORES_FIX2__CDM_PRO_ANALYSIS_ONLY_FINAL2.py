...
def _kai_cdm_pro_analysis(tot_now: dict, tot_ly_cut: dict, tot_ly_final: dict, pace: dict, price_ref_p50: float|None) -> str:
    """
    Análisis ejecutivo del Cuadro de mando PRO:
    - Actual vs LY a misma fecha de corte
    - Gap vs LY final
    - Señales (bajada temprana de ADR, trade-off, precio alto sin volumen, pickup difícil)
    ...
    if price_ref_p50 is not None and np.isfinite(price_ref_p50):
        lines.append(f"- **ADR P50 histórico** del periodo: {price_ref_p50:.2f}€; ADR actual: {adr_now:.2f}€.")
        if adr_now < price_ref_p50 * 0.90:
            ...
            lines.append("- Señal: **tarifa alta** (> P50 +15%). Revisar conversión en días valle.")

    if (not pd.isna(d_adr)) and (not pd.isna(d_occ)):
        if d_adr < 0 and d_occ <= 0:
            lines.append("- **Precio bajo sin ganar volumen** → revisar mix y fences; no compramos ocupación eficientemente.")
            ...
            lines.append("- **Precio alto** penalizando volumen → proteger picos y ajustar en días valle.")
    ...
    if isinstance(pace, dict):
        ...
        if np.isfinite(need) and np.isfinite(typ):
            if need > typ * 1.25:
                lines.append(f"- **Pickup pendiente alto**: faltan {need:.0f} noches; típico P50 ≈ {typ:.0f}. Activar campañas/partners.")
            elif need < typ * 0.75:
        ...
        if np.isfinite(tail_adr):
            lines.append(f"- ADR estimado *tail* (P50): {tail_adr:.2f}€.")
    ...
    lines.append("- **Test A/B ±5–10%** por tipología; prioriza donde el impacto en ingresos por punto de ADR sea mayor.")
    lines.append("- Priorizar canal directo cuando el **pickup pendiente** sea alto para reducir coste de adquisición.")

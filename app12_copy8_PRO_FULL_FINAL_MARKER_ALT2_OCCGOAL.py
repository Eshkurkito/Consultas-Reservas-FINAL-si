...
# ---------- Cuadro de mando PRO (con ingresos YoY) ----------
    ...
    if run_pro:
        ...
        # --- Objetivo de ocupación → ADR necesario ---
        st.markdown("### 🎯 Objetivo de ocupación → ADR necesario")
        with st.container():
            # Entrada de objetivo de ocupación y base de ingresos
            occ_goal = st.slider("Ocupación deseada (%)", 10, 100, int(round(occ)) if not np.isnan(occ) else 70, 1, key="pro_occ_goal")
            base = st.radio(
                "Base de ingresos",
                ["Mantener ingresos OTB", "Mantener ingresos forecast (P50)"],
                horizontal=False,
                key="pro_income_base"
            )
            ingresos_base = float(tot["ingresos"]) if base == "Mantener ingresos OTB" else float(forecast.get("revenue_final_p50", float(tot["ingresos"])))
            noches_disp = float(tot.get("noches_disponibles", 0.0))
            noches_objetivo = noches_disp * max(float(occ_goal), 0.0) / 100.0
            adr_necesario = (ingresos_base / noches_objetivo) if noches_objetivo > 0 else 0.0

            # Métricas resumidas
            cA, cB, cC = st.columns(3)
            cA.metric("ADR necesario", f"{adr_necesario:.2f} €")
            cB.metric("ADR OTB", f"{tot['adr']:.2f} €", delta=f"{(adr_necesario - float(tot['adr'])):.2f}")
            cC.metric("ADR final P50", f"{float(forecast.get('adr_final_p50', 0.0)):.2f} €", delta=f"{(adr_necesario - float(forecast.get('adr_final_p50', 0.0))):.2f}")

            # Contexto extra
            st.caption("Cálculo: **ingresos_base ÷ (noches_disponibles × ocupación_deseada)**. Ingresos_base = OTB actual o Forecast P50.")
            if noches_objetivo < float(tot.get("noches_ocupadas", 0.0)):
                st.info("Ya tienes más noches OTB que el objetivo de ocupación. El ADR necesario asume mantener los ingresos base con menos noches.")

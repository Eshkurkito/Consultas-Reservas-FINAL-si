...
def group_selector(label: str, all_props: list[str], key_prefix: str, default: Optional[list[str]] = None) -> list[str]:
    ...
    # Map de grupo -> lista de alojamientos (normalizados y filtrados a los existentes)
    group_to_props = {}
    if not groups_df.empty:
        for g, sub in groups_df.groupby("grupo"):
            vals = sorted({str(x).strip() for x in sub["alojamiento"].tolist()})
            # solo los que existan en las opciones reales
            group_to_props[g] = [p for p in vals if p in all_props_sorted]

    # Selección inicial
    initial = default or st.session_state.get(f"{key_prefix}_props", [])
    ...
    # Garantiza que todos existen en opciones
    initial = [p for p in initial if p in all_props_sorted]

    # Multiselect
    props = st.multiselect(label, options=all_props_sorted, default=initial, key=f"{key_prefix}_props")
    ...
    with c4:
        if st.button("Usar este grupo", key=f"{key_prefix}_grp_use"):
            # Si el multiselect está vacío, usa directamente los alojamientos del grupo elegido
            ...
            # Refleja la selección en este multiselect también
            st.session_state[f"{key_prefix}_props"] = selected

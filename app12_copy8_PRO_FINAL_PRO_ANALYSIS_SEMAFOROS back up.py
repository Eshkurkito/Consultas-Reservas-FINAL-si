...
def group_selector(label: str, all_props: list[str], key_prefix: str, default: Optional[list[str]] = None) -> list[str]:
    ...
    # Si hay grupo global activo, úsalo como inicial
    ...
    # Si el usuario elige un grupo concreto, prima ese listado
    if grp_sel and grp_sel != "(ninguno)" and not groups_df.empty:
        initial = sorted(groups_df.loc[groups_df["grupo"] == grp_sel, "alojamiento"].astype(str).tolist())
    # Multiselect
    ...
    with c4:
        if st.button("Usar este grupo", key=f"{key_prefix}_grp_use"):
            st.session_state["global_props"] = list(props)
            ...
            st.success(f"Grupo en uso: {st.session_state['global_group_name']} · {len(props)} aloj.")
    return props

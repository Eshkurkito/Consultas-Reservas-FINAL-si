...
if mode == "KPIs por meses":
    ...
    if selected_months_m:
        ...
        try:
            # Construir tabla compacta en el orden solicitado
            if compare_m and 'df_prev' in locals() and not df_prev.empty:
                act = df_actual[['Mes', 'adr', 'ocupacion_pct']].rename(columns={
                    'adr': 'ADR (€)', 'ocupacion_pct': 'Ocupación %'
                })
                prev = df_prev[['Mes', 'adr', 'ocupacion_pct']].rename(columns={
                    'adr': 'ADR LY (€)', 'ocupacion_pct': 'Ocupación LY %'
                })
                export_df = act.merge(prev, on='Mes', how='left')
                export_df = export_df[['Mes', 'ADR (€)', 'ADR LY (€)', 'Ocupación %', 'Ocupación LY %']]
            else:
                export_df = df_actual[['Mes', 'adr', 'ocupacion_pct']].rename(columns={
                    'adr': 'ADR (€)', 'ocupacion_pct': 'Ocupación %'
                })
            ...
            with pd.ExcelWriter(buffer_xlsx, engine="xlsxwriter") as writer:
                ...
                # Colores condicionales si hay columnas LY
                if compare_m and "ADR LY (€)" in export_df.columns and "Ocupación LY %" in export_df.columns:
                    from xlsxwriter.utility import xl_rowcol_to_cell
                    fmt_green = wb.add_format({"bg_color": "#d4edda", "font_color": "#155724", "bold": True})
                    fmt_red   = wb.add_format({"bg_color": "#f8d7da", "font_color": "#721c24", "bold": True})
                    n = len(export_df)
                    if n > 0:
                        ...
                        # ADR actual vs ADR LY
                        idx_adr     = export_df.columns.get_loc("ADR (€)")
                        idx_adr_ly  = export_df.columns.get_loc("ADR LY (€)")
                        adr_cell    = xl_rowcol_to_cell(first_row, idx_adr,    row_abs=False, col_abs=True)
                        adrly_cell  = xl_rowcol_to_cell(first_row, idx_adr_ly, row_abs=False, col_abs=True)
                        ws.conditional_format(first_row, idx_adr, last_row, idx_adr, {
                            "type": "formula", "criteria": f"={adr_cell}>{adrly_cell}", "format": fmt_green
                        })
                        ws.conditional_format(first_row, idx_adr, last_row, idx_adr, {
                            "type": "formula", "criteria": f"={adr_cell}<{adrly_cell}", "format": fmt_red
                        })

                        # Ocupación actual vs Ocupación LY
                        idx_occ     = export_df.columns.get_loc("Ocupación %")
                        idx_occ_ly  = export_df.columns.get_loc("Ocupación LY %")
                        occ_cell    = xl_rowcol_to_cell(first_row, idx_occ,    row_abs=False, col_abs=True)
                        occly_cell  = xl_rowcol_to_cell(first_row, idx_occ_ly, row_abs=False, col_abs=True)
                        ws.conditional_format(first_row, idx_occ, last_row, idx_occ, {
                            "type": "formula", "criteria": f"={occ_cell}>{occly_cell}", "format": fmt_green
                        })
                        ws.conditional_format(first_row, idx_occ, last_row, idx_occ, {
                            "type": "formula", "criteria": f"={occ_cell}<{occly_cell}", "format": fmt_red
                        })

...
# =============================
    ...
    if run_tar:
        ...
        st.markdown("—")
        st.subheader("📅 Calendario (previsualización)")
        grid_wide, meta_cols = build_pricing_calendar_grid(result_df, eventos_daily)
        if grid_wide.empty:
            st.info("No hay datos para el calendario.")
        else:
            styled = style_pricing_calendar(grid_wide, meta_cols)
            st.dataframe(styled, use_container_width=True)

            # Guarda en sesión para el modo independiente
            st.session_state["pricing_grid"] = grid_wide
            st.session_state["pricing_grid_meta"] = meta_cols

            # Export matriz
            csv_matrix = grid_wide.reset_index().rename(columns={"Alojamiento":"Alojamiento"})
            st.download_button(
                "⬇️ Descargar matriz (CSV)",
                data=csv_matrix.to_csv(index=False).encode("utf-8-sig"),
                file_name="tarifas_matriz_calendario.csv",
                mime="text/csv",
            )

        st.caption("Leyenda: gris = fin de semana, amarillo = evento (tooltip).")

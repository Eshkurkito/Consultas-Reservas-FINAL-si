...
# ---------- Cuadro de mando PRO (con ingresos YoY) ----------
    ...
    # ============================
    # BLOQUE AÑADIDO — Reflexión fija + Export PDF (sin matplotlib)
    # ============================
    try:
        import numpy as _np
    ...
    # ======= Motor de análisis dinámico (texto profesional) =======
    def _analytic_reflection_text(_occ_now, _occ_ly, _adr_now, _adr_ly, _rpar_now, _rpar_ly):
        import numpy as _np
        # Tasas como 0-1
        _occ_now_r = _occ_now/100.0
        _occ_ly_r  = _occ_ly/100.0

        # Descomposición de RevPAR: RevPAR = ADR * Occ
        delta_revpar = _rpar_now - _rpar_ly
        occ_contrib  = _adr_ly * (_occ_now_r - _occ_ly_r)              # € por efecto ocupación
        adr_contrib  = (_adr_now - _adr_ly) * _occ_ly_r                # € por efecto ADR
        cross_term   = (_adr_now - _adr_ly) * (_occ_now_r - _occ_ly_r) # interacción
        explained = occ_contrib + adr_contrib + cross_term

        # Señales
        d_occ_pp  = _occ_now - _occ_ly
        d_adr_eur = _adr_now - _adr_ly
        d_rpar_eur = delta_revpar
        d_rpar_pct = (delta_revpar/_rpar_ly*100.0) if _rpar_ly > 0 else (0.0 if delta_revpar==0 else (100.0 if delta_revpar>0 else -100.0))
        ...
        lines.append(f"**Resumen:** Este año estás **{dir_occ}** en ocupación (**{_occ_now:.2f}%** vs **{_occ_ly:.2f}%**) y **{dir_rpar}** en RevPAR (**{_rpar_now:.2f} €** vs **{_rpar_ly:.2f} €**, {d_rpar_pct:+.1f}%).")

        # Lectura analítica según escenario
        if adr_dn and occ_up:
            # Sacrificas ADR para llenar más; ¿compensa?
            if d_rpar_eur < 0:
                lines.append("**Lectura:** Has **bajado ADR** para ganar ocupación, pero **no ha compensado**: el RevPAR cae. La elasticidad indica que el volumen adicional no cubre la pérdida de precio.")
                # Comparar contribuciones
                if abs(adr_contrib) > abs(occ_contrib) + abs(cross_term):
                    lines.append("**Causa principal:** La **caída de ADR** pesa más que el efecto positivo de ocupación.")
                else:
                    lines.append("**Causa principal:** La mejora de ocupación **no ha sido suficiente** para compensar la pérdida de ADR.")
            else:
                lines.append("**Lectura:** Has **bajado ADR** y **subido ocupación** y **sí ha compensado**: el RevPAR sube. El mix volumen-precio ha sido eficiente en este periodo.")
        elif adr_up and occ_dn:
            if d_rpar_eur >= 0:
                lines.append("**Lectura:** Has **protegido tarifa (ADR ↑)** sacrificando algo de ocupación, y **ha compensado**: el RevPAR mejora.")
            else:
                lines.append("**Lectura:** Aunque **subiste ADR**, la **pérdida de ocupación** fue mayor y el RevPAR cae. El punto de precio puede haber superado la elasticidad del mercado.")
        elif adr_up and occ_up:
            lines.append("**Lectura:** **ADR y ocupación mejoran a la vez**: excelente señal de **demanda sólida** y **posicionamiento**. Es buen momento para testar incrementos de precio selectivos.")
        elif adr_dn and occ_dn:
            lines.append("**Lectura:** **ADR y ocupación caen** simultáneamente: alerta roja. Revisa competitividad, visibilidad y condiciones; puede haber un problema de posicionamiento o distribución.")
        else:
            # Cambios leves o mixtos
            if abs(d_rpar_eur) < max(1.0, 0.02*_rpar_ly):
                lines.append("**Lectura:** Cambios leves: el RevPAR se mantiene estable. Ajustes finos de precio o estancias mínimas pueden optimizar el rendimiento.")
            else:
                lines.append("**Lectura:** El patrón es mixto; revisa segmentos, ventanas de reserva y días con mayor sensibilidad al precio.")

        # Implicación operativa adaptativa
        if d_rpar_eur < 0 and adr_dn and occ_up:
            lines.append("**Implicación operativa:** Recupera **precio** en picos de demanda y limita descuentos en días de baja elasticidad. Evalúa **estancias mínimas**, cierres parciales de disponibilidad y upselling.")
        elif d_rpar_eur < 0 and adr_up and occ_dn:
            lines.append("**Implicación operativa:** Ajusta estrategia de **precio por demanda** (picos) y mejora conversión (fotos, título, políticas). Considera ofertas tácticas en huecos sueltos para reactivar ocupación sin erosionar tarifa base.")
        elif d_rpar_eur >= 0 and adr_up:
            lines.append("**Implicación operativa:** Mantén la **protección de tarifa**. Prueba incrementos controlados en eventos/fechas pico y consolida segmentos de alta disposición a pagar.")
        elif d_rpar_eur >= 0 and occ_up and adr_dn:
            lines.append("**Implicación operativa:** El enfoque de volumen funciona aquí; monitoriza **coste marginal** y aplica cercas tarifarias para no diluir ADR en fines de semana o eventos.")
        else:
            lines.append("**Implicación operativa:** Ajuste fino: usa sensibilidad por día de la semana y eventos. A/B testea restricciones y suplementos (late check-in/out, cleaning).")

        # Devuelve texto markdown (una lista de líneas unidas por doble salto)
        return "\n\n".join(lines), {"delta_revpar": d_rpar_eur, "occ_contrib": occ_contrib, "adr_contrib": adr_contrib, "cross_term": cross_term}
    ...
    def _safe_num(v):
        try:
            ...
            pass
        try:
            return float(v) if v is not None and _pd.notna(v) and _np.isfinite(v) else 0.0
        except Exception:
            try:
                return float(v)
            except Exception:
    ...
    # Intentamos usar tot_now/tot_ly ya existentes; si no están, los calculamos rápido
    _need_compute = False
    for _k in ["tot_now","tot_ly"]:
        if _k not in locals():
            _need_compute = True
            break
    if _need_compute:
        # Fallback: requerimos que existan cutoff_pro, start_pro, end_pro y helpers
        _props_sel = locals().get("props_pro", [])
        _props_sel = _props_sel if _props_sel else None
        _inv_override = int(locals().get("inv_pro", 0)) if locals().get("inv_pro", 0) > 0 else None

        _by_now, tot_now = compute_kpis(
            df_all=raw,
            cutoff=_pd.to_datetime(locals().get("cutoff_pro")),
            period_start=_pd.to_datetime(locals().get("start_pro")),
            period_end=_pd.to_datetime(locals().get("end_pro")),
            inventory_override=_inv_override,
            filter_props=_props_sel
        )
        _cutoff_ly = _pd.to_datetime(locals().get("cutoff_pro")) - _pd.DateOffset(years=1)
        _start_ly  = _pd.to_datetime(locals().get("start_pro")) - _pd.DateOffset(years=1)
        _end_ly    = _pd.to_datetime(locals().get("end_pro"))   - _pd.DateOffset(years=1)
        _inv_override_ly = int(locals().get("inv_pro_prev", 0)) if locals().get("inv_pro_prev", 0) > 0 else None
        _by_ly, tot_ly = compute_kpis(
            df_all=raw,
            cutoff=_cutoff_ly, period_start=_start_ly, period_end=_end_ly,
            inventory_override=_inv_override_ly, filter_props=_props_sel
        )

    _occ_now = _safe_num(tot_now.get("ocupacion_pct", 0))
    _occ_ly  = _safe_num(tot_ly.get("ocupacion_pct", 0))
    _adr_now = _safe_num(tot_now.get("adr", 0))
    _adr_ly  = _safe_num(tot_ly.get("adr", 0))
    _rpar_now = _safe_num(tot_now.get("revpar", 0))
    _rpar_ly  = _safe_num(tot_ly.get("revpar", 0))

    _delta_revpar_pct = ((_rpar_now - _rpar_ly) / _rpar_ly * 100.0) if _rpar_ly > 0 else 0.0
    _adr_needed = None
    _nights_disp = _safe_num(tot_now.get("noches_disponibles", 0))
    _nights_occ  = _safe_num(tot_now.get("noches_ocupadas", 0))
    if _nights_occ > 0 and _nights_disp > 0 and _rpar_ly > 0:
        _adr_needed = _rpar_ly * _nights_disp / _nights_occ

    
with st.expander("💬 Análisis automático", expanded=True):
        _analytic_md, _decomp = _analytic_reflection_text(_occ_now, _occ_ly, _adr_now, _adr_ly, _rpar_now, _rpar_ly)
        st.markdown(_analytic_md)
        if _adr_needed is not None:
            st.markdown(f"**ADR objetivo** para igualar el **RevPAR LY** con tu **ocupación actual**: **{_adr_needed:.2f} €** (ADR actual: {_adr_now:.2f} €).")

    # ======= Exportar PDF sin matplotlib (ReportLab puro) =======
 (ReportLab puro) =======
    st.divider()
    st.subheader("📑 Exportar panel (PDF sin gráficos)")
    ...
    if _rl_ok and st.button("📥 Generar PDF (branding + header/pie)"):
        ...
        # Header/footer
        def _header_footer(canv: rl_canvas.Canvas, doc_):
            canv.saveState()
            width, height = A4
            ...
            try:
                _sub1 = f"Periodo: {pd.to_datetime(start_pro).date()} → {pd.to_datetime(end_pro).date()}"
                _sub2 = f"Corte: {pd.to_datetime(cutoff_pro).date()}"
            except Exception:
                _sub1 = ""
                _sub2 = ""
            _props_txt = ", ".join(props_pro) if locals().get("props_pro") else "Grupo completo"
            canv.drawString(x_left + 40*mm, height - 16*mm, _sub1)
            canv.drawString(x_left + 40*mm, height - 20*mm, _sub2 + "   |   " + f"Filtro: {_props_txt}")
            canv.setLineWidth(0.5)
            canv.setStrokeColorRGB(0.6, 0.6, 0.6)
            canv.line(x_left, height - 22*mm, width - doc_.rightMargin, height - 22*mm)
            from datetime import datetime as _dt
            now_es = _dt.now().strftime("%Y-%m-%d %H:%M")
        ...
        kpi_data = [
            ["Métrica", "Actual", "LY", "Δ"],
            ["Ocupación %", f"{_occ_now:.2f}%", f"{_occ_ly:.2f}%", f"{(_occ_now-_occ_ly):.2f} p.p."],
            ["ADR (€)", f"{_adr_now:.2f}", f"{_adr_ly:.2f}", f"{(_adr_now-_adr_ly):.2f}"],
            ["RevPAR (€)", f"{_rpar_now:.2f}", f"{_rpar_ly:.2f}", f"{(_rpar_now-_rpar_ly):.2f}"],
            ["Ingresos (€)", f"{_safe_num(tot_now.get('ingresos',0)):.2f}", f"{_safe_num(tot_ly.get('ingresos',0)):.2f}", f"{(_safe_num(tot_now.get('ingresos',0))-_safe_num(tot_ly.get('ingresos',0))):.2f}"],
        ]
        ...
        # Reflexión (mismo texto que en UI)
        _dir_occ = "por encima" if _occ_now >= _occ_ly else "por debajo"
        _dir_rev = "por encima" if _rpar_now >= _rpar_ly else "por debajo"
        
        # Reflexión (mismo análisis que en la UI)
        _analytic_md, _decomp = _analytic_reflection_text(_occ_now, _occ_ly, _adr_now, _adr_ly, _rpar_now, _rpar_ly)
        flow.append(Paragraph("💬 Análisis", style_h2))
        # Convertimos markdown simple a líneas; para PDF usamos frases separadas
        for _line in _analytic_md.split("

"):
            flow.append(Paragraph(_line.replace("**","<b>").replace("__","<b>").replace("*",""), style_normal))

        if _adr_needed is not None:
            flow.append(Spacer(1, 4))
            flow.append(Paragraph(f"ADR objetivo para igualar el RevPAR LY con tu ocupación actual: {_adr_needed:.2f} € (ADR actual: {_adr_now:.2f} €).", style_normal))

        # Detalle por alojamiento si existe en la vista
        try:
            detail_now  # noqa: F401
            if isinstance(detail_now, _pd.DataFrame) and not detail_now.empty:
                flow.append(Spacer(1, 8))
                flow.append(Paragraph("🏠 Detalle por alojamiento (actual)", style_h2))
                _detail_tbl = [list(detail_now.columns)] + detail_now.round(2).values.tolist()
                table_det = Table(_detail_tbl, hAlign="LEFT", repeatRows=1)

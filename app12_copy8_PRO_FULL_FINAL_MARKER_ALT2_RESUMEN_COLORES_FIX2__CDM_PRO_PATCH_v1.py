...
# ====== PRO ANALYSIS HELPERS (added by Kai) ======
def _kai_pct(cur, ly):
    try:
...
def _kai_build_pro_text(raw_df: pd.DataFrame,
                        cutoff: pd.Timestamp,
                        start: pd.Timestamp,
                        end: pd.Timestamp,
                        props: Optional[List[str]] = None,
                        inv_override: Optional[int] = None) -> tuple[str, dict]:
    """
    Texto analítico 'espectacular' y súper específico para el Cuadro de mando PRO.
    Señala errores típicos: bajar precio antes de tiempo, vender por debajo de referencia,
    quedarse corto en pickup, etc. Devuelve (texto, kpis_dict).
    """
    # KPIs actuales y LY (mismo periodo, mismo inventario si se indica)
    by_now, tot_now = compute_kpis(raw_df, pd.to_datetime(cutoff), pd.to_datetime(start), pd.to_datetime(end),
                                   inv_override, props)
    ...
    _bp_ly, tot_ly = compute_kpis(raw_df, cut_ly, start_ly, end_ly, inv_override, props)

    # LY final (resultado) con corte al final del periodo LY
    _bp_lyf, tot_lyf = compute_kpis(raw_df, pd.to_datetime(end_ly), start_ly, end_ly, inv_override, props)

    # Pace / pickup y ADR tail estimada usando los helpers ya existentes
    try:
        pace = pace_forecast_month(raw_df, pd.to_datetime(cutoff), pd.to_datetime(start), pd.to_datetime(end),
                                   ref_years=2, dmax=180, props=props, inv_override=inv_override)
    except Exception:
        pace = {"nights_otb": np.nan, "nights_p50": np.nan, "adr_tail_p50": np.nan,
                "pickup_needed_p50": np.nan, "pickup_typ_p50": np.nan, "adr_final_p50": np.nan, "n_refs": 0}

    # ADR bands (P50) del mes para comparar si estamos vendiendo por debajo/encima de referencia
    try:
        p50_group = adr_bands_p50_for_month(raw_df, pd.to_datetime(cutoff), pd.to_datetime(start), pd.to_datetime(end),
                                            props if props else [])
    except Exception:
        p50_group = np.nan

    # Construcción del análisis
    occ_now = float(tot_now["ocupacion_pct"])
    adr_now = float(tot_now["adr"])
    revpar_now = float(tot_now["revpar"])
    ing_now = float(tot_now["ingresos"])

    occ_ly = float(tot_ly["ocupacion_pct"])
    adr_ly = float(tot_ly["adr"])
    revpar_ly = float(tot_ly["revpar"])
    ing_ly = float(tot_ly["ingresos"])

    # Deltas vs LY a misma fecha de corte
    d_occ = _kai_pct(occ_now, occ_ly)
    d_adr = _kai_pct(adr_now, adr_ly)
    d_revpar = _kai_pct(revpar_now, revpar_ly)
    d_ing = _kai_pct(ing_now, ing_ly)

    # Gap vs LY FINAL (para saber si vamos tarde o pronto)
    gap_nights_vs_ly_final = _kai_pct(tot_now["noches_ocupadas"], tot_lyf["noches_ocupadas"])
    gap_ing_vs_ly_final = _kai_pct(tot_now["ingresos"], tot_lyf["ingresos"])

    # Señales tácticas
    signals = []
    # 1) Trade-off precio/volumen
    if d_adr is not None and d_occ is not None and (not pd.isna(d_adr)) and (not pd.isna(d_occ)):
        if d_adr < 0 and d_occ <= 0:
            signals.append("**Precio bajo sin ganancia de volumen**: Hemos bajado ADR y no hemos crecido en ocupación. Ajuste de mix/canales o revisar fences.")
        elif d_adr < 0 and d_occ > 0:
            signals.append("Hemos comprado volumen **a costa de tarifa**. Verifica que el ingreso total compense esa erosión de precio.")
        elif d_adr > 0 and d_occ < 0:
            signals.append("**Precio alto** penalizando volumen. Recalibrar días valle y huecos sueltos.")
    # 2) ¿Bajamos antes de tiempo?
    if not pd.isna(p50_group) and p50_group > 0 and adr_now < p50_group * 0.9:
        signals.append(f"**Precio por debajo del P50 histórico** ({adr_now:.2f}€ vs {p50_group:.2f}€). Posible bajada **temprana**.")
    # 3) Pickup: ¿lo que falta es razonable vs histórico?
    if "pickup_needed_p50" in pace and "pickup_typ_p50" in pace:
        need = pace.get("pickup_needed_p50", np.nan)
        typ = pace.get("pickup_typ_p50", np.nan)
        if np.isfinite(need) and np.isfinite(typ):
            if need > typ * 1.25:
                signals.append(f"**Pickup pendiente exigente**: faltan {need:.0f} noches y el típico P50 aporta ≈ {typ:.0f}. Activar palancas (promos suaves, cross-sell, campañas).")
            elif need < typ * 0.75:
                signals.append(f"**Pickup cómodo**: lo pendiente ({need:.0f} noches) es menor que un P50 típico ({typ:.0f}). Se puede sostener precio en días fuertes.")
    # 4) Gap vs LY final
    if not pd.isna(gap_nights_vs_ly_final) and gap_nights_vs_ly_final < -0.1:
        signals.append("**Vamos por detrás** de las noches finales del LY (a misma ventana). Refuerza captación a medio plazo (partners, SEO/SEM, contenidos).")
    if not pd.isna(gap_ing_vs_ly_final) and gap_ing_vs_ly_final < -0.1:
        signals.append("**Ingresos en retraso** vs cierre de LY. Revisa comisiones y prioriza canal directo.")

    # Recomendaciones accionables
    recs = []
    recs.append("Días con **ocupación < objetivo** y **ADR < P50** → sube visibilidad y abre promos con *fences* (mín. 2 noches, non-refundable).")
    recs.append("Días con **ocupación > objetivo** y **ADR > P75** → sostiene precio; sólo micro-ajustes en huecos de 1 noche.")
    recs.append("Implementa **test A/B ±5–10%** por tipología; prioriza donde |Δingreso| por punto de ADR sea mayor.")
    if pace.get("n_refs", 0) < 1:
        recs.append("Aumenta la base de referencia para Pace (carga al menos 2 años de histórico) para mejorar la estimación de *pickup*.")

    lines = []
    lines.append("**Resumen ejecutivo PRO**")
    lines.append(f"- Ocupación vs LY: {_kai_fmt_pct(d_occ)} · ADR vs LY: {_kai_fmt_pct(d_adr)} · RevPAR vs LY: {_kai_fmt_pct(d_revpar)} · Ingresos vs LY: {_kai_fmt_pct(d_ing)}.")
    lines.append(f"- Gap vs LY final (noches): {_kai_fmt_pct(gap_nights_vs_ly_final)} · Gap vs LY final (ingresos): {_kai_fmt_pct(gap_ing_vs_ly_final)}.")
    if not pd.isna(p50_group):
        lines.append(f"- **Referencia de precio (P50 ADR reserva)** del periodo: {p50_group:.2f}€; ADR actual: {adr_now:.2f}€.")
    if "adr_tail_p50" in pace and np.isfinite(pace.get("adr_tail_p50", np.nan)):
        lines.append(f"- ADR estimado en *tail* (P50 histórico): {pace['adr_tail_p50']:.2f}€. ADR final estimado (P50): {pace.get('adr_final_p50', np.nan):.2f}€.")
    if signals:
    ...
    kpis = {
        "occ_now": occ_now, "adr_now": adr_now, "revpar_now": revpar_now, "ing_now": ing_now,
        "occ_ly": occ_ly, "adr_ly": adr_ly, "revpar_ly": revpar_ly, "ing_ly": ing_ly,
        "d_occ": d_occ, "d_adr": d_adr, "d_revpar": d_revpar, "d_ing": d_ing,
        "gap_nights_vs_ly_final": gap_nights_vs_ly_final,
        "gap_ing_vs_ly_final": gap_ing_vs_ly_final,
        "p50_group": p50_group,
        "pace": pace,
    }
    return ("\n".join(lines), kpis)

def _kai_export_pro_pdf(summary_text: str, kpis: dict, title: str) -> bytes:
    """PDF profesional con ReportLab sin dependencias externas raras."""
    ...
    h = styles["Heading1"]; h.textColor = colors.HexColor("#163e64")
    p = styles["BodyText"]; p.alignment = TA_LEFT

    elements = []
    elements.append(Paragraph(title, h)); elements.append(Spacer(1, 8))
    ...
    data = [
        ["KPI", "Actual", "LY", "Δ %"],
        ["Ocupación", _fmt_pct2(kpis.get("occ_now", np.nan)/100), _fmt_pct2(kpis.get("occ_ly", np.nan)/100), _fmt_pct2(kpis.get("d_occ", np.nan))],
        ["ADR (€)", _fmt_num(kpis.get("adr_now", np.nan)), _fmt_num(kpis.get("adr_ly", np.nan)), _fmt_pct2(kpis.get("d_adr", np.nan))],
        ["RevPAR (€)", _fmt_num(kpis.get("revpar_now", np.nan)), _fmt_num(kpis.get("revpar_ly", np.nan)), _fmt_pct2(kpis.get("d_revpar", np.nan))],
        ["Ingresos (€)", _fmt_money(kpis.get("ing_now", np.nan)), _fmt_money(kpis.get("ing_ly", np.nan)), _fmt_pct2(kpis.get("d_ing", np.nan))],
    ]
    tbl = Table(data, hAlign="LEFT", colWidths=[45*mm, 30*mm, 30*mm, 25*mm])
    ...
    elements.append(tbl); elements.append(Spacer(1, 10))

    elements.append(Paragraph("Análisis PRO", styles["Heading2"]))
    for line in summary_text.split("\n"):
        elements.append(Paragraph(line, p))

    doc.build(elements)
    return buf.getvalue()
# ====== END PRO ANALYSIS HELPERS ======
# ---------------------------
...
# ---------- Cuadro de mando PRO (con ingresos YoY) ----------
    ...
    with st.sidebar:
        st.header("Parámetros – Cuadro de mando PRO")
        cutoff_pro = st.date_input("Fecha de corte", value=date.today(), key="cut_cdm_pro")
        ...
        inv_pro = st.number_input("Sobrescribir inventario (opcional)", min_value=0, value=0, step=1, key="inv_cdm_pro")

    st.subheader("🧠 Análisis PRO (automático y específico)")
    text_pro, kpis_pro = _kai_build_pro_text(
        raw_df=raw,
        cutoff=pd.to_datetime(cutoff_pro),
        start=pd.to_datetime(start_pro),
        end=pd.to_datetime(end_pro),
        props=props_pro if props_pro else None,
        inv_override=int(inv_pro) if inv_pro>0 else None
    )
    st.markdown(text_pro)

    # Export PDF
    st.divider()
    st.subheader("⬇️ Exportar")
    pdf_bytes = _kai_export_pro_pdf(
        text_pro, kpis_pro,
        f"Cuadro de mando PRO – {pd.to_datetime(start_pro).date()} a {pd.to_datetime(end_pro).date()} (corte {pd.to_datetime(cutoff_pro).date()})"
    )
    st.download_button(
        "Descargar PDF PRO",
        data=pdf_bytes,
        file_name=f"CDM_PRO_{pd.to_datetime(start_pro).date()}_{pd.to_datetime(end_pro).date()}_corte_{pd.to_datetime(cutoff_pro).date()}.pdf",

...
# --- Safety defaults for inventory UI variables if not defined (no-op) ---
try:
...
def _read_inventory(file, period_start=None, period_end=None, col_name="Alojamiento"):
    ...
    if period_start is not None and "Desde" in df.columns:
        df = df[(df["Desde"].isna()) or (df["Desde"] <= _pd.to_datetime(period_start))]
    if period_end is not None and "Hasta" in df.columns:
        df = df[(df["Hasta"].isna()) or (df["Hasta"] >= _pd.to_datetime(period_end))]
...
def compute_kpis(
    ...
    # If a declared inventory list is provided, use its length
    ...
    \1    # Reindex to include declared inventory with zeros
    if declared_inventory:
        _decl = sorted(set([str(x).strip() for x in declared_inventory]))
        by_prop = (by_prop.set_index(\"Alojamiento\")
                           .reindex(_decl, fill_value=0)
                           .reset_index())
...
if mode == "Consulta normal":
    ...
    with st.sidebar:
        ...
        if use_declared_inventory:
            ...
            with c_inv2:
                inv_file_ly = st.file_uploader("Inventario LY (opcional)", type=["csv","xlsx"], key="inv_ly_norm")
        declared_list_now = _read_inventory(inv_file_cur, start_normal, end_normal) if use_declared_inventory else None

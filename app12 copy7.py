...
# =============================
    ...
    if run_evo:
        ...
        base = alt.Chart(df_now).encode(x=alt.X("Corte:T", title="Fecha de corte"))

        hover = alt.selection_point(fields=["Corte"], nearest=True, on="mousemove", empty="none")
        ...
        def line_with_hover(data, y_field: str, title: str, color: str, axis=None, dash=False, opacity=1.0):
            ...
            line = (
                ...
                .encode(
                    ...
                    tooltip=[alt.Tooltip("Corte:T", title="Día"), alt.Tooltip(f"{y_field}:Q", title=title, format=".2f")],
                )
            )
            points = (
                alt.Chart(data)
        ...
        vrule = (
            ...
            .add_params(hover)
            .encode(x="Corte:T", opacity=alt.condition(hover, alt.value(1), alt.value(0)))
        )
        ...
        # ----- Ocupación
        if occ_selected:
            l, p = line_with_hover(df_now, "ocupacion_pct", "Ocupación %", "#1f77b4", axis=alt.Axis(titleColor="#1f77b4"))
            layers += [l, p]
            if compare_e and not df_prev.empty:
                l2, p2 = line_with_hover(df_prev, "ocupacion_pct", "Ocupación % (LY)", "#1f77b4", dash=True, opacity=0.35)
                layers += [l2, p2]

        # ----- ADR / RevPAR
        if "ADR (€)" in selected_kpis:
            r, rp = line_with_hover(
                df_now, "adr", "ADR (€)", "#ff7f0e",
                axis=alt.Axis(orient="right", titleColor="#ff7f0e") if occ_selected else alt.Axis(titleColor="#ff7f0e")
            )
            layers += [r, rp]
            if compare_e and not df_prev.empty:
                r2, rp2 = line_with_hover(df_prev, "adr", "ADR (€) (LY)", "#ff7f0e", dash=True, opacity=0.35)
                layers += [r2, rp2]

        if "RevPAR (€)" in selected_kpis:
            g, gp = line_with_hover(
                df_now, "revpar", "RevPAR (€)", "#2ca02c",
                axis=alt.Axis(orient="right", titleColor="#2ca02c") if occ_selected else alt.Axis(titleColor="#2ca02c")
            )
            layers += [g, gp]
            if compare_e and not df_prev.empty:
                g2, gp2 = line_with_hover(df_prev, "revpar", "RevPAR (€) (LY)", "#2ca02c", dash=True, opacity=0.35)
                layers += [g2, gp2]

        chart = (
            alt.layer(*layers, vrule)
            .resolve_scale(y="independent" if (occ_selected and euro_selected) else "shared")

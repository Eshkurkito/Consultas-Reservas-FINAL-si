...
# =============================
    ...
    if run_evo:
        ...
        # Base común + selección para tooltip “nearest”
        base = alt.Chart(df_now).encode(
            x=alt.X("Corte:T", title="Fecha de corte")
        )

        hover = alt.selection_point(
            fields=["Corte"], nearest=True, on="mousemove", empty="none"
        )

        def line_with_hover(data, y_field: str, title: str, color: str, axis=None, dash=False, opacity=1.0):
            mark_kwargs = dict(strokeWidth=2, interpolate="monotone")
            if dash:
                mark_kwargs["strokeDash"] = [5, 3]
            ...
            line = (
                ...
                .encode(
                    x="Corte:T",
                    y=alt.Y(f"{y_field}:Q", title=title, axis=axis),
                    color=alt.value(color),
                    opacity=alt.value(opacity),
                    tooltip=[
                        alt.Tooltip("Corte:T", title="Día"),
                        alt.Tooltip(f"{y_field}:Q", title=title, format=".2f"),
                    ],
                )
            )
            points = (
        ...
        # Línea vertical del hover (a toda la altura)
        vrule = (
            base.mark_rule(color="#666", strokeWidth=1)
            .add_params(hover)
            .encode(
                x="Corte:T",
                opacity=alt.when(hover).then(1).otherwise(0)
            )
        )

        layers = []

        # ¿Hay KPI de € seleccionados?
        euro_flags = {
            "ADR (€)": ("adr",  "#ff7f0e"),
            "RevPAR (€)": ("revpar", "#9467bd"),
        }
        occ_selected  = ("Ocupación %" in selected_kpis)
        euro_selected = any(k in selected_kpis for k in ("ADR (€)", "RevPAR (€)"))

        # ----- Ocupación (eje izquierdo si está seleccionada)
        if occ_selected:
            l, p = line_with_hover(df_now, "ocupacion_pct", "Ocupación %", "#1f77b4",
                                   axis=alt.Axis(titleColor="#1f77b4"))
            layers += [l, p]
            if compare_e and not df_prev.empty:
                l2, p2 = line_with_hover(df_prev, "ocupacion_pct", "Ocupación % (LY)", "#1f77b4",
                                         axis=alt.Axis(titleColor="#1f77b4"), dash=True, opacity=0.35)
                layers += [l2, p2]

        # ----- Métricas en € (ADR / RevPAR)
        # Usamos fold para que ambas compartan UN MISMO eje cuando haya más de una.
        euro_fields = []
        euro_legend_names = {}
        for label, (col, _) in euro_flags.items():
            if label in selected_kpis:
                euro_fields.append(col)
                euro_legend_names[col] = label
        if euro_fields:
            # Dataset “long” para actuales
            euro_now = alt.Chart(df_now).transform_fold(
                euro_fields, as_=["metric", "valor"]
            )
            # Eje: si también está ocupación, los € al eje derecho; si no, al izquierdo (único eje)
            euro_axis = alt.Axis(title="€", orient="right") if occ_selected else alt.Axis(title="€")

            euro_line = (
                euro_now.mark_line(strokeWidth=2, interpolate="monotone")
                .encode(
                    x="Corte:T",
                    y=alt.Y("valor:Q", title="€", axis=euro_axis),
                    color=alt.Color("metric:N",
                                    title="KPI (€)",
                                    scale=alt.Scale(
                                        domain=list(euro_fields),
                                        range=[euro_flags["ADR (€)"][1], euro_flags["RevPAR (€)"][1]][:len(euro_fields)]
                                    ),
                                    legend=alt.Legend(labelExpr=f"datum.label == 'adr' ? '{euro_legend_names.get('adr','ADR (€)')}' : '{euro_legend_names.get('revpar','RevPAR (€)')}'")),
                    tooltip=[alt.Tooltip("Corte:T", title="Día"),
                             alt.Tooltip("valor:Q", title="€", format=".2f")]
                )
            )
            euro_point = (
                euro_now.mark_point(size=60, filled=True)
                .encode(
                    x="Corte:T",
                    y=alt.Y("valor:Q"),
                    color=alt.Color("metric:N", legend=None,
                                    scale=alt.Scale(domain=list(euro_fields),
                                                    range=[euro_flags["ADR (€)"][1], euro_flags["RevPAR (€)"][1]][:len(euro_fields)]))
                )
                .add_params(hover)
                .transform_filter(hover)
            )
            layers += [euro_line, euro_point]

            # LY para € (si procede)
            if compare_e and not df_prev.empty:
                euro_prev = alt.Chart(df_prev).transform_fold(
                    euro_fields, as_=["metric", "valor"]
                )
                euro_line_prev = (
                    euro_prev.mark_line(strokeWidth=1.5, interpolate="monotone", strokeDash=[4, 3])
                    .encode(
                        x="Corte:T",
                        y=alt.Y("valor:Q", title="€", axis=euro_axis),
                        color=alt.Color("metric:N", legend=None,
                ...
                euro_point_prev = (
                    euro_prev.mark_point(size=40, filled=True)
                    .encode(
                        x="Corte:T",
                        y="valor:Q",
                        color=alt.Color("metric:N", legend=None,
                ...
                layers += [euro_line_prev, euro_point_prev]

        chart = alt.layer(*layers, vrule).resolve_scale(
            # Si hay € y Ocupación, los ejes Y son independientes (izda y dcha).
            # Si SOLO hay un tipo, comparten el único eje (no se crea el derecho).
            y="independent" if (occ_selected and euro_fields) else "shared"
        ).properties(height=380)

        # Pan/zoom horizontal en todas las capas
        zoomx = alt.selection_interval(bind="scales", encodings=["x"])
        st.altair_chart(chart.add_params(zoomx), use_container_width=True)

        # ===== Tabla y export rápido =====
        show_cols = ["Corte"]
        rename_map = {}
        if "Ocupación %" in selected_kpis:
            show_cols += ["ocupacion_pct"]; rename_map["ocupacion_pct"] = "Ocupación %"
        if "ADR (€)" in selected_kpis:
            show_cols += ["adr"]; rename_map["adr"] = "ADR (€)"
        if "RevPAR (€)" in selected_kpis:
            show_cols += ["revpar"]; rename_map["revpar"] = "RevPAR (€)"

        df_show = df_now[show_cols].rename(columns=rename_map)

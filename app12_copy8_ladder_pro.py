...
# =============================
        ...
        # ---------- Ladder de precios (rápido, integrado en PRO) ----------
        st.markdown("### 🪜 Ladder de precios (rápido)")
        st.caption("Define escalera por lead time a partir de un ADR base (OTB o P50 histórico).")

        # Intentamos recuperar percentiles (q25, q50, q75) y ADR OTB (adr) calculados arriba
        q25 = locals().get("q25", float("nan"))
        q50 = locals().get("q50", float("nan"))
        q75 = locals().get("q75", float("nan"))
        adr_otb = float(locals().get("adr", float("nan")))  # ADR actual del periodo (OTB)

        with st.expander("⚙️ Configurar ladder", expanded=True):
            base_opt = st.radio(
                "Base del ladder",
                ["ADR OTB actual", "P50 histórico del periodo"],
                horizontal=True,
                key="ladder_base_pro"
            )

            # Base ADR: OTB o P50 (si no hay P50 válido, cae a OTB)
            adr_base = adr_otb if (base_opt == "ADR OTB actual" or not (isinstance(q50, (int, float)) and q50>0)) else float(q50)

            c1, c2, c3 = st.columns([1,1,1])
            with c1:
                round_rule_lad = st.selectbox(
                    "Redondeo",
                    ["Terminar en ,99", "A 1€", "A 5€", "Sin redondeo"],
                    index=0,
                    key="ladder_round_rule"
                )
            with c2:
                adr_min_lad = st.number_input("ADR mínimo (€)", value=35.0, step=1.0, key="ladder_min")
            with c3:
                adr_max_lad = st.number_input("ADR máximo (€)", value=500.0, step=5.0, key="ladder_max")

            st.markdown("**Buckets de lead (días) y multiplicadores**")
            default_mults = {
                "0–3": 0.97, "4–7": 0.99, "8–14": 1.00, "15–21": 1.02,
                "22–30": 1.04, "31–45": 1.06, "46–60": 1.08, "61–90": 1.10
            }
            order = list(default_mults.keys())
            sel = st.multiselect(
                "Selecciona buckets",
                options=order,
                default=order,
            ...
            # Inputs de multiplicadores por bucket (grid responsivo)
            mults = {}
            ncols = 4 if len(sel) >= 4 else max(1, len(sel))
            cols = st.columns(ncols)
            for i, b in enumerate(sel):
                with cols[i % ncols]:
                    mults[b] = st.number_input(
                        f"{b}",
                        value=float(default_mults.get(b, 1.00)),
                        min_value=0.50, max_value=2.00, step=0.01,
                        key=f"ladder_m_{b}"
                    )

        # Función de redondeo
        def _round_price(x, rule: str):
            import numpy as _np
            x = _np.asarray(x, dtype=float)
            if rule == "A 1€":
                return _np.round(x)
            elif rule == "A 5€":
                return (5 * _np.round(x / 5)).astype(float)
            elif rule == "Terminar en ,99":
                return _np.floor(x) + 0.99
            else:
                return x

        # Construcción del ladder
        if sel:
            base_val = float(adr_base if isinstance(adr_base, (int, float)) and adr_base > 0 else 0.0)
            if base_val <= 0:
                st.warning("No hay ADR base válido (OTB o P50). Revisa filtros/periodo.")
            else:
                rows = []
                for b in sel:
                    m = float(mults[b])
                    price_raw = base_val * m
                    price_cap = float(__import__("numpy").clip(price_raw, adr_min_lad, adr_max_lad))
                    price_out = float(_round_price([price_cap], round_rule_lad)[0])
                    rows.append({
                        "Lead (días)": b,
                        "Multiplicador": m,
                        "ADR base (€)": base_val,
                        "Precio bruto (€)": round(price_raw, 2),
                        "Cap min/max": (price_raw != price_cap),
                        "Precio sugerido (€)": round(price_out, 2),
                    })

                df_ladder = pd.DataFrame(rows)
                st.dataframe(df_ladder, use_container_width=True)

                csv_ladder = df_ladder.to_csv(index=False).encode("utf-8-sig")
                st.download_button(
                    "📥 Descargar ladder (CSV)",
                    data=csv_ladder,
                    file_name="ladder_precios.csv",
                    mime="text/csv",
                    key="ladder_csv"
                )

                # Mini-notas de contexto
                note = []
                if isinstance(q25, (int, float)) and isinstance(q75, (int, float)) and q25>0 and q75>0:
                    note.append(f"Bandas ADR (histórico en periodo): P25≈ {q25:.2f} · P50≈ {q50 if isinstance(q50, (int, float)) else float('nan'):.2f} · P75≈ {q75:.2f}")
                if base_opt == "ADR OTB actual":
                    note.append("Base=ADR OTB del periodo.")
                else:
                    note.append("Base=P50 histórico del periodo.")
                st.caption(" · ".join(note))
        else:
            st.info("Selecciona al menos un bucket para generar el ladder.")

    ...
    if run_pro:
        ...
        if not base_pace.empty and not ly_pace.empty:
            ...
            try:
                ...
                if d_today in df_chart["D"].values:
                    _val = float(df_chart.loc[df_chart["D"]==d_today, "Occ % actual"].iloc[0])
                    point_df = pd.DataFrame({"D":[d_today], "serie":["Occ % actual"], "value":[_val]})
                    chart_point = alt.Chart(point_df).mark_point(size=90).encode(
                        x="D:Q", y="value:Q", color="serie:N"
                    )
                    chart = (chart_lines + chart_point).properties(height=260)
                else:

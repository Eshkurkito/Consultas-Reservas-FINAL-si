...
# ===========================
    ...
    # ====== NUEVO: Objetivo LY – ADR mínimo para igualar ingresos LY finales ======
    st.subheader("🎯 Objetivo LY – ADR mínimo para igualar ingresos LY finales")

    # Datos base
    rev_now  = float(tot_now.get("ingresos", 0.0))
    rev_tgt  = float(tot_ly_final.get("ingresos", np.nan))
    nights_otb_now = float(tot_now.get("noches_ocupadas", 0.0))
    disponibles    = float(tot_now.get("noches_disponibles", 0.0))

    # Trayectoria actual (P50) con Pace
    nights_p50 = float(pace_res.get("nights_p50", nights_otb_now)) if isinstance(pace_res, dict) else nights_otb_now
    rev_p50    = float(pace_res.get("revenue_final_p50", rev_now))  if isinstance(pace_res, dict) else rev_now
    occ_p50    = (nights_p50 / disponibles * 100.0) if disponibles > 0 else 0.0

    t1, t2, t3 = st.columns(3)
    t1.metric("Trayectoria P50 – Noches", f"{nights_p50:,.0f}".replace(",", "."))
    t2.metric("Trayectoria P50 – Ocupación", f"{occ_p50:.2f}%")
    t3.metric("Trayectoria P50 – Ingresos", f"{rev_p50:,.2f}")

    # Parámetros del cálculo (locales a esta sección)
    with st.expander("Parámetros de cálculo", expanded=False):
        eps_pro = st.slider("Elasticidad precio (ε)", min_value=0.1, max_value=1.0, value=0.8, step=0.05, key="pro_eps",
                            help="Sensibilidad de volumen al precio en el remanente (0.1–1.0).")
        st.caption("Se usa un modelo simple: rem_sim(ADR) = min(R × (B/ADR)^ε, capacidad_remanente).")

    # Referencia de ADR (mediana robusta entre ADR actual, ADR tail (P50) y P50 del grupo)
    adr_now = float(tot_now.get("adr", np.nan))
    try:
        adr_p50_group = float(adr_bands_p50_for_month(
            df_all=raw,
            cutoff=pd.to_datetime(pro_cut),
            start=pd.to_datetime(pro_start),
            end=pd.to_datetime(pro_end),
            props=(props_pro if props_pro else list(map(str, raw["Alojamiento"].unique()))),
        ))
    except Exception:
        adr_p50_group = np.nan
    adr_ref_candidates = [
        adr_now if np.isfinite(adr_now) and adr_now > 0 else np.nan,
        float(adr_tail_p50) if np.isfinite(adr_tail_p50) and adr_tail_p50 > 0 else np.nan,
        adr_p50_group if np.isfinite(adr_p50_group) and adr_p50_group > 0 else np.nan,
    ]
    adr_ref = float(np.nanmedian(adr_ref_candidates)) if np.isfinite(np.nanmedian(adr_ref_candidates)) else (adr_now if (np.isfinite(adr_now) and adr_now > 0) else 50.0)

    # Remanente base (P50) y capacidad remanente
    R   = max(nights_p50 - nights_otb_now, 0.0)                     # remanentes esperadas P50
    cap = max(disponibles - nights_otb_now, 0.0)                    # límite por capacidad

    # Cálculo del ADR mínimo para alcanzar ingresos LY finales
    def simulate_rev(adr: float) -> dict:
        """Devuelve rev_final, nights_final a un ADR objetivo, con CAP por capacidad."""
        if adr <= 0 or R <= 0 or cap <= 0:
            nf = nights_otb_now
            revf = rev_now
        else:
            rem_unbounded = R * (adr_ref / adr) ** float(eps_pro)
            rem = min(max(rem_unbounded, 0.0), cap)
            nf = nights_otb_now + rem
            revf = rev_now + adr * rem
        occ = (nf / disponibles * 100.0) if disponibles > 0 else 0.0
        return {"rev_final": float(revf), "nights_final": float(nf), "occ_final_pct": float(occ)}

    def solve_min_adr(rev_target: float) -> Optional[float]:
        if not np.isfinite(rev_target):
            return None
        gap = rev_target - rev_now
        if gap <= 0:
            return 0.0  # ya alcanzado
        if R <= 0 or cap <= 0:
            return None  # sin remanente o sin capacidad: inviable

        cands = []

        # Candidato con CAP activo (rem = cap): adr = gap / cap, válido si a ese ADR la demanda >= cap
        adr_cap_thresh = (adr_ref * (R / cap) ** (1.0 / float(eps_pro))) if (R > 0 and cap > 0 and eps_pro > 0) else np.inf
        adr_min_cap = gap / cap if cap > 0 else np.inf
        if np.isfinite(adr_min_cap) and adr_min_cap >= 0 and adr_min_cap <= adr_cap_thresh:
            cands.append(adr_min_cap)

        # Caso sin CAP (solución cerrada si ε != 1): gap = R * adr_ref^ε * ADR^(1−ε)
        if abs(float(eps_pro) - 1.0) > 1e-6 and R > 0 and adr_ref > 0:
            adr_no_cap = (gap / (R * (adr_ref ** float(eps_pro)))) ** (1.0 / (1.0 - float(eps_pro)))
            if np.isfinite(adr_no_cap) and adr_no_cap >= 0:
                cands.append(adr_no_cap)
        else:
            # Búsqueda numérica (bisección)
            lo, hi = 0.01, max(adr_ref * 4.0, (gap / max(R, 1e-6)) * 2.0)
            for _ in range(60):
                mid = (lo + hi) / 2.0
                if simulate_rev(mid)["rev_final"] >= rev_target:
                    hi = mid
                else:
                    lo = mid
            cands.append(hi)

        return max(0.0, min(c for c in cands if np.isfinite(c))) if cands else None

    adr_min_req = solve_min_adr(rev_tgt)

    # Resultados y proyección a ese ADR mínimo
    if not np.isfinite(rev_tgt):
        st.info("No se pudo determinar el objetivo de ingresos LY final para este periodo.")
    else:
        g1, g2, g3 = st.columns(3)
        g1.metric("Objetivo ingresos LY final (€)", f"{rev_tgt:,.2f}")
        g2.metric("Ingresos actuales (€)", f"{rev_now:,.2f}")
        gap_val = rev_tgt - rev_now
        g3.metric("Gap vs objetivo (€)", f"{gap_val:,.2f}")

        if adr_min_req is None:
            st.warning("No se puede alcanzar el objetivo con el modelo actual (sin remanente/capacidad o elasticidad).")
        else:
            proj = simulate_rev(float(adr_min_req))
            c1, c2, c3, c4 = st.columns(4)
            c1.metric("ADR mínimo requerido (€)", f"{float(adr_min_req):.2f}")
            c2.metric("Ocupación proyectada", f"{proj['occ_final_pct']:.2f}%")
            c3.metric("Noches finales proyectadas", f"{proj['nights_final']:,.0f}".replace(",", "."))
            c4.metric("Ingresos finales proyectados (€)", f"{proj['rev_final']:.2f}")

            # Viabilidad rápida: ¿estamos en el borde por CAP?
            rem_unbounded_min = R * (adr_ref / max(float(adr_min_req), 1e-6)) ** float(eps_pro) if R > 0 else 0.0
            if rem_unbounded_min > cap - 1e-6:
                st.caption("ℹ️ El cálculo está limitado por capacidad. Con más inventario no sería necesario un ADR tan alto.")
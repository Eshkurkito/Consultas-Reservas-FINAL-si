...
# ===========================
    ...
    # Referencia ADR más robusta: P50 del apto (histórico del mismo periodo), ADR actual y tail
    p50_by_apto = adr_bands_p50_for_month_by_apto(
        df_all=raw,
        cutoff=pd.to_datetime(sim_cut),
        start=pd.to_datetime(sim_start),
        end=pd.to_datetime(sim_end),
    ...
    adr_p50_apto = float(p50_by_apto.get(sim_prop, np.nan)) if isinstance(p50_by_apto, dict) else np.nan
    adr_ref_candidates = [
        adr_now if np.isfinite(adr_now) and adr_now > 0 else np.nan,
        adr_tail_p50 if np.isfinite(adr_tail_p50) and adr_tail_p50 > 0 else np.nan,
        adr_p50_apto if np.isfinite(adr_p50_apto) and adr_p50_apto > 0 else np.nan,
    ]
    adr_ref = float(np.nanmedian(adr_ref_candidates)) if np.isfinite(np.nanmedian(adr_ref_candidates)) else (adr_now if adr_now > 0 else 50.0)

    # ADR a simular (lo introduces tú)
    default_sim_adr = float(round(adr_ref, 2))
    sim_adr = st.number_input("ADR a simular (€)", min_value=0.0, value=default_sim_adr, step=1.0, key="sim_adr")

    # Simulación con elasticidad (ε acotada ≤ 1.0) y CAP de capacidad también en ingresos
    def simulate_for_adr(adr_target: float) -> dict:
        eps_eff = min(max(float(sim_eps), 0.1), 1.0)  # evita ε>1 que empuja óptimo al mínimo
        max_add_nights = max(disponibles - nights_otb, 0.0)

        if adr_target <= 0 or rem_base <= 0 or max_add_nights <= 0:
            nights_final = nights_otb
            rev_final = rev_now
        else:
            # Ajuste de volumen por precio relativo a la referencia
            base_ref = adr_ref if np.isfinite(adr_ref) and adr_ref > 0 else adr_tail_p50 if adr_tail_p50 > 0 else adr_now if adr_now > 0 else 1.0
            adj_factor = (base_ref / max(adr_target, 1e-6)) ** eps_eff
            rem_sim_raw = max(rem_base * adj_factor, 0.0)
            # CAP por capacidad disponible
            rem_sim = min(rem_sim_raw, max_add_nights)
    ...
    # Curva ADR→Ingresos (rango centrado en adr_ref)
    ref_adr = default_sim_adr
    adr_min = max(ref_adr * 0.6, 40.0)
    adr_max = ref_adr * 1.4
    ...
    # Aviso si el óptimo cae en el borde del rango (recomendación de ampliar rango o ajustar ε)
    if best_row is not None and (abs(float(best_row["adr_target"]) - adr_min) < 1e-6 or abs(float(best_row["adr_target"]) - adr_max) < 1e-6):
        st.caption("ℹ️ El óptimo cae en el borde del rango de búsqueda. Amplía el rango o ajusta la elasticidad.")
...
def compute_kpis(
    ...
    # Inventario considerado para noches disponibles
...
def pace_series(df: pd.DataFrame, period_start: pd.Timestamp, period_end: pd.Timestamp, d_max: int, props: Optional[List[str]], inv_override: Optional[int]) -> pd.DataFrame:
    ...
    inv = len(set(props)) if props else df["Alojamiento"].nunique()
...
# ---------- Predicción (Pace) ----------
    ...
    if run_f:
        res = pace_forecast_month(
            raw,
            pd.to_datetime(cut_f),
            pd.to_datetime(f_start),
            pd.to_datetime(f_end),
            ref_years=int(ref_years),
            dmax=int(dmax_f),
            props=props_f if props_f else None,
            inv_override=int(inv_f) if inv_f > 0 else None,
        )
...
# ===========================
    ...
    pace_res = pace_forecast_month(
        ...
        start=pd.to_datetime(pro_start),
        end=pd.to_datetime(pro_end),
        ref_years=2,
        dmax=180,
    ...
    if raw is None:
        st.warning("⚠️ Sube tus Excel y pulsa 'Usar estos archivos' en la barra lateral.")
        st.stop()

    with st.sidebar:
        st.header("Parámetros – Resumen & Simulador")
        sim_cut = st.date_input("Fecha de corte", value=date.today(), key="sim_cut")
        sim_start, sim_end = period_inputs(
            "Inicio del periodo", "Fin del periodo",
            date(date.today().year, date.today().month, 1),
            (pd.Timestamp.today().to_period("M").end_time).date(),
            "simulador_period"
        )
        props_all = sorted(raw["Alojamiento"].astype(str).unique().tolist())
        sim_prop = st.selectbox("Piso", options=props_all, index=0 if props_all else None, key="sim_prop")
        sim_eps = st.slider("Elasticidad precio (ε)", min_value=0.3, max_value=2.5, value=1.0, step=0.1,
                            help="A mayor ε, más sensible el volumen a cambios de ADR.")

    if not sim_prop:
        st.info("Selecciona un piso para simular.")
        st.stop()

    # Inventario = 1 para el piso seleccionado
    inv_one = 1
    days_period = (pd.to_datetime(sim_end) - pd.to_datetime(sim_start)).days + 1
    disponibles = max(days_period * inv_one, 0)

    # KPIs OTB actuales del piso
    _, tot_now_sim = compute_kpis(
        df_all=raw,
        cutoff=pd.to_datetime(sim_cut),
        period_start=pd.to_datetime(sim_start),
        period_end=pd.to_datetime(sim_end),
        inventory_override=inv_one,
        filter_props=[sim_prop],
    )
    adr_now = float(tot_now_sim["adr"])
    occ_now = float(tot_now_sim["ocupacion_pct"])
    rev_now = float(tot_now_sim["ingresos"])
    nights_otb = float(tot_now_sim["noches_ocupadas"])

    st.subheader("📌 Estado actual del periodo (OTB)")
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("ADR actual (€)", f"{adr_now:.2f}")
    c2.metric("Ocupación actual", f"{occ_now:.2f}%")
    c3.metric("Noches OTB", f"{nights_otb:,.0f}".replace(",", "."))
    c4.metric("Ingresos OTB (€)", f"{rev_now:.2f}")

    # Base estadística Pace (P50) para ese piso
    pace_res_sim = pace_forecast_month(
        df=raw,
        cutoff=pd.to_datetime(sim_cut),
        start=pd.to_datetime(sim_start),
        end=pd.to_datetime(sim_end),
        ref_years=2,
        dmax=180,
        props=[sim_prop],
        inv_override=inv_one,
    )
    nights_p50 = float(pace_res_sim.get("nights_p50", 0.0))
    adr_tail_p50 = float(pace_res_sim.get("adr_tail_p50", adr_now if np.isfinite(adr_now) else 0.0))
    nights_p50 = max(nights_p50, nights_otb)
    rem_base = max(nights_p50 - nights_otb, 0.0)

    st.subheader("🔮 Base estadística (P50)")
    b1, b2, b3 = st.columns(3)
    b1.metric("Noches finales (P50)", f"{nights_p50:,.0f}".replace(",", "."))
    b2.metric("Noches remanentes base", f"{rem_base:,.0f}".replace(",", "."))
    b3.metric("ADR tail remanente (P50)", f"{adr_tail_p50:.2f}")
    st.caption("Las noches remanentes se ajustan por elasticidad cuando simulas un ADR objetivo.")

    # ADR a simular (lo introduces tú)
    default_sim_adr = adr_tail_p50 if np.isfinite(adr_tail_p50) and adr_tail_p50 > 0 else (adr_now if np.isfinite(adr_now) and adr_now > 0 else 50.0)
    sim_adr = st.number_input("ADR a simular (€)", min_value=0.0, value=float(round(default_sim_adr, 2)), step=1.0, key="sim_adr")

    # Simulación con elasticidad
    def simulate_for_adr(adr_target: float) -> dict:
        if adr_target <= 0 or rem_base <= 0:
            nights_final = nights_otb
            rev_final = rev_now
        else:
            adj_factor = (adr_tail_p50 / max(adr_target, 1e-6)) ** float(sim_eps) if np.isfinite(adr_tail_p50) and adr_tail_p50 > 0 else 1.0
            rem_sim = max(rem_base * adj_factor, 0.0)
            nights_final = nights_otb + rem_sim
            rev_final = rev_now + adr_target * rem_sim
        nights_final = min(nights_final, disponibles) if disponibles > 0 else nights_final
        occ_final = (nights_final / disponibles * 100.0) if disponibles > 0 else 0.0
        adr_final = (rev_final / nights_final) if nights_final > 0 else 0.0
        return {"nights_final": float(nights_final), "occ_final_pct": float(occ_final), "rev_final": float(rev_final), "adr_final": float(adr_final)}

    # Curva ADR→Ingresos para hallar el óptimo y veredicto
    ref_adr = default_sim_adr
    adr_min = max(ref_adr * 0.6, 5.0)
    adr_max = ref_adr * 1.4
    grid = np.linspace(adr_min, max(adr_max, adr_min + 1.0), 35)
    df_curve = pd.DataFrame([simulate_for_adr(float(a)) | {"adr_target": float(a)} for a in grid])
    best_row = df_curve.loc[df_curve["rev_final"].idxmax()] if not df_curve.empty else None

    # Resultado del ADR que pones
    sim_res = simulate_for_adr(sim_adr)
    st.subheader("🧪 Resultado simulado")
    s1, s2, s3, s4 = st.columns(4)
    s1.metric("ADR final sim (€)", f"{sim_res['adr_final']:.2f}")
    s2.metric("Ocupación simulada", f"{sim_res['occ_final_pct']:.2f}%")
    s3.metric("Noches finales", f"{sim_res['nights_final']:,.0f}".replace(",", "."))
    s4.metric("Ingresos finales (€)", f"{sim_res['rev_final']:.2f}", delta=f"{(sim_res['rev_final'] - rev_now):+.2f}€")

    # Veredicto del ADR introducido
    if best_row is not None and sim_res["rev_final"] > 0:
        ratio = sim_res["rev_final"] / float(best_row["rev_final"]) if float(best_row["rev_final"]) > 0 else np.nan
        if np.isfinite(ratio):
            if ratio >= 0.98:
                verdict_icon, verdict = "🟢", "Bueno (casi óptimo)"
            elif ratio >= 0.93:
                verdict_icon, verdict = "🟠", "Aceptable"
            else:
                verdict_icon, verdict = "🔴", "No recomendado"
            loss = float(best_row["rev_final"]) - sim_res["rev_final"]
            st.info(f"{verdict_icon} Veredicto ADR {sim_adr:.2f} €: {verdict}. Diferencia vs óptimo ≈ {loss:,.2f} €.".replace(",", "."))
            st.caption(f"Sugerencia: ADR óptimo ≈ {float(best_row['adr_target']):.2f} € → Ingresos {float(best_row['rev_final']):.2f} € · Ocupación {float(best_row['occ_final_pct']):.1f}%")

    # Gráfico
    try:
        base = alt.Chart(df_curve).mark_line(point=True).encode(
            x=alt.X("adr_target:Q", title="ADR simulado (€)"),
            y=alt.Y("rev_final:Q", title="Ingresos finales (€)"),
            tooltip=[
                alt.Tooltip("adr_target:Q", title="ADR (€)", format=",.2f"),
                alt.Tooltip("rev_final:Q", title="Ingresos (€)", format=",.2f"),
                alt.Tooltip("occ_final_pct:Q", title="Ocupación (%)", format=",.1f"),
                alt.Tooltip("nights_final:Q", title="Noches", format=",.0f"),
            ],
        ).properties(height=320)
        sel_pt = alt.Chart(pd.DataFrame([{"adr_target": float(sim_adr), "rev_final": float(sim_res["rev_final"])}])).mark_point(
            size=120, color="#d62728"
        ).encode(x="adr_target:Q", y="rev_final:Q")
        layers = [base, sel_pt]
        if best_row is not None:
            best_pt = alt.Chart(pd.DataFrame([best_row])).mark_point(size=120, color="#2ca02c").encode(x="adr_target:Q", y="rev_final:Q")
            layers.append(best_pt)
        st.altair_chart(alt.layer(*layers), use_container_width=True)
    except Exception:
        st.line_chart(df_curve.set_index("adr_target")[["rev_final"]], height=320)

    with st.expander("Descargar resultados", expanded=False):
        base_row = pd.DataFrame([{
            "Piso": sim_prop,
            "Periodo inicio": pd.to_datetime(sim_start).date(),
            "Periodo fin": pd.to_datetime(sim_end).date(),
            "Corte": pd.to_datetime(sim_cut).date(),
            "ADR_actual": adr_now, "Ocupacion_actual_%": occ_now,
            "Ingresos_OTB": rev_now, "Noches_OTB": nights_otb,
            "Nights_P50_base": nights_p50, "ADR_tail_P50": adr_tail_p50,
            "Elasticidad": sim_eps,
            "ADR_sim": sim_adr, "ADR_final_sim": sim_res["adr_final"],
            "Ocupacion_final_sim_%": sim_res["occ_final_pct"],
            "Noches_finales_sim": sim_res["nights_final"],
            "Ingresos_finales_sim": sim_res["rev_final"],
            "ADR_optimo": float(best_row["adr_target"]) if best_row is not None else np.nan,
            "Ingresos_optimos": float(best_row["rev_final"]) if best_row is not None else np.nan,
        }])
        st.download_button(
            "📥 Descargar simulación (CSV)",
            data=base_row.to_csv(index=False).encode("utf-8-sig"),
            file_name="simulador_resumen.csv",
            mime="text/csv",
        )
# ...existing code...
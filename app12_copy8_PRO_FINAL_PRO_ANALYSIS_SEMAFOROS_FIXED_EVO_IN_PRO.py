...
def render_evolucion_por_corte(
    raw,
    cutoff_actual,
    start_period,
    end_period,
    ...
    inv_now=None,
    *,
    titulo="Evolución de la ocupación por fecha de corte",
    dias_ventana_default=60,
    key_ns="EVO"
):
    import pandas as pd
    import numpy as np
    ...
    c_a, _ = st.columns([1, 3])
    dias_ventana = c_a.slider(
        "Ventana (días antes del inicio del periodo)",
        min_value=7, max_value=180, value=int(dias_ventana_default), step=7,
        key=f"{key_ns}_ventana_dias"
    )

    cutoff_ini = pd.to_datetime(start_period) - pd.Timedelta(days=int(dias_ventana))
    cutoff_fin = pd.to_datetime(cutoff_actual)
    if cutoff_fin < cutoff_ini:
        cutoff_ini = cutoff_fin

    cortes = pd.date_range(cutoff_ini, cutoff_fin, freq="D")

    evo_rows = []
    for c in cortes:
        _, _tot_c = compute_kpis(
            df_all=raw,
            cutoff=pd.to_datetime(c),
            period_start=pd.to_datetime(start_period),
            period_end=pd.to_datetime(end_period),
            inventory_override=(int(inv_now) if (inv_now is not None and inv_now > 0) else None),
            filter_props=(props_sel if props_sel else None),
        )
        evo_rows.append({
            "corte": pd.to_datetime(c).date(),
            "ocupacion_pct": float(_tot_c.get("ocupacion_pct", np.nan)),
            "adr": float(_tot_c.get("adr", np.nan)),
            "revpar": float(_tot_c.get("revpar", np.nan)),
            "ingresos": float(_tot_c.get("ingresos", np.nan)),
        })

    df_evo = pd.DataFrame(evo_rows)

    if df_evo.empty or df_evo["ocupacion_pct"].isna().all():
        st.info("No hay datos suficientes para trazar la evolución por fecha de corte.")
        return

    occ_ini = float(df_evo["ocupacion_pct"].iloc[0])
    occ_fin = float(df_evo["ocupacion_pct"].iloc[-1])
    delta_pp = occ_fin - occ_ini

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Ocupación (inicio ventana)", f"{occ_ini:.1f}%")
    c2.metric("Ocupación (corte actual)", f"{occ_fin:.1f}%")
    c3.metric("Δ p.p. en ventana", f"{delta_pp:+.1f} p.p.")
    c4.metric("Cortes", f"{len(df_evo)}")

    chart_evo = alt.Chart(df_evo).mark_line().encode(
        x=alt.X("corte:T", title="Fecha de corte"),
        y=alt.Y("ocupacion_pct:Q", title="Ocupación %"),
        tooltip=[
            alt.Tooltip("corte:T", title="Corte"),
            alt.Tooltip("ocupacion_pct:Q", title="Ocupación %", format=".1f"),
    ...
    point = alt.Chart(
        pd.DataFrame({"corte":[df_evo["corte"].iloc[-1]], "ocupacion_pct":[occ_fin]})
    ).mark_point(size=110).encode(x="corte:T", y="ocupacion_pct:Q")

    st.altair_chart(chart_evo + point, use_container_width=True)

    tendencia = "🟢 subiendo" if delta_pp > 1 else ("🔴 bajando" if delta_pp < -1 else "🟠 estable")
    st.markdown(
        f"**Lectura rápida:** del {cutoff_ini:%d-%b} al {cutoff_fin:%d-%b} la ocupación del periodo "
        f"pasó de **{occ_ini:.1f}%** a **{occ_fin:.1f}%** (**{delta_pp:+.1f} p.p.**, {tendencia})."
    )
...
# ---------- Cuadro de mando PRO (con ingresos YoY) ----------
    ...
    if run_pro:
        ...
        # === Evolución por fecha de corte embebida en PRO (2 meses por defecto) ===
        render_evolucion_por_corte(
            raw,
            cutoff_actual=cutoff_pro,
            start_period=start_pro,
            end_period=end_pro,
            props_sel=(props_sel if props_sel else None),
            inv_now=(int(inv_now) if inv_now else None),
            titulo="Evolución por fecha de corte (embebida en PRO)",
            dias_ventana_default=60,
            key_ns="EVO_PRO"
        )

            # === Evolución de la ocupación por fecha de corte (últimos 2 meses) ===
        st.markdown("### Evolución de la ocupación por fecha de corte (últimos 2 meses)")
        try:
            # Ventana: 60 días antes del inicio del periodo hasta el corte actual
            cutoff_ini = pd.to_datetime(start_pro) - pd.Timedelta(days=60)
            cutoff_fin = pd.to_datetime(cutoff_pro)
            if cutoff_fin < cutoff_ini:
                cutoff_ini = cutoff_fin
            ...
            evo_rows = []
            for c in cortes:
                # Mantenemos SIEMPRE el mismo periodo [start_pro, end_pro]
                # y solo cambiamos la "fecha de corte" desde la que miramos ese periodo.
                _, _tot_c = compute_kpis(
                    raw,
                    pd.to_datetime(c),
                    pd.to_datetime(start_pro),
                    pd.to_datetime(end_pro),
                    (int(inv_now) if (inv_now is not None and inv_now > 0) else None),
                    (props_sel if props_sel else None),
                )
                evo_rows.append({
                    "corte": pd.to_datetime(c).date(),
                    "ocupacion_pct": float(_tot_c.get("ocupacion_pct", np.nan)),
            ...
            if df_evo.empty or df_evo["ocupacion_pct"].isna().all():
                ...
                # Métricas rápidas: primera vs última y delta en p.p.
                occ_ini = float(df_evo["ocupacion_pct"].iloc[0])
                occ_fin = float(df_evo["ocupacion_pct"].iloc[-1])
                delta_pp = occ_fin - occ_ini

                c1, c2, c3 = st.columns(3)
                c1.metric("Ocupación al inicio de la ventana", f"{occ_ini:.1f}%")
                c2.metric("Ocupación al corte actual", f"{occ_fin:.1f}%")
                c3.metric("Δ p.p. (60 días)", f"{delta_pp:+.1f} p.p.")
                ...
                # Punto del corte actual (última fila)
                point = alt.Chart(
                    pd.DataFrame({"corte":[df_evo["corte"].iloc[-1]], "ocupacion_pct":[occ_fin]})
                ).mark_point(size=110).encode(x="corte:T", y="ocupacion_pct:Q")
            ...
            st.warning(f"No se pudo generar la evolución por fecha de corte: {e}")

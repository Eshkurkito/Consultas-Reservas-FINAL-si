...
# ---------- Cuadro de mando PRO (con ingresos YoY) ----------
    ...
    if run_pro:
        ...
        if not base_pace.empty and not ly_pace.empty:
            ...
            df_chart = comp.set_index("D")[["Occ % actual", "Occ % LY"]]
            fig, ax = plt.subplots(figsize=(6,3))
            df_chart.plot(ax=ax, linewidth=2)
            # Punto de fecha de corte sobre la curva actual
            try:
                d_today = (pd.to_datetime(start_pro) - pd.to_datetime(cutoff_pro)).days
                if d_today in df_chart.index:
                    occ_today = float(df_chart.loc[d_today, "Occ % actual"])
                    ax.scatter(d_today, occ_today, color=ax.get_lines()[0].get_color(), s=80, zorder=5)
            except Exception as _e:
                pass
            ax.set_ylabel("Ocupación %")
            ax.set_xlabel("Días antes de la estancia")
            st.pyplot(fig)

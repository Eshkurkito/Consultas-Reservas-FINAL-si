...
def _kai_fmt_pct(x, decimals=1):
    return "-" if (x is None or pd.isna(x)) else f"{float(x):.{decimals}%}"

def _kai_cdm_pro_analysis(tot_now: dict, tot_ly_cut: dict, tot_ly_final: dict, pace: dict, price_ref_p50: float|None) -> str:
    \"\"\"
    Reescritura del análisis del Cuadro de mando PRO:
    - Actual vs LY a misma fecha de corte
    - Gap vs LY final
    - Señales específicas (precio temprano, trade-off, precio alto sin volumen, pickup difícil)
    - Recomendaciones tácticas
    tot_* son dicts con claves: ocupacion_pct, adr, revpar, ingresos, noches_ocupadas
    \"\"\"
    # Valores actuales
    ...
    # Gap vs LY final
    gap_nights_vs_ly_final = _kai_pct(tot_now.get("noches_ocupadas", np.nan), nights_ly_final)
    gap_ing_vs_ly_final = _kai_pct(ing_now, ing_ly_final)
    ...
    # Referencia de precio del periodo (percentil 50 histórico)
    if price_ref_p50 is not None and np.isfinite(price_ref_p50):
        lines.append(f"- **Referencia ADR P50 histórico** del periodo: {price_ref_p50:.2f}€; ADR actual: {adr_now:.2f}€.")
        if adr_now < price_ref_p50 * 0.90:
            lines.append("- Señal: **posible bajada temprana de tarifa** (ADR < P50 -10%).")
        elif adr_now > price_ref_p50 * 1.15:
            lines.append("- Señal: **tarifa alta** (> P50 +15%). Asegurar visibilidad en días valle.")

    # Trade-offs
    if not pd.isna(d_adr) and not pd.isna(d_occ):
        if d_adr < 0 and d_occ <= 0:
            lines.append("- **Precio bajo sin ganar volumen** → revisar mix y fences; no estamos comprando ocupación de forma eficiente.")
        elif d_adr < 0 and d_occ > 0 and (pd.isna(d_ing) or d_ing <= 0):
            lines.append("- **Volumen barato**: sube ocupación pero el ingreso no acompaña → ajustar descuentos/promos.")
        elif d_adr > 0 and d_occ < 0:
            lines.append("- **Precio alto con penalización de volumen** → proteger días fuertes y bajar micro en huecos sueltos.")

    # Pace & pickup
    if isinstance(pace, dict):
        need = pace.get("pickup_needed_p50", np.nan)
        typ = pace.get("pickup_typ_p50", np.nan)
        tail_adr = pace.get("adr_tail_p50", np.nan)
        if np.isfinite(need) and np.isfinite(typ):
            if need > typ * 1.25:
                lines.append(f"- **Pickup pendiente alto**: faltan {need:.0f} noches; típico P50 ≈ {typ:.0f}. Activar campañas y partners.")
            elif need < typ * 0.75:
                lines.append(f"- **Pickup holgado**: pendiente {need:.0f} < P50 típico {typ:.0f}. Se puede sostener precio en días fuertes.")
        if np.isfinite(tail_adr):
            lines.append(f"- ADR estimado en tail (P50 histórico): {tail_adr:.2f}€.")

    # Recomendaciones accionables
    lines.append("**Acciones recomendadas**")
    lines.append("- Días con **ocupación < objetivo** y **ADR < P50** → subir visibilidad y abrir promos con *fences* (mín. 2 noches, NR).")
    lines.append("- Días con **ocupación > objetivo** y **ADR > P75** → sostener precio; micro-ajustes en huecos de 1 noche.")
    lines.append("- **Test A/B ±5–10%** por tipología; prioriza donde el impacto en ingresos sea mayor por punto de ADR.")
    lines.append("- Vigilar costes de adquisición: dar prioridad al canal directo cuando el **pickup pendiente** sea alto.")

    return "\\n".join(lines)
# ===== END CDM PRO ANALYSIS (Kai upgrade) =====
# ---------------------------
...
# ---------- Cuadro de mando PRO (con ingresos YoY) ----------
    ...
    # --- Kai upgraded analysis (only text) ---

...
# ---------- Evolución por fecha de corte ----------
    ...
    with st.sidebar:
        ...
        metric_choice_e = st.radio("Métrica a graficar (para tabla rápida)", ["Ocupación %", "ADR (€)", "RevPAR (€)"],
                                   horizontal=True, key="metric_evo")
    ...
    if run_evo:
        ...
        if cut_start_ts > cut_end_ts:
            ...
            if df_evo.empty:
                ...
                # --- Serie LY (opcional): calculamos ambas métricas (Occ% y ADR) y alineamos +1 año al eje actual
                if compare_e:
                    rows_prev = []
                ...
                layers = [chart_occ, chart_adr]

                # Añadimos LY si está activado y hay columnas
                if compare_e and ("ocupacion_pct_prev" in df_evo.columns) and ("adr_prev" in df_evo.columns):
                    chart_occ_prev = (
                        alt.Chart(df_evo)
                        .mark_line(strokeWidth=1.5)
                        .encode(
                            x="Corte:T",
                            y=alt.Y("Q:Q", title="Ocupación %"),  # usa el eje izquierdo
                            color=alt.value("#1f77b4"),
                            opacity=alt.value(0.35),
                        )
                        .transform_calculate(Q="datum['ocupacion_pct_prev']")
                    )
                    chart_adr_prev = (
                        alt.Chart(df_evo)
                        .mark_line(strokeDash=[2, 2], strokeWidth=1.5)
                        .encode(
                            x="Corte:T",
                            y=alt.Y("Q:Q", title="ADR (€)", axis=alt.Axis(orient='right')),
                            color=alt.value("#ff7f0e"),
                            opacity=alt.value(0.35),
                        )
                        .transform_calculate(Q="datum['adr_prev']")
                    )
                    layers += [chart_occ_prev, chart_adr_prev]
                ...
                quick_plot = pd.DataFrame({"Actual": df_evo[key_col].values}, index=idx)

                if compare_e:
                    # Si el usuario quiere, sigue mostrando la serie elegida en formato “rápido”
                    prev_col = key_col + "_prev" if key_col + "_prev" in df_evo.columns else None
                    if prev_col:
                        quick_plot["Año anterior"] = df_evo[prev_col].values
                # (Opcional) Si quieres mantener también esta mini-gráfica:
                # st.line_chart(quick_plot, height=220)

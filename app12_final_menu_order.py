...
# =============================
    ...
    with st.sidebar:
        ...
        cutp = st.date_input("Fecha de corte", value=date.today(), key="pro_cut")
        p_start = st.date_input("Inicio periodo", value=date.today().replace(day=1), key="pro_start")
        p_end   = st.date_input("Fin periodo", value=(pd.Timestamp.today().to_period("M").end_time).date(), key="pro_end")
        props = st.multiselect("Alojamientos (opcional)", options=sorted(raw["Alojamiento"].unique()), default=[], key="pro_props")
        inv_override = st.number_input("Inventario (para Occ%)", min_value=0, value=0, step=1, key="pro_inv")
        run_pro = st.button("Actualizar", type="primary")

    if run_pro:
        inv_now = get_inventory(raw, int(inv_override) if inv_override>0 else None)
        # Serie diaria del periodo
        ser = daily_series(raw, pd.to_datetime(cutp), pd.to_datetime(p_start), pd.to_datetime(p_end), props if props else None, inv_now)
        ser['Occ %'] = (ser['noches_ocupadas'] / inv_now * 100.0) if inv_now>0 else 0.0
        ser['ADR día'] = np.where(ser['noches_ocupadas']>0, ser['ingresos']/ser['noches_ocupadas'], np.nan)

        st.subheader("📊 KPIs del periodo")
        kpis = {
            "Noches": int(ser['noches_ocupadas'].sum()),
            "Ingresos €": float(ser['ingresos'].sum()),
            "Occ %": float(np.nanmean(ser['Occ %'])) if len(ser)>0 else 0.0,
            "ADR €": float(np.nanmean(ser['ADR día'])) if len(ser)>0 else 0.0,
            "RevPAR €": float((ser['ingresos'].sum() / (inv_now*len(ser))) if (inv_now>0 and len(ser)>0) else 0.0),
        }
        cols = st.columns(5)
        for i,(k,v) in enumerate(kpis.items()):
            cols[i].metric(k, f"{v:,.2f}" if isinstance(v,float) else f"{v:,}".replace(",", "."))

        st.markdown("—")
        c1,c2 = st.columns(2)
        with c1:
            st.write("Evolución diaria (Ingresos, Occ%)")
            st.line_chart(ser.set_index('Fecha')[['ingresos']], height=260)
            st.line_chart(ser.set_index('Fecha')[['Occ %']], height=200)
        with c2:
            st.write("Distribución LOS & DOW")
            # LOS
            los_df = build_los_distribution(raw, pd.to_datetime(cutp), pd.to_datetime(p_start), pd.to_datetime(p_end), props if props else None)
            st.bar_chart(los_df.set_index('LOS')['reservas'], height=200)
            # DOW heatmap-ready table
            dow_tbl = build_dow_table(ser.copy())
            st.dataframe(dow_tbl, use_container_width=True)

        st.markdown("—")
        st.write("Portal mix (reservas e ingresos)")
        pmix = portal_mix(raw, pd.to_datetime(cutp), pd.to_datetime(p_start), pd.to_datetime(p_end), props if props else None)
        st.dataframe(pmix, use_container_width=True)



# =============================
# Vista: Evolución por corte
# =============================
elif mode == "Evolución por corte":
    if raw is None:
        st.stop()
    from datetime import date, timedelta
    with st.sidebar:
        st.header("Parámetros — Evolución por corte")
        base_cut = st.date_input("Fecha de corte base", value=date.today(), key="evc_base")
        n_cortes = st.number_input("Nº de cortes a comparar", min_value=2, value=4, step=1, key="evc_n")
        step_days = st.number_input("Separación entre cortes (días)", min_value=1, value=7, step=1, key="evc_step")
        p_start = st.date_input("Inicio periodo estancias", value=date.today().replace(day=1), key="evc_start")
        p_end   = st.date_input("Fin periodo estancias", value=(pd.Timestamp.today().to_period('M').end_time).date(), key="evc_end")
        inv_evc = st.number_input("Inventario (para Occ%)", min_value=0, value=0, step=1, key="evc_inv")
        props = st.multiselect("Alojamientos (opcional)", options=sorted(raw["Alojamiento"].unique()), default=[], key="evc_props")
        run_evc = st.button("Comparar", type="primary")

    if run_evc:
        cortes = [pd.to_datetime(base_cut) - pd.to_timedelta(step_days*i, unit='D') for i in range(int(n_cortes))][::-1]
        inv_now = get_inventory(raw, int(inv_evc) if inv_evc>0 else None)

        tbls = []
        for c in cortes:
            ser = daily_series(raw, c, pd.to_datetime(p_start), pd.to_datetime(p_end), props if props else None, inv_now)
            ser['Occ %'] = (ser['noches_ocupadas']/inv_now*100.0) if inv_now>0 else 0.0
            ser['Corte'] = c.date()
            tbls.append(ser[['Fecha','noches_ocupadas','ingresos','Occ %','Corte']])
        evo = pd.concat(tbls, ignore_index=True)

        st.subheader("📈 Evolución por fecha de corte (noches)")
        # pivot para líneas por corte
        pv_n = evo.pivot(index='Fecha', columns='Corte', values='noches_ocupadas').sort_index()
        st.line_chart(pv_n, height=280)

        st.subheader("💶 Evolución por fecha de corte (ingresos)")
        pv_r = evo.pivot(index='Fecha', columns='Corte', values='ingresos').sort_index()
        st.line_chart(pv_r, height=280)

        st.subheader("🧩 Tabla resumida (últimos 60 días del periodo)")
        st.dataframe(evo.tail(60), use_container_width=True)



# =============================
# Vista: Ranking de alojamientos
# =============================
elif mode == "Ranking de alojamientos":
    if raw is None:
        st.stop()
    from datetime import date
    with st.sidebar:
        st.header("Parámetros — Ranking")
        cutr = st.date_input("Fecha de corte", value=date.today(), key="rk_cut")
        r_start = st.date_input("Inicio periodo", value=date.today().replace(day=1), key="rk_start")
        r_end   = st.date_input("Fin periodo", value=(pd.Timestamp.today().to_period("M").end_time).date(), key="rk_end")
        inv_rk = st.number_input("Inventario (para Occ%)", min_value=0, value=0, step=1, key="rk_inv")
        run_rk = st.button("Ver ranking", type="primary")

    if run_rk:
        # Ranking por alojamiento usando helper compute_kpis si existe
        try:
            by_prop, total = compute_kpis(
                df_all=raw,
                cutoff=pd.to_datetime(cutr),
                period_start=pd.to_datetime(r_start),
                period_end=pd.to_datetime(r_end),
                inventory_override=int(inv_rk) if inv_rk>0 else None,
                filter_props=None,
            )
            st.dataframe(by_prop.sort_values(["revpar","adr"], ascending=[False, False]), use_container_width=True)
        except Exception as e:
            # Fallback: agrupación directa
            dfp = raw.copy()
            dfp["Fecha entrada"] = pd.to_datetime(dfp["Fecha entrada"])
            dfp["Fecha salida"] = pd.to_datetime(dfp["Fecha salida"])
            dfp["Fecha alta"] = pd.to_datetime(dfp["Fecha alta"])
            dfp = dfp[(dfp["Fecha alta"] <= pd.to_datetime(cutr))]
            # expand a noches del periodo
            rows = []
            for _,r in dfp.iterrows():
                s = max(r["Fecha entrada"].normalize(), pd.to_datetime(r_start))
                e = min(r["Fecha salida"].normalize(), pd.to_datetime(r_end) + pd.Timedelta(days=1))
                if pd.isna(s) or pd.isna(e) or s>=e:
                    continue
                los = (e - s).days
                adr = r["Precio"]/los if los>0 else None
                for d in pd.date_range(s, e - pd.Timedelta(days=1), freq="D"):
                    rows.append((r["Alojamiento"], d, adr))
            if rows:
                df_n = pd.DataFrame(rows, columns=["Alojamiento","Fecha","ADR"])
                agg = df_n.groupby("Alojamiento").agg(noches=("Fecha","size"), adr=("ADR","mean")).reset_index()
                inv = int(inv_rk) if inv_rk>0 else 1
                dias = (pd.to_datetime(r_end) - pd.to_datetime(r_start)).days + 1
                agg["revpar"] = (agg["adr"] * (agg["noches"]/(inv*dias))).fillna(0.0)
                st.dataframe(agg.sort_values(["revpar","adr"], ascending=[False, False]), use_container_width=True)
            else:
    ...
    from datetime import date
    with st.sidebar:
        st.header("Parámetros Pickup & Pace")
        cut1 = st.date_input("Fecha de corte 1", value=date.today(), key="pp_cut1")
        cut2 = st.date_input("Fecha de corte 2", value=date.today(), key="pp_cut2")
        p_start = st.date_input("Inicio periodo", value=date.today().replace(day=1), key="pp_start")
        p_end   = st.date_input("Fin periodo", value=(pd.Timestamp.today().to_period("M").end_time).date(), key="pp_end")
        inv_pp = st.number_input("Inventario (para Occ%)", min_value=0, value=0, step=1, key="pp_inv")
        props_pp = st.multiselect("Alojamientos (opcional)", options=sorted(raw["Alojamiento"].unique()), default=[], key="pp_props")
        run_pp = st.button("Calcular", type="primary")

    if run_pp:
        inv_now = get_inventory(raw, int(inv_pp) if inv_pp>0 else None)
        # Serie por día a corte 1 y 2
        ser1 = daily_series(raw, pd.to_datetime(cut1), pd.to_datetime(p_start), pd.to_datetime(p_end), props_pp if props_pp else None, inv_now)
        ser2 = daily_series(raw, pd.to_datetime(cut2), pd.to_datetime(p_start), pd.to_datetime(p_end), props_pp if props_pp else None, inv_now)
        ser1['Occ %'] = np.where(inv_now>0, ser1['noches_ocupadas']/inv_now*100.0, 0.0)
        ser2['Occ %'] = np.where(inv_now>0, ser2['noches_ocupadas']/inv_now*100.0, 0.0)
        st.subheader("📈 Pickup entre cortes")
        pickup = ser2[['Fecha','noches_ocupadas','ingresos']].merge(ser1[['Fecha','noches_ocupadas','ingresos']], on='Fecha', suffixes=('_2','_1'))
        pickup['Pickup noches'] = pickup['noches_ocupadas_2'] - pickup['noches_ocupadas_1']
        pickup['Pickup ingresos'] = pickup['ingresos_2'] - pickup['ingresos_1']
        st.line_chart(pickup.set_index('Fecha')[['Pickup noches','Pickup ingresos']], height=280)
        st.dataframe(pickup[['Fecha','Pickup noches','Pickup ingresos']].tail(60), use_container_width=True)

        st.subheader("⏱️ Pace (curva D)")
        # Pace desde cutoff 2 como referencia
        dmax = int((pd.to_datetime(p_end) - pd.to_datetime(cut2)).days)
        pace = pace_series(raw, pd.to_datetime(p_start), pd.to_datetime(p_end), dmax, props_pp if props_pp else None, inv_now)
        st.line_chart(pace.set_index('D')[['F','F_hist_mean']].dropna(), height=280)
        st.caption("F = fracción del total alcanzada a cada D (días hasta entrada).")

# =============================
# Vista combinada: Ranking & Eficiencia
# =============================
elif mode == "Ranking & Eficiencia":
    if raw is None:
        st.stop()
    with st.sidebar:
        st.header("Parámetros Ranking & Eficiencia")
        cut_px = st.date_input("Fecha de corte", value=date.today(), key="re_cut")
        px_start = st.date_input("Inicio periodo", value=date.today().replace(day=1), key="re_start")
        px_end = st.date_input("Fin periodo", value=(pd.Timestamp.today().to_period("M").end_time).date(), key="re_end")
        inv_px = st.number_input("Inventario (para Occ%)", min_value=0, value=0, step=1, key="re_inv")
        run_re = st.button("Ver", type="primary")
    st.subheader("🏆 Ranking & 💸 Eficiencia diaria")
    if run_re:
        # Ranking por alojamiento en el periodo
        by_prop, total = compute_kpis(
            df_all=raw,
            cutoff=pd.to_datetime(cut_px),
            period_start=pd.to_datetime(px_start),
            period_end=pd.to_datetime(px_end),
            inventory_override=int(inv_px) if inv_px>0 else None,
            filter_props=None,
        )
        st.dataframe(by_prop.sort_values("revpar", ascending=False), use_container_width=True)
